...
const GetGuardSettings = async () => {
    ...
    var table = document.getElementById("ayarlar");
    _result.map((item, index) => {
        txt = item.value ? Array.isArray(item.value) ? item.type == "cogul" ?  `${item.value.length || 0} adet kullanıcı tokeni bulunuyor.` :  item.value.map(x => x).join(", ") : item.value == true ? "Aktif!" : item.value : "Deaktif!"
        ...
        if(item.type == "açmalı" || item.type == "type") {
            td2.innerHTML = `	
...
<label class="custom-switch-btn" for="${item.name}_d" style="align-items: left;"></label>
        ...
        if(item.type == "string" || item.type == "limit") {
...
const GetStatSettings = async () => {
    ...
    _result.filter(x => x.type != "kategori").map((item, index) => {
        txt = item.value ? Array.isArray(item.value) ? item.value.map(x => x).join(", ") : item.value == true ? "Aktif!" : item.value : "Deaktif!"
...
async function get_backup() {
    ...
    link.download = `Synlio-${String(JSON.parse(_result).Date)}.takachi`;
    link.click();
...
async function connection(t) {
    Swal.fire({
        title: `Bağlanamadı!`,
        text: `API ile bağlantı sağlanamadı, lütfen kontrol edip yeniden deneyin.`,
        type: "error",
        showCancelButton: 0,
        confirmButtonColor: "#DD6B55",
...
async function checkGuild() {
    let _get  = await fetch(`${API.url}/get?type=99`, {
        ...
        document.title = `apollon ~ Web Panel [Bağlanılmadı!]` 
        let sunucuAra = document.getElementById("sunucuAra")
        sunucuAra.innerHTML = `Sunucu: Bağlanılamadı! <span class="badge bg-danger"><i class="fa-solid fa-globe fa-beat-fade" style="--fa-beat-fade-opacity: 0.1; --fa-beat-fade-scale: 1.25; color: "></i></span>`
        connection();
    })
    ...
    let saniye = 0
    let sunucuAra = document.getElementById("sunucuAra")
    document.title = `apollon ~ Web Panel [${_result ? _result.name : "Bağlanılmadı!"}]` 
    sunucuAra.innerHTML = `Sunucu: ${_result.name} <span class="badge bg-success"><i class="fa-solid fa-globe fa-beat-fade" style="--fa-beat-fade-opacity: 0.1; --fa-beat-fade-scale: 1.25; color: "></i></span>`
...
async function checkServer() {
    ...
    if(!_result) return;
    let sunucuAra = document.getElementById("sunucuAra")
    if(sunucuAra) sunucuAra.innerHTML = `Sunucu: ${_result.name} <span class="badge bg-success"><i class="fa-solid fa-globe fa-beat-fade" style="--fa-beat-fade-opacity: 0.1; --fa-beat-fade-scale: 1.25; color: "></i></span>`
...
async function refreshStatSettings() {
    ...
    let newSettings = await fetch(`${API.url}/get?type=6`, {
        ...
        w3.filter(x => x.type != "kategori").map((item, int) => {
            let txt = item.value ? Array.isArray(item.value) ? item.value.map(x => x).join(", ") : item.value == true ? "Aktif!" : item.value : "Deaktif!"
...
async function refreshGuardSettings() {
    ...
    let newSettings = await fetch(`${API.url}/get?type=9`, {
        ...
        w3 = await x.json()
        w3.map((item, int) => {
            let txt = item.value ? Array.isArray(item.value) ? item.type == "cogul" ?  `${item.value.length || 0} adet kullanıcı tokeni bulunuyor.` :  item.value.map(x => x).join(", ") : item.value == true ? "Aktif!" : item.value : "Deaktif!"
