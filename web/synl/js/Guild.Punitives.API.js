    ...
    if(_get && t) {
        $(document).ready(function() {
            _get.sort((a, b) => Date.parse(b.Date) - Date.parse(a.Date)).forEach(async (x, index) => {
                ...
                t.row.add( [
                        `${index + 1}`,
                        `${member.avatarURL ? `<img class="server-profile2" style="width: 30px" src="${member.avatarURL}"></img>` : ""} ${member.username}`,
                        `${staff ? staff.avatarURL ? `<img class="server-profile2" style="width: 30px" src="${staff.avatarURL}"></img> ${staff.username}` : staff.displayAvatarURL ? `<img class="server-profile2" style="width: 30px" src="${staff.displayAvatarURL}"></img> ${staff.tag}` : `${staff.tag}` : `Yetkili Bulunamadı!`}`,
