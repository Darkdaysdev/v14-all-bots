...
async function patchBots(token, id, username = null, type, input) {
    if(type == "AVATAR") {
...
async function GetBots() {
    ...
    if(_result) {
       _result.sort((a, b) => Number(b.id) - Number(a.id)).forEach((bot) => {
        ...
        td2.innerHTML = `<center>${bot.username}</center>`
    
            td3.innerHTML = `<center><div class="input-group mb-3 input-primary">
          
            <button id="${bot.id}_avatarbutton" class="btn btn-success btn-md" onclick="patchBots('${bot.token}', '${bot.id}' ,'${bot.username}', 'AVATAR');" type="button">Güncelle</button>
 ...
 <button class="btn btn-secondary btn-sm" id="${bot.id}_isimbutton" onclick="patchBots('${bot.token}', '${bot.id}' ,'${bot.user.username}', 'USERNAME', '${bot.username}_username');" type="button">Güncelle</button>
</div>
        <input type="text" id="${bot.user.username}_username" style="width: 1px" class="form-control" placeholder="${bot.user.username}">
    </div>`
            ...
            async function refreshBotStatus() {
                ...
                let newSettings = await fetch(`${API.url}/get?type=7`, {
                    ...
                    w3.sort((a, b) => Number(b.id) - Number(a.id)).map((bot, int) => {
                        ...
                        table.rows[1 + int].cells[1].innerHTML = `<center>${bot.user.username}</center>`
