...
GuildMember.prototype.recordSetRoles = function (roles = []) {
  ...
  this.roles.cache
    ...
    .map((e) => rol.push(e.id));
  rol.push(...roles);
  if (roller.vipRolü && this.roles.cache.has(roller.vipRolü))
    rol.push(roller.vipRolü);
  if (
    ayarlar.type &&
    roller.tagRolü &&
    !this.roles.cache.has(roller.tagRolü) &&
    this.user.globalName.includes(ayarlar.tag)
  )
    rol.push(roller.tagRolü);
  if (ayarlar.type && roller.tagRolü && this.roles.cache.has(roller.tagRolü))
    rol.push(roller.tagRolü);
  if (
    roller.Buttons &&
    roller.Buttons.vk &&
    this.roles.cache.has(roller.Buttons.vk)
  )
    rol.push(roller.Buttons.vk);
  if (
    roller.Buttons &&
    roller.Buttons.dc &&
    this.roles.cache.has(roller.Buttons.dc)
  )
    rol.push(roller.Buttons.dc);
  if (
    roller.etkinlikKatılımcısı &&
    this.roles.cache.has(roller.etkinlikKatılımcısı)
  )
    rol.push(roller.etkinlikKatılımcısı);
  if (
    roller.cekilisKatılımcısı &&
    this.roles.cache.has(roller.cekilisKatılımcısı)
  )
...
GuildMember.prototype.Register = async function (
  name,
  gender = undefined,
  ...
  let rolver;
  if (gender == "Erkek") {
    rol = roller.erkekRolleri
      .map((x) => this.guild.roles.cache.get(x))
      .join(",");
    rolver = roller.erkekRolleri;
  } else if (gender == "Kadın") {
    rol = roller.kadınRolleri
      .map((x) => this.guild.roles.cache.get(x))
      .join(",");
    rolver = roller.kadınRolleri;
  }
  await this.recordSetRoles(rolver);
  await GUILDS_SETTINGS.updateOne(
    { guildID: this.guild.id },
    { $set: { [`Caches.lastRecord`]: this.id } },
    { upsert: true },
  );
  let seskanal = registrant.guild.channels.cache
    .filter(
      (x) =>
        x.parentId == kanallar.publicKategorisi &&
        x.type == ChannelType.GuildVoice &&
        x.id != kanallar.sleepRoom &&
        _statSystem.musicRooms &&
        _statSystem.musicRooms.some((kanal) => x.id != kanal),
    )
    .random();
  ...
  if (this && this.voice && this.voice.channel && otomatikTaşıma) {
    setTimeout(() => {
      if (this)
        this.send({
          content: `**Sunucumuza Hoşgeldiniz!** ${ayarlar.serverName} sunucusuna kayıt olduğunuz için Teşekkür Ederiz!
...
Sana İyi Sohbetler Dilerim! :tada: :tada: :tada:`,
        }).catch((err) => {});
  ...
  if (kanallar.chatKanalı) {
    ...
    if (ayarlar.type) {
      if (kanal) {
        if (this.user.globalName.includes(ayarlar.tag))
          kanal
            .send(
              `:tada: ${this} ailemize katıldı! ailemize hoş geldin, İyi Eğlenceler.`,
            )
            .then((x) => {
              setTimeout(() => {
                x.delete();
        ...
        if (!this.user.globalName.includes(ayarlar.tag))
          kanal
            .send(
              `:tada: ${this} aramıza katıldı! Sen de bizden biri olmak ister misin? O zaman ${ayarlar.serverName} tagı (\` ${ayarlar.tag} \`) almalısın, şimdiden iyi eğlenceler.`,
            )
            .then((x) => {
              setTimeout(() => {
                x.delete();
  ...
  await Users.updateOne(
    { _id: this.id },
    {
      $set: { Name: name, Gender: gender, Registrant: registrant.id },
      $push: {
        Names: {
          Staff: registrant.id,
          Name: name,
  ...
  await Users.updateOne(
    { _id: registrant.id },
    { $push: { Records: { User: this.id, Gender: gender, Date: Date.now() } } },
  );
    ...
    kayıtLog.send({
      embeds: [
        new genEmbed().setDescription(
          `${this} isimli üye ${registrant} yetkili tarafından <t:${String(Date.now()).slice(0, 10)}:R> **${gender}** olarak \`${this.guild.name}\` sunucusuna kayıt edildi.`,
        ),
      ],
    });
};
...
GuildMember.prototype.Rename = async function (
  name,
  registrant = undefined,
  state = undefined,
) {
...
GuildMember.prototype.removeStaff = async function (lastRole, manuel) {
  ...
  if (userData && userData.Staff && userData.StaffGiveAdmin) {
    ...
    if (Yetkili && Yetkili.Staffs) {
      const LanYarramBakSikildiler = this.guild.members.cache.get(
        userData.StaffGiveAdmin,
      );
      if (LanYarramBakSikildiler && !manuel)
        LanYarramBakSikildiler.send({
          content: `
${this} (**${this.user.username}**)
ID: ${this.id}`,
          embeds: [
            new genEmbed().setDescription(
              `Merhaba ${LanYarramBakSikildiler} sunucumuza ${this} kullanıcısını yetkili olarak başlatmışsınız fakat <t:${String(Date.now()).slice(0, 10)}:R> yetkiyi bıraktı.`,
            ),
          ],
        }).catch((err) => {});
      const findUser = Yetkili.Staffs.find((takachi) => takachi.id == this.id);
      await Users.updateOne(
        { _id: userData.StaffGiveAdmin },
        { $pull: { Staffs: findUser } },
        { upsert: true },
      );
      client.Upstaffs.removePoint(
        userData.StaffGiveAdmin,
        -_statSystem.points.staff,
        "Yetkili",
      );
    }
  }
  if (userData && userData.Staff) {
    await Users.updateOne(
      { _id: this.id },
      { $set: { Staff: false, StaffGiveAdmin: new String() } },
      { upsert: true },
    );
    ...
    await Stats.updateOne(
      ...
      {
        upstaffVoiceStats: new Map(),
        upstaffChatStats: new Map(),
        taskVoiceStats: new Map(),
      },
      { upsert: true },
    );
    ...
    if (!manuel) {
      ...
      let arr = [];
      lastRole
        .filter((rol) => altYetki.position <= rol.position)
        .forEach(async (rol) => {
      ...
      await Unleash.updateOne(
        { _id: this.id },
        { $set: { unleashRoles: arr }, $inc: { unleashPoint: 1 } },
        { upsert: true },
      );
      let yetkiyiBırakan = this.guild.kanalBul("yetki-bırakan");
      let embeds = new genEmbed().setDescription(
        `${this} isimli kullanıcı yetkiyi <t:${String(Date.now()).slice(0, 10)}:R> saldı.`,
      );
      if (Data) {
      ...
      embeds.addFields({
        name: "Son rolleri şunlardır",
        value: `${arr.map((x) => this.guild.roles.cache.get(x)).join(", ")}`,
      });
      if (yetkiyiBırakan)
        yetkiyiBırakan
          .send({
            content: `${this} [**\` ${this.id} \`**]
${[...roller.altYönetimRolleri, ...roller.yönetimRolleri, ...roller.üstYönetimRolleri].filter((x) => this.guild.roles.cache.get(x)).map((x) => this.guild.roles.cache.get(x))}`,
            embeds: [embeds],
            components: [Row],
          })
          .then((msg) => {
            ...
            collector.on("collect", async (i) => {
              ...
              if (kapan) {
                ...
                this.send({
                  content: `Merhaba! ${this}
Sunucumuz da yetkiyi bıraktığınız için çok üzgünüz. Neden bıraktığınızı bilmiyoruz bizlere belirtmek ister misiniz?
O zaman bana ${kapan} (${kapan.user.username} | ${kapan.id}) yazarsanız derdinize deva olabiliriz.`,
                })
                  .then((a) => {
                    i.reply({
                      content: `Başarıyla ${this} üyesine yetkiyi bıraktığı için DM üzerinden mesaj ile bildirildi. Yine de sen yazmak ister misin?`,
                      ephemeral: true,
                    });
                  })
                  .catch(async (err) => {
...
İlgilenmeye çalıştığınız kullanıcının **DM** kapalı olduğu için ben mesaj gönderemedim. Arkadaş ekleyip sen atmak ister misin?`);
                ...
                kapan.Leaders("Yetkili", 5.0, {
                  type: "STAFF_CHECK",
                  user: this.id,
                });
                await USERS_COMPONENTS.updateOne(
                  { _id: kapan.id },
                  {
                    $push: {
                      Checks: {
...
GuildMember.prototype.removeTagged = async function () {
  ...
  if (userData && userData.Tagged && userData.TaggedGiveAdmin) {
    ...
    if (Yetkili && Yetkili.Taggeds) {
      const koşKoşBabanıSikiyorlar = this.guild.members.cache.get(
        userData.TaggedGiveAdmin,
      );
      const findUser = Yetkili.Taggeds.find((takachi) => takachi.id == this.id);
      await Users.updateOne(
        { _id: userData.TaggedGiveAdmin },
        { $pull: { Taggeds: findUser } },
        { upsert: true },
      );
      client.Upstaffs.removePoint(
        userData.TaggedGiveAdmin,
        -_statSystem.points.tagged,
        "Taglı",
      );
    }
    await Users.updateOne(
      { _id: this.id },
      { $set: { Tagged: false, TaggedGiveAdmin: new String() } },
      { upsert: true },
    );
  }
};
...
GuildMember.prototype.dangerRegistrant = async function () {
  ...
  if (userData && userData.Name && userData.Registrant) {
    ...
    if (Yetkili && Yetkili.Records) {
      const koşAnanıSikiyorlar = this.guild.members.cache.get(
        userData.Registrant,
      );
      if (koşAnanıSikiyorlar)
        koşAnanıSikiyorlar
          .send({
            content: `Kayıt ettiğiniz ${this} (${this.user.username}) (**Ceza Puanı**: \`${await this.cezaPuan()}\`) üyesi sunucuda **Kural Dışı** eylem sergiledi ve cezalandırıldı.
Bunun sonucunda sana düşen kayıt ettiğiniz üyenin sorumluluğunu almalısın ve uyarmalısın.`,
          })
          .catch((err) => {});
    }
  }
...
GuildMember.prototype._views = async function () {
  return new Promise(async (resolve, reject) => {
    ...
    resolve(
      `Başarıyla ${this.user.username} üyesinin görüntülemesi ${_cached.Views || 0} oldu.`,
    );
  });
};
...
GuildMember.prototype.Delete = async function (type = "default") {
  ...
  if (userData && userData.Name && userData.Registrant) {
    ...
    if (Yetkili && Yetkili.Records) {
      ...
      const findUser = Yetkili.Records.find(
        (takachi) => takachi.User == this.id,
      );
      await Users.updateOne(
        { _id: userData.Registrant },
        { $pull: { Records: findUser } },
        { upsert: true },
      );
      await client.Upstaffs.removePoint(
        userData.Registrant,
...
GuildMember.prototype.yetkiliCezaPuan = async function (action, number) {
  ...
  if (data && action == "process") {
    ...
    await Users.updateOne(
      { _id: this.id },
      { $set: { staffCriminalPoint: Number(newPoint) } },
    );
    data = await Users.findOne({ _id: this.id });
    return `${Number(data.staffCriminalPoint)}`;
  } else {
...
GuildMember.prototype.cezaPuan = async function (check, güm) {
  ...
  let kalkamayanAmcıkBan =
    filArray.filter((x) => x == "Kalkmaz Yasaklama").length || 0;
  ...
  let cezaPuanı =
    ...
    Underworld * 20 +
    TeyitYasakli * 20 +
    kalkamayanAmcıkBan * 50 +
    Warn * 2;
  if (check) {
    let logKanalı =
      this.guild.kanalBul("ceza-puan") || this.guild.kanalBul("oto-ceza");
    if (
      roller.Yetkiler &&
      roller.Yetkiler.some((x) => this.roles.cache.has(x)) &&
      cezaPuanı >= 50
    ) {
      if (cezaPuanı >= 52) {
      ...
      if (yetkiliCezaPuanı == "BITTI") {
        ...
        if (altYetki)
          await this.roles.remove(
            this.roles.cache.filter((rol) => altYetki.position <= rol.position),
          );
        await this.yetkiliCezaPuan("reset");
        this.setRoles(roller.şüpheliRolü);
        if (logKanalı)
          return logKanalı.send({
            content: `${this} Yetkilisinin Yetkili Ceza Notu Düştüğü İçin Yaptığı Kural-Dışı Eylemlere Yaptırım Uygulanarak Yetkisi Çekildi Ve Şüpheli Konumuna Getirildi.`,
          });
      }

      if (logKanalı)
        logKanalı.send({
          content: `${this} Yetkilisinin Ceza Puanı Güncellendi. Fakat **50** Ceza Puanını Geçtiği İçin "Yetkili Ceza Notu" Sisteminden Düşüş Sağlanacak.
Düşüm Sonrası Yetkili Ceza Notu **0** Altına İndiğinde Otomatik Olarak Yetkisi Çekilecektir. Yetkili Ceza Notu: \`${yetkiliCezaPuanı}\` (Anlık Ceza Puanı: \`${cezaPuanı}\`)`,
      ...
      if (logKanalı)
        logKanalı.send({
          content: `${this} Yetkilisinin Ceza Puanı Güncellendi. Yetkili Ceza Notu: \`${yetkiliCezaPuanı}\` (Anlık Ceza Puanı: \`${cezaPuanı}\`)`,
        });
    }
    ...
    if (
      ...
      if (logKanalı)
        logKanalı.send({
          content: `${this} Üyesi **50** Ceza Puanını ${güm ? `Geçtiği İçin Otomatik Olarak Şüpheli Hesap Konumuna Gönderildi.` : `Geçti Fakat Son Atılan Cezası Şüpheli Konumundan Daha Üstün Olduğu İçin Gönderilemedi.`} Anlık Ceza Puanı: \`${cezaPuanı}\``,
        });
    } else if (
      roller.Yetkiler &&
      !roller.Yetkiler.some((x) => this.roles.cache.has(x))
      ...
      if (logKanalı)
        logKanalı.send({
          content: `${this} Üyesinin Ceza Puanı Güncellendi. Anlık Ceza Puanı: \`${cezaPuanı}\``,
        });
    }
  }
  return cezaPuanı;
...
GuildMember.prototype.Left = async function (lastRole) {
  ...
  if (userData && userData.Tagged && userData.TaggedGiveAdmin) {
    let Yetkili =
      (await Users.findOne({ _id: userData.TaggedGiveAdmin })) || {};
    if (Yetkili && Yetkili.Taggeds) {
      const koşKoşBabanıSikiyorlar = this.guild.members.cache.get(
        userData.TaggedGiveAdmin,
      );
  ...
  if (userData && userData.Staff && userData.StaffGiveAdmin) {
    let Yetkili = (await Users.findOne({ _id: userData.StaffGiveAdmin })) || {};
    if (Yetkili && Yetkili.Staffs) {
      const BabanıSikeyim = this.guild.members.cache.get(
        userData.StaffGiveAdmin,
      );
      if (BabanıSikeyim)
        BabanıSikeyim.send({
          embeds: [
            new genEmbed().setDescription(
              `${BabanıSikeyim} Merhaba, ${this} (\`${this.id}\`) isimli çektiğin yetkili yetkiyi sunucudan çıkarak <t:${String(Date.now()).slice(0, 10)}:R> bıraktı.`,
            ),
          ],
        }).catch((err) => {});
      const findUser = Yetkili.Staffs.find((takachi) => takachi.id == this.id);
  ...
  if (userData && userData.Staff) {
    ...
    if (yetkiyiBırakan)
      yetkiyiBırakan.send({
        content: `${this} [**\` ${this.id} \`**]
${[...roller.altYönetimRolleri, ...roller.yönetimRolleri, ...roller.üstYönetimRolleri].map((x) => this.guild.roles.cache.get(x))}`,
        embeds: [
          new genEmbed()
            .setDescription(
              `${this} isimli eski yetkili, yetkiyi sunucudan ayrılarak **<t:${String(Date.now()).slice(0, 10)}:R>** yetkisini saldı.`,
            )
            .addFields({
              name: "Son rolleri şunlardır",
              value: `${arr.map((x) => this.guild.roles.cache.get(x)).join(", ")}`,
...
GuildMember.prototype.Leaders = async function (
  text,
  point,
  option = {
    type: "MORE",
  ...
  if (
    roles &&
    roles.length > 0 &&
    roles.some((b) => this.roles.cache.has(b.role))
  ) {
    let Data = await Users.findOne({ _id: this.id });
    if (!Data) return;
    let detay = {};
    ...
    if (option.type == "TAGGED")
      detay = {
        type: "TAGLI",
        channel: option.user,
      };
    if (option.type == "TAG_CHECK")
      detay = {
        type: "TAG SALAN İLE İLGİLENME",
        channel: option.user,
      };
    if (option.type == "STAFF_CHECK")
      detay = {
    ...
    if (option.type == "ETKINLIK")
      detay = {
        type: "ETKİNLİK BAŞLATMA",
      };
    if (option.type == "KONSER")
      detay = {
    ...
    for (let index = 0; index < roles.length; index++) {
      ...
      await Users.updateOne(
        { _id: this.id },
        { $inc: { [`Responsibilitys.${sorumluluk.role}`]: point } },
      );
      Data = await Users.findOne({ _id: this.id });
      ...
      if (isimne.includes("lider")) {
        ...
        isimne.includes("sorumlu") ||
        isimne.includes("sorumluluğu") ||
        isimne.includes("sorum")
      ) {
      ...
      let embed = new genEmbed()
        .setAuthor({
          name: `${this.displayName} ${header} Puan'ı Kazandı!`,
...
Yapılan İşlem Türü İse "**${detay.type}**" Olarak Tespit Edildi.`,
      ...
      if (option.user) {
        ...
        embed.addFields({
          ...
          value: `${target ? `${target} (\`${target.id}\`)` : `<@${option.user}> (\`Sunucudan Ayrılmış\`)`}`,
        });
      }
      if (option.channel) {
        let target =
          this.guild.channels.cache.get(option.channel) ||
          this.guild.channels.cache.get(option.channel.id);
        embed.addFields({
          name: `İşlem Yapılan Kanal`,
          value: `${target ? `${target}` : `@deleted-channel`}`,
        });
      }
      if (option.role) {
        ...
        embed.addFields({
          ...
          value: `${target ? `${target}` : `@deleted-roles`}`,
        });
      }
...
GuildMember.prototype.addPunitives = async function (
  type,
  staff,
    ...
    if (findedChannel)
      findedChannel.send({
        embeds: [
          new genEmbed()
            .setFooter({
              text: `${ayarlar ? (ayarlar.embedSettings ? (ayarlar.embedSettings.Footer ? `${ayarlar.embedSettings.Footer}` : message.guild.name) : message.guild.name) : message.guild.name} • Ceza Numarası: #${cezano}`,
              iconURL: this.user.avatarURL({ extension: "png" }),
            })
            .setDescription(
              `${this.toString()} üyesine <t:${String(Date.now()).slice(0, 10)}:F> tarihinde ${duration ? `${moment.duration(ms(duration)).format("Y [Yıl,] M [Ay,] d [Gün,] h [Saat,] m [Dakika]")} boyunca` : ``} \`${reason}\` nedeniyle ${staff.toString()} tarafından "${type}" türünde ceza işlemi uygulandı.`,
            ),
        ],
      });
  }
  if (nofi == 1 && !muteFlood && !rulesFlood)
    await message
      .reply(
        `Başarıyla ${this.toString()} isimli kullanıcıya "__${type}__" işlemi uygulandı. (\`Ceza Numarası: #${cezano}\`)`,
      )
      .then((a) => {
        setTimeout(() => {
          a.delete().catch((err) => {});
  ...
  await this.send({
    content: `Sunucumuz da \`${staff.user.username}\` tarafından **${reason}** sebebi ile ${
            ...
            : type == "Ses Susturulma"
              ...
              : type == "Metin Susturulma"
                ...
                : type == "Underworld"
                  ...
                  : type == "Uyarılma"
                    ? "uyarıldın."
                    : `"${type}" türünde ceza-i işlem uygulandı.`
    } ${duration ? `Ceza bitiş tarihi \`${tarihsel(Date.now() + ms(duration))}\`. Eğer bu konu hakkında bir itirazın var ise üst yetkililerimize ulaşmaktan çekinme!` : "Eğer bu konu hakkında bir itirazın var ise üst yetkililerimize ulaşmaktan çekinme!"} (**Ceza Numarası**: \` #${cezano} \`)`,
  }).catch((err) => {});
  if (nofi == 0 && !muteFlood && !rulesFlood)
    await message.editReply({
      embeds: [
        new genEmbed()
          .setDescription(
            `${this.toString()} isimli üyeye **${reason}** sebebiyle ${duration ? `\`${moment.duration(ms(duration)).format("Y [Yıl,] M [Ay,] d [Gün,] h [Saat,] m [Dakika]")}\` boyunca` : ``} ${
              type == "Kalkmaz Yasaklama"
                ? "tamamiyle kalkmaz yasaklamaya tabi tutuldun."
                : type == "Yasaklama"
                  ? "yasakladın."
          ...
          .setFooter({
            name: `${this.user.username} • Ceza Numarası: #${cezano} • ${type}`,
            iconURL: this.user.avatarURL({ extension: "png" }),
          }),
      ],
      components: [],
  ...
  switch (type) {
    case "Kalkmaz Yasaklama": {
      await this.guild.members
        .ban(this.id, { reason: `#${ceza.No} (${ceza.Reason})` })
        .catch((err) => {});
      await Users.updateOne(
        { _id: staff.id },
        { $inc: { "Uses.Forceban": 1 } },
    ...
    case "Yasaklama": {
      await this.guild.members
        ...
        .catch((err) => {});
      return (
        await Users.updateOne({ _id: staff.id }, { $inc: { "Uses.Ban": 1 } }),
        this.cezaPuan(true)
      );
    }
    case "Atılma": {
    ...
    case "Streamer Cezalandırma": {
      ...
      if (this && this.manageable && roller.streamerCezalıRolü)
        await this.roles.add(roller.streamerCezalıRolü).catch((err) => {});
    ...
    case "VK Cezalandırma": {
      await this.voice.disconnect().catch((err) => {});
      if (this && this.manageable && roller.vkCezalıRolü)
        await this.roles.add(roller.vkCezalıRolü).catch((err) => {});
    ...
    case "Etkinlik Cezalandırma": {
      await this.voice.disconnect().catch((err) => {});
      if (this && this.manageable && roller.etkinlikCezalıRolü)
        await this.roles.add(roller.etkinlikCezalıRolü).catch((err) => {});
      await Users.updateOne(
        { _id: staff.id },
        { $inc: { "Uses.Etkinlik": 1 } },
      );
    ...
    case "DC Cezalandırma": {
      await this.voice.disconnect().catch((err) => {});
      if (this && this.manageable && roller.dcCezalıRolü)
        await this.roles.add(roller.dcCezalıRolü).catch((err) => {});
    ...
    case "Uyarılma": {
      return (
        await Users.updateOne({ _id: staff.id }, { $inc: { "Uses.Warns": 1 } }),
        this.cezaPuan(true, true)
      );
    }
  }
};
