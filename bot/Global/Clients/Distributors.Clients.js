...
class takachi extends Client {
  ...
  async fetchCommands(active = true) {
    ...
    this.logger.log(
      `${black.bgHex("#D9A384")(this.botName.toUpperCase())} ${dirs.length} category in client loaded.`,
      "category",
    );
    dirs.forEach((dir) => {
      let files = fs
        .readdirSync(`../../Server/${this.botName}/_commands/${dir}`, {
          encoding: "utf8",
        })
        .filter((file) => file.endsWith(".js"));
      this.logger.log(
        `${black.bgHex("#D9A384")(this.botName.toUpperCase())} ${files.length} commands loaded in ${dir} category.`,
        "load",
      );
      files.forEach((file) => {
        let referans = require(
          `../../Server/${this.botName}/_commands/${dir}/${file}`,
        );
        if (
          referans.onLoad != undefined &&
          typeof referans.onLoad == "function"
        )
          referans.onLoad(this);
        this.commands.set(referans.Isim, referans);
        if (referans.Komut)
          referans.Komut.forEach((alias) => this.aliases.set(alias, referans));
      });
    });
  }
  ...
  async fetchEvents(active = true) {
    ...
    dirs.forEach((dir) => {
      let files = fs
        .readdirSync(`../../Server/${this.botName}/_events/${dir}`, {
          encoding: "utf8",
        })
        .filter((file) => file.endsWith(".js"));
      files.forEach((file) => {
        let referans = require(
          `../../Server/${this.botName}/_events/${dir}/${file}`,
        );
  ...
  async startDistributors() {
    ...
    if (
      ...
      setInterval(async () => {
        ...
        let log = this.channels.cache.find(
          (x) => x.name.includes("guard") || x.name == "guard-log",
        );
        if (log)
          log.send({
            content: `@everyone`,
            embeds: [
              new EmbedBuilder()
                ...
                .setAuthor({
                  name: guild.name,
                  iconURL: guild.iconURL({ dynmaic: true }),
                })
                .setDescription(
                  `**${guild.name}** sunucusunun **Özel URL** değiştiğinden dolayı güvenlik amacıyla sistem üzerinde belirlenen "${Data.spamURL}" urlsi otomatik olarak tekrar güncellendi.`,
                )
                .setFooter({
                  text: `Synl.io > Genel Bot Ayarları > Güvenlik Ayarları > URL Tekrarlayıcı | Tarafından Bu Ayarları Kontrol Edebilirsiniz.`,
                  iconURL:
                    "https://cdn.discordapp.com/attachments/1034818711921643560/1049216437727801414/favicon.png",
                }),
            ],
          });
    ...
    sistem.TOKENS.SECURITY.DISTS.forEach(async (token) => {
      ...
      botClient.on("ready", async () => {
        ...
        if (!guild) {
          console.log(
            `https://discord.com/api/oauth2/authorize?client_id=${botClient.user.id}&permissions=0&scope=bot%20applications.commands`,
          );
          return;
        }
        this.logger.log(
          `${botClient.user.username} isimli dağıtıcı başarıyla aktif oldu.`,
          "dist",
        );
      ...
      await botClient.login(token).catch((err) => {
        this.logger.log(
          `${black.bgHex("#D9A384")("Dağıtıcı Token Arızası" + ` : ${token}`)}`,
          "error",
        );
      });
    });
  ...
  async checkMember(id, type, process = "İşlem Bulunamadı.") {
    ...
    if (
      uye.id === this.user.id ||
      uye.id === uye.guild.ownerId ||
      Whitelist.unManageable.some(
        (g) => uye.id === g || uye.roles.cache.has(g),
      ) ||
      Whitelist.BOTS.some((g) => uye.id === g || uye.roles.cache.has(g)) ||
      guildSettings.staff.includes(uye.id)
    )
      return true;
    ...
    switch (type) {
      case "guild": {
        if (
          Whitelist.fullAccess.some(
            (g) => uye.id === g || uye.roles.cache.has(g),
          ) ||
          Whitelist.guildAccess.some(
            (g) => uye.id === g || uye.roles.cache.has(g),
          )
        )
          return this.checkProcessLimit(
            uye,
            Whitelist.auditLimit,
            Whitelist.auditInLimitTime,
            process,
          );
        return false;
      }
...
${limitController
  ...
  .map((x, index) => `${index + 1}. | ${x.proc} | ${tarihsel(x.date)}`)
  .join("\n")}
                \`\`\``;
  ...
  async queryManage(oldData, newData) {
    ...
    if (sunucuData) {
      ...
      if (oldData === sunucuData.şüpheliRolü) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.şüpheliRolü": newData } },
          { upsert: true },
        );
      }
      if (oldData === sunucuData.yasaklıTagRolü) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.yasaklıTagRolü": newData } },
          { upsert: true },
        );
      }
      ...
      if (oldData === sunucuData.altilkyetki) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.altilkyetki": newData } },
          { upsert: true },
        );
      }
      if (oldData === sunucuData.etkinlikKatılımcısı) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.etkinlikKatılımcısı": newData } },
          { upsert: true },
        );
      }
      if (oldData === sunucuData.cekilisKatılımcısı) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.cekilisKatılımcısı": newData } },
          { upsert: true },
        );
      }
      ...
      if (oldData === sunucuData.kurallarKanalı) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.kurallarKanalı": newData } },
          { upsert: true },
        );
      }
      if (oldData === sunucuData.hoşgeldinKanalı) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.hoşgeldinKanalı": newData } },
          { upsert: true },
        );
      }
      if (oldData === sunucuData.chatKanalı) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.chatKanalı": newData } },
          { upsert: true },
        );
      }
      if (oldData === sunucuData.toplantıKanalı) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.toplantıKanalı": newData } },
          { upsert: true },
        );
      }
      if (oldData === sunucuData.davetKanalı) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.davetKanalı": newData } },
          { upsert: true },
        );
      }
      if (oldData === sunucuData.publicKategorisi) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.publicKategorisi": newData } },
          { upsert: true },
        );
      }
      if (oldData === sunucuData.registerKategorisi) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.registerKategorisi": newData } },
          { upsert: true },
        );
      }
      if (oldData === sunucuData.streamerKategorisi) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.streamerKategorisi": newData } },
          { upsert: true },
        );
      }
      if (oldData === sunucuData.photoChatKanalı) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.photoChatKanalı": newData } },
          { upsert: true },
        );
      }
      ...
      if (oldData === sunucuData.başlangıçYetki) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.başlangıçYetki": newData } },
          { upsert: true },
        );
      }
      if (
        sunucuData.erkekRolleri &&
        sunucuData.erkekRolleri.includes(oldData)
      ) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.erkekRolleri": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.erkekRolleri": newData } },
          { upsert: true },
        );
      }
      if (
        sunucuData.kadınRolleri &&
        sunucuData.kadınRolleri.includes(oldData)
      ) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.kadınRolleri": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.kadınRolleri": newData } },
          { upsert: true },
        );
      }
      if (
        sunucuData.kayıtsızRolleri &&
        sunucuData.kayıtsızRolleri.includes(oldData)
      ) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.kayıtsızRolleri": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.kayıtsızRolleri": newData } },
          { upsert: true },
        );
      }
      if (sunucuData.Yetkiler && sunucuData.Yetkiler.includes(oldData)) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.Yetkiler": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.Yetkiler": newData } },
          { upsert: true },
        );
      }
      if (
        sunucuData.teyitciRolleri &&
        sunucuData.teyitciRolleri.includes(oldData)
      ) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.teyitciRolleri": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.teyitciRolleri": newData } },
          { upsert: true },
        );
      }
      if (
        sunucuData.kurucuRolleri &&
        sunucuData.kurucuRolleri.includes(oldData)
      ) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.kurucuRolleri": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.kurucuRolleri": newData } },
          { upsert: true },
        );
      }
      if (
        sunucuData.ayrıkKanallar &&
        sunucuData.ayrıkKanallar.includes(oldData)
      ) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.ayrıkKanallar": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.ayrıkKanallar": newData } },
          { upsert: true },
        );
      }
      if (
        sunucuData.izinliKanallar &&
        sunucuData.izinliKanallar.includes(oldData)
      ) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.izinliKanallar": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.izinliKanallar": newData } },
          { upsert: true },
        );
      }
      if (
        sunucuData.rolPanelRolleri &&
        sunucuData.rolPanelRolleri.includes(oldData)
      ) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.rolPanelRolleri": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.rolPanelRolleri": newData } },
          { upsert: true },
        );
      }
      if (
        sunucuData.üstYönetimRolleri &&
        sunucuData.üstYönetimRolleri.includes(oldData)
      ) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.üstYönetimRolleri": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.üstYönetimRolleri": newData } },
          { upsert: true },
        );
      }
      if (
        sunucuData.altYönetimRolleri &&
        sunucuData.altYönetimRolleri.includes(oldData)
      ) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.altYönetimRolleri": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.altYönetimRolleri": newData } },
          { upsert: true },
        );
      }
      if (
        sunucuData.yönetimRolleri &&
        sunucuData.yönetimRolleri.includes(oldData)
      ) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.yönetimRolleri": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.yönetimRolleri": newData } },
          { upsert: true },
        );
      }
      ...
      if (sunucuData.banHammer && sunucuData.banHammer.includes(oldData)) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.banHammer": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.banHammer": newData } },
          { upsert: true },
        );
      }
      if (sunucuData.jailHammer && sunucuData.jailHammer.includes(oldData)) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.jailHammer": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.jailHammer": newData } },
          { upsert: true },
        );
      }
      if (
        sunucuData.voiceMuteHammer &&
        sunucuData.voiceMuteHammer.includes(oldData)
      ) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.voiceMuteHammer": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.voiceMuteHammer": newData } },
          { upsert: true },
        );
      }
      if (sunucuData.muteHammer && sunucuData.muteHammer.includes(oldData)) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.muteHammer": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.muteHammer": newData } },
          { upsert: true },
        );
      }
      if (sunucuData.warnHammer && sunucuData.warnHammer.includes(oldData)) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.warnHammer": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.warnHammer": newData } },
          { upsert: true },
        );
      }
      if (sunucuData.coinChat && sunucuData.coinChat.includes(oldData)) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.coinChat": oldData } },
          { upsert: true },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.coinChat": newData } },
          { upsert: true },
        );
      }
    }
  ...
  rolVer(sunucu, role) {
    ...
    Distributors.forEach((guard, _index) => {
      const members = uyelercik.splice(
        0,
        _index == 0 ? sayı + countUser : sayı,
      );
      if (members.length <= 0)
        return client.logger.log(
          `[${role}] Olayında kayıtlı üye olmadığından veya rol üyelerine dağıtıldığından dolayı rol dağıtımı gerçekleştirmedim.`,
          "log",
        );
      guard.queryTasks.query(async () => {
        return new Promise(async (resolve) => {
          for (let index = 0; index < members.length; index++) {
            if (!role) {
              client.logger.log(
                `[${role}] - ${bot.user.username} - Rol Silindi Dağıtım İptal`,
                "log",
              );
              break;
            }
            let uyeid = members[index];
            let uye = guard.guilds.cache
              .get(global.sistem.SERVER.ID)
              .members.cache.get(uyeid);
            if (!uye || (uye && uye.roles.cache.has(role.id))) continue;
            await uye.roles.add(role.id).catch(() => {
              client.logger.log(
                `${uye.user.username} - Üyesine rol verilemedi.`,
                "log",
              );
            });
          }
  ...
  rolKur(role, newRole) {
    GUILD_ROLE_DATAS.findOne({ roleID: role }, async (err, data) => {
      ...
      const sayı = Math.floor(length / Distributors.length);
      if (sayı < 1) {
        const channelPerm = data.channelOverwrites.filter((e) =>
          client.guilds.cache.get(sistem.SERVER.ID).channels.cache.get(e.id),
        );
        for await (const perm of channelPerm) {
          ...
          kanal.permissionOverwrites
            .create(newRole, newPerm)
      ...
      let countUser = length % Dists;
      Distributors.forEach((guard, _index) => {
        const members = data.members.splice(
          0,
          _index == 0 ? sayı + countUser : sayı,
        );
        if (members.length <= 0)
      ...
      const newData = new GUILD_ROLE_DATAS({
        ...
        channelOverwrites: data.channelOverwrites.filter((e) =>
          newRole.guild.channels.cache.get(e.id),
        ),
      });
      newData.save();
    });
  ...
  async punitivesAdd(id, type) {
    ...
    if (type == "jail") {
      ...
      return (await uye.roles.cache.has(roller.boosterRolü))
        ? uye.roles.set([roller.boosterRolü, roller.şüpheliRolü])
        : uye.roles.set([roller.şüpheliRolü]).catch((err) => {});
    }

    if (type == "ban")
      return await uye
        .ban({ reason: "Guard Tarafından Siki Tuttu." })
        .catch((err) => {});
  }
  ...
  async allPermissionClose() {
    ...
    let roller = sunucu.roles.cache
      .filter((rol) => rol.manageable)
      .filter((rol) => perms.some((yetki) => rol.permissions.has(yetki)));
    roller.forEach(async (rol) => {
      await Roles.updateOne(
        { Role: rol.id },
        {
          $set: {
            guildID: sunucu.id,
            Reason: "Guard Tarafından Devre-Dışı Kaldı!",
            PermissionsBitFieldBitField: rol.permissions.bitfield,
          },
        },
        { upsert: true },
      );
          ...
          .send({
            embeds: [
              new EmbedBuilder().setColor("Yellow")
                .setDescription(`**Merhaba!** ${sunucu.name}
...
Aşağıda bulunan düğme ile tekrardan aktif edebilirsin. Bunun için sunucu sahibi veya bot sahibi olmalısın.`),
            ],
            components: [Rows],
          })
          .then(async (msg) => {
            ...
            var filter = (i) =>
              i.customId == "proc_off" &&
              (ayarlar.staff.includes(i.user.id) || i.user.id === tacsahip.id);
            ...
            collector.on("collect", async (i) => {
              ...
              if (checkRoles)
                checkRoles
                  .filter((x) => msg.guild.roles.cache.get(x.Role))
                  .forEach(async (data) => {
                    let rolgetir = msg.guild.roles.cache.get(data.Role);
                    if (rolgetir)
                      rolgetir
                        .setPermissionsBitFieldBitField(
                          data.PermissionsBitFieldBitField,
                        )
                        .catch((err) => {});
                  });
              Rows.components[0]
                .setStyle(ButtonStyle.Success)
                .setLabel("Başarıyla Rol İzinleri Aktif Edildi")
                .setDisabled(true);
...
${checkRoles.length >= 1 ? `\` ••❯ \` **İzinleri Tekrardan Açılan Rol(ler)**:\n` + checkRoles.map((x) => `\` • \` ${sunucu.roles.cache.get(x.Role)} (\`${x.Role}\`)`).join("\n") : `\` • \` İzinleri Kapatılan Rol Bulunamadı!`}`),
  ...
  connect(token) {
    if (!token) {
      this.logger.log(
        `${black.bgHex("#D9A384")(this.botName.toUpperCase())} Tokeni girilmediğinden dolayı bot kapanıyor...`,
        "error",
      );
      process.exit();
      return;
    ...
    this.login(token)
      .then((takachi) => {
        this.logger.log(
          `${black.bgHex("#D9A384")(this.botName.toUpperCase())} BOT kullanıma aktif edilmiştir.`,
          "botReady",
        );
        ...
        this.on("ready", async () => {
          ...
          this._statSystem =
            global._statSystem = require("../../Global/Plugins/Staff/Sources/_settings");
          await this.startDistributors();
          let guild = client.guilds.cache.get(global.sistem.SERVER.ID);
          if (!guild) {
            console.log(
              `https://discord.com/api/oauth2/authorize?client_id=${this.user.id}&permissions=0&scope=bot%20applications.commands`,
            );
            return;
          }
          ...
          this.user.setPresence({
            activities: [
              {
                name: this.sistem.botStatus.Name,
                url: "https://www.twitch.tv/kxgutsu",
          ...
          const channel = client.channels.cache.get(
            ayarlar.botSesKanalı
              ? ayarlar.botSesKanalı
              : this.system.botStatus.BOT_SES,
          );
          joinVoiceChannel({
      ...
      .catch((takachi) => {
        this.logger.log(
          `${black.bgHex("#D9A384")(this.botName.toUpperCase())} Botun tokeni doğrulanamadı. 5 Saniye sonra tekrardan denenecektir...`,
          "reconnecting",
        );
        setTimeout(() => {
          this.login().catch((takachi) => {
            this.logger.log(
              `${black.bgHex("#D9A384")(this.botName.toUpperCase())} => Bot tokeni tamamiyle doğrulanamadı.. Bot kapanıyor...`,
              "error",
            );
            process.exit();
          });
