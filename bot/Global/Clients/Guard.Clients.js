  ...
  async fetchCommands(active = true, slash = false) {
    if (slash) {
      const slashcommands = await globPromise(
        `../../Server/${this.botName}/_slashcommands/*/*.js`,
      );
      const arrSlash = [];
  ...
  async queryManage(oldData, newData) {
    ...
    if (sunucuData) {
      ...
      if (oldData === sunucuData.yasaklıtagrol) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.yasaklıtagrol": newData } },
        );
      }
      ...
      if (oldData === sunucuData.teyitkanali) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $set: { "Ayarlar.teyitkanali": newData } },
        );
      }
      ...
      if (sunucuData.man.includes(oldData)) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.man": oldData } },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.man": newData } },
        );
      }
      if (sunucuData.woman.includes(oldData)) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.woman": oldData } },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.woman": newData } },
        );
      }
      if (sunucuData.unregister.includes(oldData)) {
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $pull: { "Ayarlar.unregister": oldData } },
        );
        await guildSettings.updateOne(
          { guildID: sistem.SERVER.ID },
          { $push: { "Ayarlar.unregister": newData } },
        );
      }
      ...
      await Roles.updateOne(
        { Role: rol.id },
        {
          $set: {
            ...
            PermissionsBitField: rol.permissions.bitfield,
          },
        },
        { upsert: true },
      );
              ...
              if (checkRoles)
                checkRoles
                  ...
                  .forEach(async (data) => {
                    ...
                    if (rolgetir)
                      rolgetir
                        .setPermissionsBitField(data.PermissionsBitField)
                        .catch((err) => {});
                  });
              Rows.components[0]
    ...
    this.login(token)
      .then((takachi) => {
        ...
        this.on("ready", async () => {
          ...
          let Whitelist = await GUARD_SETTINGS.findOne({
            guildID: sistem.SERVER.ID,
          });
          if (!Whitelist)
            await GUARD_SETTINGS.updateOne(
              { guildID: sistem.SERVER.ID },
              { $set: { auditLimit: 10, auditInLimitTime: "2m" } },
              { upsert: true },
            );
        });
      })
