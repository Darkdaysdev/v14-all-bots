...
const tarihsel = (global.tarihsel = function (tarih) {
  let tarihci =
    moment(tarih).tz("Europe/Istanbul").format("DD") +
    " " +
    global.aylar[moment(tarih).tz("Europe/Istanbul").format("MM")] +
    " " +
    moment(tarih).tz("Europe/Istanbul").format("YYYY HH:mm");
  return tarihci;
});

const kalanzaman = (global.kalanzaman = function (tarih) {
  return moment
    .duration(tarih - Date.now())
