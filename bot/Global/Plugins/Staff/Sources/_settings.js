let _chats = [
  {
    id: ayarlar.chatKanalı
      ? client.channels.cache.get(ayarlar.chatKanalı)
        ? client.channels.cache.get(ayarlar.chatKanalı).parentId
        : "x"
      : "x",
    isim: `${ayarlar.serverName} Chat`,
  },
...
module.exports = {
  generalChatCategory: _chats.find(
    (x) => x.isim == `${ayarlar.serverName} Chat`,
  ).id,
  ...
  fullPointChannels: [
    ..._voices
      .filter(
        (x) =>
          x.isim.includes("Public") ||
          x.isim.includes("Kayıt") ||
          x.isim.includes("Sorun") ||
          x.isim.includes("Streamer"),
      )
      .map((x) => x.id),
