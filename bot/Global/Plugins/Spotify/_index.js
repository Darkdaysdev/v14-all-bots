...
module.exports = class Spotify {
  ...
  setAlbum(name) {
    if (!name || typeof name !== "string")
      throw new Error("The argument of the setAlbum method must be a string.");
    this.album = name;
    return this;
  }
  ...
  setAuthor(name) {
    if (!name || typeof name !== "string")
      throw new Error("The argument of the setAuthor method must be a string.");
    this.artist = name;
    return this;
  }
    ...
    if (type === "color" && typeof value === "string") {
      if (value) {
        if (/^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$/.test(value)) {
          ...
          throw new Error(
            "Invalid color for the second argument in setBackground method. You must give a hexadecimal color.",
          );
        }
      } else {
        throw new Error(
  ...
  setForegroundOpacity(opacity = 0.8) {
    if (opacity) {
      if (opacity >= 0 && opacity <= 1) {
        ...
        throw new Error(
          "The value of the opacity of setForegroundOpacity method must be between 0 and 1 (0 and 1 included).",
        );
      }
    }
  }
  ...
  setImage(image) {
    if (!image)
      throw new Error(
        "The argument of the setAuthor method must be a string or a Buffer or a Canvas.Image.",
      );
    this.image = image;
    return this;
  }
  ...
  setTitle(title) {
    if (!title || typeof title !== "string")
      throw new Error("The argument of the setTitle method must be a string.");
    this.title = title;
    return this;
  }
  ...
  setTimestamp(start, end) {
    if (!start || typeof start !== "number")
      throw new Error(
        "The first argument of the setTimestamp method must be a number.",
      );
  ...
  async build() {
    ...
    const start_format = Util.format_time(
