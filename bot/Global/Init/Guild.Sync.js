...
const {
  ...
  CategoryChannels,
  Roles,
} = require("../Databases/Schemas/Guards/Backup/Guild.Sync");
...
class guildBackup {
    ...
    if (guild) {
      ...
      for (let index = 0; index < channels.length; index++) {
        ...
        if (channel.type === ChannelType.GuildText) {
          await GUILD_TEXT.findOne(
            { channelID: channel.id },
            async (err, kanalYedek) => {
              if (!kanalYedek) {
            ...
            async (err, kanalYedek) => {
              if (!kanalYedek) {
                ...
                (kanalYedek.name = channel.name),
                  ...
                  (kanalYedek.parentID = channel.parentId),
                  (kanalYedek.position = channel.position),
                  (kanalYedek.userLimit = channel.userLimit
                    ? channel.userLimit
                    : 0),
                  (kanalYedek.overwrites = ChannelPermissions);
                kanalYedek.save();
      ...
      await client.logger.log(
        `${tarihsel(Date.now())} tarihinde kanal senkronizasyonu yapıldı.`,
        "backup",
      );
    }
  }
  ...
  static async guildRoles() {
    ...
    await client.logger.log(
      `${tarihsel(Date.now())} tarihinde rol senkronizasyonu yapıldı.`,
      "backup",
    );
  }
}
