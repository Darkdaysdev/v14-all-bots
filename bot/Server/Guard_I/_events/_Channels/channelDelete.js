...
module.exports = async (channel) => {
  ...
  if (
    ...
    (await client.checkMember(entry.executor.id, "channels", "Kanal Silme!"))
  )
    return;
  ...
  if (channel.type === ChannelType.GuildCategory) {
    ...
    await TextChannels.updateMany(
      { parentID: channel.id },
      { parentID: newChannel.id },
    );
    textChannels.forEach((c) => {
      const textChannel = channel.guild.channels.cache.get(c.channelID);
      if (textChannel)
        textChannel.setParent(newChannel, { lockPermissions: false });
    });
    const voiceChannels = await VoiceChannels.find({ parentID: channel.id });
    await VoiceChannels.updateMany(
      { parentID: channel.id },
      { parentID: newChannel.id },
    );
    voiceChannels.forEach((c) => {
      ...
      if (voiceChannel)
        voiceChannel.setParent(newChannel, { lockPermissions: false });
    });
  }
  ...
  embed.setDescription(
    `${entry.executor} (\`${entry.executor.id}\`) tarafından \`#${channel.name}\` isimli kanal silindi ve geri oluşturularak yapan kişi yasaklandı.`,
  );
