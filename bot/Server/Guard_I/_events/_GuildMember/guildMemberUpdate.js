...
module.exports = async (oldMember, newMember) => {
  ...
  if (newMember.roles.cache.size > oldMember.roles.cache.size) {
    let entry = await newMember.guild
      .fetchAuditLogs({ type: AuditLogEvent.MemberRoleUpdate })
    ...
    if (
      ...
      (await client.checkMember(
        entry.executor.id,
        "member",
        "Sağ-Tık Rol/Ver Alma!",
      ))
    )
      return;
    if (
      permissionStaff.some(
        (p) => oldMember.permissions.has(p) && newMember.permissions.has(p),
      )
    ) {
      await newMember.roles.set(oldMember.roles.cache.map((r) => r.id));
      embed.setDescription(
        `${newMember} (__${newMember.id}__) üyesine ${entry.executor} (__${entry.executor.id}__) tarafından Sağtık Rol İşlemi Yapıldı! Veren kişi cezalandırıldı ve verilen kişiden rol geri alındı.`,
      );
...
client.on(
  ...
  async (member, oldNickname, newNickname) => {
      ...
      if (
          ...
          { _id: member.id },
          {
            $push: {
              Names: {
                ...
                Name: newNickname == null ? member.user.username : newNickname,
                State: "Sağ-Tık Değiştirme",
                Date: Date.now(),
              },
            },
          },
        ...
        if (Log)
          Log.send({
            embeds: [
              embed.setDescription(
                `${member} (__${member.id}__) üyesinin ismi \`${oldNickname == null ? member.user.username : oldNickname} => ${newNickname == null ? member.user.username : newNickname}\` olarak ${entry.executor} (__${entry.executor.id}__) tarafından **${tarihsel(Date.now())}** güncellendi.`,
              ),
            ],
          });
      }
    ...
    embed.setDescription(
      `${member} (__${member.id}__) üyesinin ismi \`${oldNickname == null ? member.user.username : oldNickname} => ${newNickname == null ? member.user.username : newNickname}\` olarak ${entry.executor} (__${entry.executor.id}__) tarafından izinsiz olarak güncellendi ve ${member} üyesinin ismi eski haline getirildi.`,
    );
