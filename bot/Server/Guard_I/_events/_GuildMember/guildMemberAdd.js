...
module.exports = async (member) => {
  ...
  if (
    ...
    (await client.checkMember(entry.executor.id, "bot", "Sunucuya Bot Ekleme!"))
  )
    return;
  ...
  embed.setDescription(
    `${entry.executor} (\`${entry.executor.id}\`) isimli üye tarafından ${member} (\`${member.id}\`) adında bir bot ekledi ve eklenen bot ile ekleyen üye cezalandırıldı.`,
  );
