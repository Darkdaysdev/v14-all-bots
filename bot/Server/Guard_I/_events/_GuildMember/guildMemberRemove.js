...
module.exports = async (member) => {
  ...
  let embed = new genEmbed().setTitle("Sunucuda Sağ-Tık Üye Atıldı!");
  ...
  if (
    ...
    (await client.checkMember(entry.executor.id, "member", "Sağ-Tık Atma!"))
  )
    return;
  client.punitivesAdd(entry.executor.id, "jail");
  client.allPermissionClose();
  embed.setDescription(
    `${member} (\`${member.id}\`) üyesi, ${entry.executor} (\`${entry.executor.id}\`) tarafından sunucudan \`Sağ-Tık\` ile atıldı! atan kişi ise yasaklandı.`,
  );
