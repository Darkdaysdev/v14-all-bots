  ...
  if (ayarlar && ayarlar.özelOda) {
    ...
    if (guild) {
      ...
      if (
        olur &&
        kanalcık &&
        (kanalcık.parentId == olur.id ||
          kanalcık.parentId == kanallar.streamerKategorisi ||
          kanalcık.parentId == kanallar.sorunCozmeKategorisi)
      )
        return;
      ...
      if (
        ...
        kanalcık &&
        kanalgetir.id &&
        kanalcık.parentId == kanalgetir.id
      )
        return;
    }
  }
  if (kanalcık === null) {
    let entry = await (oldState, newState).guild
    ...
    embed.setDescription(
      `${entry.executor} (\`${entry.executor.id}\`) tarafından ${oldState.member} üyesinin bağlantısı kesildi. Bu işlemi yapan kişi sunucudan cezalandırıldı.`,
    );
...
client.on("voiceChannelMute", async (member, muteType) => {
  ...
  if (
    ...
    !entry.executor.bot &&
    checkRegister &&
    checkRegister.parentId
  ) {
    if (
      checkRegister.parentId == kanallar.registerKategorisi ||
      checkRegister.parentId == kanallar.streamerKategorisi ||
      checkRegister.parentId == kanallar.sorunCozmeKategorisi
    ) {
      embed.setDescription(
        `${member} (__${member.id}__) üyesine ${entry.executor} (__${entry.executor.id}__) tarafından ${checkRegister} kanalında Sağ-tık susturma işlemi yapıldı!`,
      );
  ...
  //client.allPermissionClose()
  embed.setDescription(
    `${member} (__${member.id}__) üyesine ${entry.executor} (__${entry.executor.id}__) tarafından Sağ-tık susturma işlemi yapıldı! yapan kişi ise cezalandırıldı.`,
  );
