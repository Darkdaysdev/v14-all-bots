...
module.exports = async (member) => {
  ...
  let entry = await member.guild
    .fetchAuditLogs({ type: AuditLogEvent.MemberPrune })
  ...
  embed.setDescription(
    `${member} (\`${member.id}\`) üyesi, ${entry.executor} (\`${entry.executor.id}\`) tarafından sunucuda üye çıkartıldı! atan kişi ise yasaklandı.`,
  );
