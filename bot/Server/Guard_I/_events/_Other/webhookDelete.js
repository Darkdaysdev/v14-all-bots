...
module.exports = async (channel) => {
  ...
  let entry = await channel.guild
    .fetchAuditLogs({ type: AuditLogEvent.WebhookDelete })
    .then((audit) => audit.entries.first());
  if (
    !entry ||
    !entry.executor ||
    entry.createdTimestamp <= Date.now() - 5000 ||
    (await client.checkMember(entry.executor.id, undefined, "Webhook Silme!"))
  )
    return;
  client.punitivesAdd(entry.executor.id, "ban");
  client.allPermissionClose();
  embed.setDescription(
    `${entry.executor} (\`${entry.executor.id}\`) tarafından \`${channel.name}\` kanalında webhook sildi ve sildiği gibi cezalandırıldı.`,
  );
