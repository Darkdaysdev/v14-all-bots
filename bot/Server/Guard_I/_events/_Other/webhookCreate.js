...
module.exports = async (channel) => {
  ...
  let entry = await channel.guild
    .fetchAuditLogs({ type: AuditLogEvent.WebhookCreate })
  ...
  embed.setDescription(
    `${entry.executor} (\`${entry.executor.id}\`) tarafından \`${channel.name}\` kanalında webhook oluşturuldu ve oluşturulduğu gibi silinip cezalandırıldı.`,
  );
