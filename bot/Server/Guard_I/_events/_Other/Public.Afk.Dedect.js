...
client.on("voiceStateUpdate", async (oldState, newState) => {
  ...
  if (oldState.channelId && !newState.channelId) {
    if (streamerAfks.get(member.id))
      return await streamerAfks.delete(member.id);
  }
  if (
    member.guild.channels.cache.get(newState.channelId).parentId !=
    kanallar.streamerKategorisi
  )
    return;
  if (member.voice.selfDeaf && member.voice.selfMute) {
    streamerAfks.set(member.id, {
      channel: newState.channelId,
      date: Date.now() + ms(streamersure),
    });
  } else {
...
module.exports = async (oldState, newState) => {
  ...
  if (member.voice.selfDeaf || member.voice.selfMute) {
    voiceAfks.set(member.id, {
      channel: newState.channelId,
      date: Date.now() + ms(sure),
    });
  } else {
  ...
  guild.channels.cache
    .filter(
      (e) =>
        ...
        e.members.size > 0 &&
        e.parentId == kanallar.streamerKategorisi,
    )
    .forEach((channel) => {
      channel.members
        .filter(
          (member) =>
            !member.user.bot && member.voice.selfDeaf && member.voice.selfMute,
        )
        .forEach(async (member) => {
          if (!streamerAfks.get(member.id))
            return streamerAfks.set(member.id, {
              channel: channel.id,
              date: Date.now() + ms(streamersure),
            });
        });
  ...
  guild.channels.cache
    .filter(
      (e) =>
        ...
        kanallar.musicRooms.some((x) => e.id != x),
    )
    .forEach((channel) => {
      channel.members
        .filter(
        ...
        .forEach(async (member) => {
          if (!voiceAfks.get(member.id))
            return voiceAfks.set(member.id, {
              channel: channel.id,
              date: Date.now() + ms(sure),
            });
        });
...
async function checkStreamer() {
  ...
  guild.channels.cache
    ...
    .forEach((channel) => {
      channel.members
        .filter((member) => !member.user.bot && streamerAfks.get(member.id))
        .forEach(async (member) => {
          ...
          if (data && Date.now() >= data.date) {
            ...
            if (member)
              member
                .send({
                  content: `**${channel.name}** sohbet kanalında afk olduğunu fark ettik.
Seni otomatik olarak "**${member.guild.channels.cache.get(kanallar.sleepRoom).name}**" kanalına gönderdim.`,
                })
                .catch((err) => {});
            if (member && member.voice.channel)
              return member.voice
                .setChannel(kanallar.sleepRoom)
                .catch((err) => {});
          }
        });
    });
...
async function checkAfk() {
  ...
  guild.channels.cache
    ...
    .forEach((channel) => {
      channel.members
        .filter((member) => !member.user.bot && voiceAfks.get(member.id))
        .forEach(async (member) => {
