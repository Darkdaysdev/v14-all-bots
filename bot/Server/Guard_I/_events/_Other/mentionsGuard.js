  ...
  if (
    ...
    if (
      (await client.checkMember(
        ...
        undefined,
        "Gereksiz Duyuru Kullanımı!",
      )) ||
      (_set.chatİzinliler && _set.chatİzinliler.includes(newMessage.member.id))
    )
      return;
    ...
    embed.setDescription(
      `${uye} (\`${uye.id}\`) üyesi @everyone & @here yetkisine sahip olup kullanım sağladığı için cezalandırıldı.`,
    );
...
client.on("messageCreate", async (message) => {
  ...
  if (
    ...
    if (
      ...
      (_set.chatİzinliler && _set.chatİzinliler.includes(message.member.id))
    )
      return;
