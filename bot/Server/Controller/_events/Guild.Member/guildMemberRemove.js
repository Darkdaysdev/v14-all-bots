    ...
    kanalBul.send({
      ...
      embeds: [
        new genEmbed().setDescription(
          `${member} (\`${member.id}\`) üyesi sunucudan <t:${String(Date.now()).slice(0, 10)}:R> (<t:${String(Date.now()).slice(0, 10)}:F>) ayrıldı.`,
        ),
      ],
    });
  if (
    member.roles.cache.has(roller.jailRolü) ||
    ...
    (roller.kayıtsızRolleri &&
      roller.kayıtsızRolleri.some((rol) => member.roles.cache.has(rol)))
  )
    return;
  member.Left();
};
...
client.on("guildMemberRemove", async (member) => {
  ...
  if (yetkiliRol.rawPosition < uyeUstRol.rawPosition) {
    ...
    member.roles.cache
      ...
      .map((x) => rolleri.push(x.id));
    await leftRoles.updateOne(
      { _id: member.id },
      { $set: { _roles: rolleri } },
      { upsert: true },
    );
...
client.on("guildMemberRemove", async (member) => {
  ...
  if (!entry || !entry.executor || entry.createdTimestamp <= Date.now() - 5000)
    return;
  await Users.updateOne(
    { _id: entry.executor.id },
    { $inc: { "Uses.Kick": 1 } },
    { upsert: true },
  );
});
