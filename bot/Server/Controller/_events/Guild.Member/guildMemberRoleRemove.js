...
module.exports = async (member, role) => {
  ...
  if (
    ...
    roller.başlangıçYetki != role.id
  ) {
    let guild = member.guild;
    if (!guild) return;
    await member.roles
      .add(role.id, "Sağ-tık yetki çekildiğinden dolayı tekrar verildi.")
      .catch((err) => {});
    let added = guild.members.cache.get(entry.executor.id);
    if (added)
      added
        .send({
          content: `Yetki vermek istediğin ${member} isimli üyeye Sağ-tık ile yetki alamazsın.
Almak istediğin **@${role.name}** rolü sadece komut üzerinden verilmeye ve alınmaya açıktır.
Lütfen websitesi veya komut üzerinden tekrardan verebilir veya da alabilirsiniz. ${guild.emojiGöster(emojiler.Iptal)}`,
        })
        .catch((err) => {});
  } else {
    let görevGetir =
      ...
      logChannel.send({
        embeds: [
          new genEmbed().setDescription(
            `${member} isimli üyeden <t:${String(Date.now()).slice(0, 10)}:R> ${entry.executor} tarafından ${role} adlı rol geri alındı.`,
          ),
        ],
      });
  }
