...
module.exports = async (oldMember, newMember) => {
  let _staffs = require("../../../../Global/Plugins/Staff/Sources/_settings");
  ...
  newMember.roles.cache.forEach(async (role) => {
    if (!oldMember.roles.cache.has(role.id)) {
      if (
        _staffs.staffs.some((x) => role.id == x.rol) &&
        roller.altilkyetki != role.id &&
        roller.ilkYetki != role.id
      ) {
        ...
        if (added)
          added
            .send({
              content: `Merhaba ${added}! ${newMember} kişisine, el ile yetkili rol verdiğin için kişinin @${role.name} rolü alındı, yetkili rol vermek için \`.yetkili ${newMember}\` komutunu kullan.`,
            })
            .catch((err) => {});
      } else {
        let görevGetir =
          (await Tasks.findOne({ roleID: role.id })) ||
          (await Tasks.findOne({ roleID: role.id }));
        ...
        if (
          ...
          KullaniciData &&
          yetkiliBilgisi &&
          yetkiliBilgisi.Yönetim &&
          KullaniciData.Staff &&
          KullaniciData.StaffGiveAdmin
        ) {
          await yetkiSistemi.updateOne(
            { _id: newMember.id },
            { $set: { Rolde: Date.now() } },
          ...
          logChannel.send({
            embeds: [
              new genEmbed().setDescription(
                `${entry.executor} yetkilisi, ${newMember} kişisine ${tarihsel(Date.now())} tarihinde <@&${role.id}> rolünü verdi.`,
              ),
            ],
          });
      }
      ...
      if (
        ...
        if (logChannel)
          logChannel.send({
            embeds: [
              new genEmbed().setDescription(
                `${entry.executor} yetkilisi, ${newMember} kişisinden ${tarihsel(Date.now())} tarihinde <@&${role.id}> rolünü aldı.`,
              ),
            ],
          });
      }
