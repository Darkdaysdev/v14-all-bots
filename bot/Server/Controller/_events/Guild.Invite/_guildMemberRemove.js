...
module.exports = async (member) => {
  ...
  if (member.user.bot)
    return channel.send({
      content: `${member.guild.emojiGöster(emojiler.Iptal)} **${member.user.username}** üyesi sunucumuzdan <t:${Number(String(Date.now()).substring(0, 10))}:R> ayrıldı!`,
    });
  const inviteMemberData =
    (await GUILD_INVITE.findOne({ userID: member.user.id })) || [];
  if (!inviteMemberData.Inviter) {
    ...
    if (Date.now() - member.user.createdTimestamp <= 1000 * 60 * 60 * 24 * 7) {
      ...
      const total = inviterData ? inviterData.total : 0;
      return channel.send({
        content: `${member.guild.emojiGöster(emojiler.Iptal)} **${member.user.username}** üyesi sunucumuzdan <t:${Number(String(Date.now()).substring(0, 10))}:R> ayrıldı! Sunucumuza **${inviter.tag}** üyesi tarafından davet edilmiş. ${total > 0 ? `**(**Toplam Davet: \`${total}\`**)**` : ""}`,
      });
    } else {
      let inviteOwn = member.guild.members.cache.get(inviteMemberData.Inviter);
      const inviter = await client.users.fetch(inviteMemberData.Inviter);
      const ivSync = await GUILD_INVITE.findOne({
        guildID: member.guild.id,
        userID: inviter.id,
      });
      if (ivSync && ivSync.total - 1 >= 0)
        await GUILD_INVITE.findOneAndUpdate(
      ...
      const total = inviterData ? inviterData.total : 0;
      if (inviteOwn && _statSystem.system)
