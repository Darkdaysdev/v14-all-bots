...
module.exports = async (member) => {
  ...
  const _findServer = await GUILD_SETTINGS.findOne({
  ...
  let entry = await member.guild
    .fetchAuditLogs({ type: AuditLogEvent.BotAdd })
    .then((audit) => audit.entries.first());
  if (member.user.bot && entry)
    return channel.send({
      content: `${member.guild.emojiGöster(emojiler.Onay)} ${member} isimli bot, **${entry.executor.tag}** tarafından \`${member.guild.name}\` sunucusuna davet edildi.`,
    });
  ...
  if (invite === null) {
    channel.send({
      content: `${member.guild.emojiGöster(emojiler.Onay)} ${member} üyesi sunucumuza <t:${Number(String(Date.now()).substring(0, 10))}:R> katıldı!`,
    });
    ...
    await GUILD_INVITE.findOneAndUpdate(
      { userID: member.user.id },
      { $set: { Inviter: member.guild.id } },
      { upsert: true },
    );
    await GUILD_INVITE.findOneAndUpdate(
      { guildID: member.guild.id, userID: member.guild.id },
      { $inc: { total: 1 } },
      { upsert: true },
    );
    const inviterData = await GUILD_INVITE.findOne({
      guildID: member.guild.id,
      userID: member.guild.id,
    });
    const total = inviterData ? inviterData.total : 0;
    return channel.send({
      content: `${member.guild.emojiGöster(emojiler.Onay)} ${member} üyesi sunucumuza **Özel URL** kullanarak <t:${Number(String(Date.now()).substring(0, 10))}:R> katıldı!`,
    });
  } else {
    ...
    if (Date.now() - member.user.createdTimestamp <= 1000 * 60 * 60 * 24 * 7) {
      ...
      const total = inviterData ? inviterData.total : 0;
      channel.send({
        content: `${member.guild.emojiGöster(emojiler.Onay)} ${member} üyesi **${invite.inviter.tag}** tarafından <t:${Number(String(Date.now()).substring(0, 10))}:R> sunucumuza davet edildi. ${total > 0 ? `**(**Toplam Davet: \`${total}\`**)**` : ""}`,
      });
    } else {
      await GUILD_INVITE.findOneAndUpdate(
        { guildID: member.guild.id, userID: invite.inviter.id },
      ...
      const total = inviterData ? inviterData.total : 0;
      if (inviteOwn)
        await Upstaff.addPoint(
          inviteOwn.id,
          _statSystem.points.invite,
          "Invite",
        );
      if (inviteOwn)
        inviteOwn.Leaders("davet", _statSystem.points.invite, {
          type: "INVITE",
          user: member.id,
        }),
          inviteOwn.Leaders("invite", _statSystem.points.invite, {
            type: "INVITE",
            user: member.id,
          });
      channel.send({
