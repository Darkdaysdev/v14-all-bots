...
module.exports = async () => {
  ...
  setInterval(async () => {
    ...
    köpekoçlar.forEach(async (ceza) => {
      ...
      if (!uye && ceza.Duration && Date.now() >= ceza.Duration) {
        ...
        if (await Punitives.findOne({ No: ceza.No }))
          await Punitives.updateOne(
            { No: ceza.No },
            { $set: { Active: false, Expried: Date.now() } },
            { upsert: true },
          );
      }
      if (uye && ceza.Duration && Date.now() >= ceza.Duration) {
        ...
        if (uye && !uye.roles.cache.get(roller.jailRolü))
          await uye.setRoles(roller.jailRolü);
      }
    });
  }, 3000);
  ...
  setInterval(async () => {
    ...
    OrospucocuklarıKüfürEtti.forEach(async (ceza) => {
      ...
      if (uye && ceza.Duration && Date.now() >= ceza.Duration) {
        if (roller.muteRolü)
          await uye.roles.remove(roller.muteRolü).catch((err) => {});
        await Mutes.deleteOne({ _id: ceza._id });
        if (await Punitives.findOne({ No: ceza.No }))
          await Punitives.updateOne(
            { No: ceza.No },
        ...
        if (uye && !uye.roles.cache.get(roller.muteRolü))
          await uye.roles.add(roller.muteRolü).catch((err) => {});
      }
    });
  }, 5000);
    ...
    Sesmute.forEach(async (ceza) => {
      ...
      if (uye && ceza.Duration && Date.now() >= ceza.Duration) {
        if (uye && uye.voice.channel)
          await uye.voice.setMute(false).catch((err) => {});
        await voiceMutes.deleteOne({ _id: ceza._id });
        if (await Punitives.findOne({ No: ceza.No }))
          await Punitives.updateOne(
            { No: ceza.No },
        ...
        if (uye && uye.voice.channel) {
          if (
            ayarlar &&
            ayarlar.sorunCozmeKategorisi &&
            uye.voice.channel.parentId == ayarlar.sorunCozmeKategorisi
          )
            return;
  ...
  setInterval(async () => {
    ...
    VKDATA.forEach(async (ceza) => {
      ...
      if (uye && ceza.Duration && Date.now() >= ceza.Duration) {
        if (roller.vkCezalıRolü)
          await uye.roles.remove(roller.vkCezalıRolü).catch((err) => {});
        await VK.deleteOne({ _id: ceza._id });
        if (await Punitives.findOne({ No: ceza.No }))
          await Punitives.updateOne(
            { No: ceza.No },
        ...
        if (uye && !uye.roles.cache.get(roller.vkCezalıRolü))
          await uye.roles.add(roller.vkCezalıRolü).catch((err) => {});
      }
    });
  }, 30000);
  ...
  setInterval(async () => {
    ...
    STREAMDATA.forEach(async (ceza) => {
      ...
      if (uye && ceza.Duration && Date.now() >= ceza.Duration) {
        if (roller.streamerCezalıRolü)
          await uye.roles.remove(roller.streamerCezalıRolü).catch((err) => {});
        await STREAM.deleteOne({ _id: ceza._id });
        if (await Punitives.findOne({ No: ceza.No }))
          await Punitives.updateOne(
            { No: ceza.No },
        ...
        if (uye && !uye.roles.cache.get(roller.streamerCezalıRolü))
          await uye.roles.add(roller.streamerCezalıRolü).catch((err) => {});
      }
    });
  }, 25000);
  ...
  setInterval(async () => {
    ...
    DCDATA.forEach(async (ceza) => {
      ...
      if (uye && ceza.Duration && Date.now() >= ceza.Duration) {
        if (roller.dcCezalıRolü)
          await uye.roles.remove(roller.dcCezalıRolü).catch((err) => {});
        await DC.deleteOne({ _id: ceza._id });
        if (await Punitives.findOne({ No: ceza.No }))
          await Punitives.updateOne(
            { No: ceza.No },
        ...
        if (uye && !uye.roles.cache.get(roller.dcCezalıRolü))
          await uye.roles.add(roller.dcCezalıRolü).catch((err) => {});
      }
    });
  }, 45000);
  ...
  setInterval(async () => {
    ...
    ETKINLIKDATA.forEach(async (ceza) => {
      ...
      if (uye && ceza.Duration && Date.now() >= ceza.Duration) {
        if (roller.etkinlikCezalıRolü)
          await uye.roles.remove(roller.etkinlikCezalıRolü).catch((err) => {});
        await ETKINLIK.deleteOne({ _id: ceza._id });
        if (await Punitives.findOne({ No: ceza.No }))
          await Punitives.updateOne(
            { No: ceza.No },
        ...
        if (uye && !uye.roles.cache.get(roller.etkinlikCezalıRolü))
          await uye.roles.add(roller.etkinlikCezalıRolü).catch((err) => {});
      }
    });
  }, 60000);
};
