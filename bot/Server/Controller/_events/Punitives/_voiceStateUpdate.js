...
module.exports = async (oldState, newState) => {
  if (
    ...
    if (
      ...
      newState.channel.parentId == ayarlar.sorunCozmeKategorisi
    )
      return;
    ...
    if (data) {
      if (data.Duration && Date.now() >= data.Duration) {
        if (member && member.voice.channel)
          await member.voice.setMute(false).catch((err) => {});
...
client.on("voiceChannelJoin", async (member, channel) => {
  ...
  if (
    (kanallar.ayrıkKanallar &&
      kanallar.ayrıkKanallar.some((x) => channel.id == x)) ||
    channel.parentId == kanallar.streamerKategorisi ||
    channel.parentId == kanallar.registerKategorisi
  )
    return;
  let data = await VMute.findOne({ _id: member.id });
  if (!data) {
