...
client.on("messageCreate", async (message) => {
  if (
    ...
    sistem.botSettings.Prefixs.some((x) => message.content.startsWith(x))
  )
    return;
  await Seens.updateOne(
    ...
    async (err, data) => {
      ...
      if (!data) {
        ...
        if (guildData && guildData.seviyeSistemi)
          await messageXP(message.author.id, message);
        if (
          _statSystem.system &&
...
async function dayStatsUpdate(id, channel) {
  ...
  if (kanal) {
    ...
    if (uye && kanallar.chatKanalı && kanallar.chatKanalı == kanal.id) {
      ...
      if (veri >= 10) {
        uye.Leaders("chat", 0.1, { type: "MESSAGE", channel: kanal.id });
...
async function messageXP(id, message) {
  ...
  await Stats.updateOne(
    ...
    { $inc: { messageXP: msgXp[Math.floor(Math.random() * msgXp.length)] } },
    { upsert: true },
  );
  ...
  if (_getStat) {
    ...
    if (yeniLevel <= _getStat.messageXP) {
      await Stats.updateOne(
        { guildID: sistem.SERVER.ID, userID: id },
        { $inc: { messageLevel: 1, messageXP: 0 } },
        { upsert: true },
      );
      ...
      const image = new Canvas(740, 128)
        ...
        .setColor("#fff")
        .printText(
          `+${2500 * (_getStat.messageLevel + 1)} ${ayarlar.serverName} Parası`,
          350,
          70,
          350,
        )
      ...
      message
        .reply({
          content: `**Tebrikler!** ${message.member.user.username}
Sohbet seviyeniz yükseldi ve ödülleri kaptınız.`,
        ...
        .then(async (msg) => {
          await client.Economy.updateBalance(
            message.author.id,
            Number(2500 * (_getStat.messageLevel + 1)),
            "add",
            1,
          );
          await client.Economy.updateBalance(
