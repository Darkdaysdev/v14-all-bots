...
module.exports = async () => {
  ...
  if (data && data.leaderboardVoice && data.leaderboardText) {
    try {
      ...
      LeaderBoard = await sıralamaKanalı.send(
        `Yeni bir ses sıralama mesajı oluşturuluyor...`,
      );
      await GUILD_SETTINGS.updateOne(
        { guildID: guild.id },
        { $set: { [`Caches.leaderboardVoice`]: LeaderBoard.id } },
        { upsert: true },
      );
    }

    try {
      LeaderBoardiki = await sıralamaKanalı.messages.fetch(
        data.leaderboardText,
      );
    } catch (err) {
      LeaderBoardiki = await sıralamaKanalı.send(
        `Yeni bir metin sıralama mesajı oluşturuluyor...`,
      );
      await GUILD_SETTINGS.updateOne(
        { guildID: guild.id },
        { $set: { [`Caches.leaderboardText`]: LeaderBoardiki.id } },
        { upsert: true },
      );
    }
  } else {
  ...
  async function checkingLeader() {
    ...
    if (
      ayarlar &&
      ayarlar.haftaBirincileri &&
      ayarlar.haftaninBirinciRolü &&
      onecik &&
      onecik.length > 0
    ) {
      let eklenecek = sunucu.members.cache.filter(
        (x) =>
          ...
          !x.permissions.has(Discord.PermissionFlagsBits.Administrator) &&
      ...
      let uyeler = sunucu.members.cache.filter(
        (x) =>
          x.roles.cache.has(ayarlar.haftaninBirinciRolü) &&
          !onecik.includes(x.id),
      );
      ...
      let uyeler = sunucu.members.cache.filter((x) =>
        x.roles.cache.has(ayarlar.haftaninBirinciRolü),
      );
      if (uyeler) {
        uyeler.forEach((uye) => {
    ...
    MessageEdit.setAuthor({
      name: client.guilds.cache.get(sistem.SERVER.ID).name,
    ...
    LeaderBoard.edit({
      content: null,
      embeds: [
        MessageEdit.setDescription(`Aşağı da **${ayarlar.serverName ? ayarlar.serverName : client.guilds.cache.get(sistem.SERVER.ID).name}** sunucusuna ait haftalık genel ses sıralaması listelenmektedir.\n\n${voiceList}\n
Bu sıralama <t:${Number(String(Date.now()).substring(0, 10))}:R> düzenlendi.`),
    ...
    LeaderBoardiki.edit({
      content: null,
      embeds: [
        MessageEdit.setDescription(`Aşağı da **${ayarlar.serverName ? ayarlar.serverName : client.guilds.cache.get(sistem.SERVER.ID).name}** sunucusuna ait haftalık genel mesaj sıralaması listelenmektedir.\n\n${messageList}\n
Bu sıralama <t:${Number(String(Date.now()).substring(0, 10))}:R> düzenlendi.`),
      ],
    });
  }
...
client.on("ready", async () => {
  ...
  if (Data.Ayarlar && Data.Ayarlar.yetkiliSıralamaKanalı)
    yetkiliSıralaması(guild);
  ...
  setInterval(() => {
    if (Data.Ayarlar && Data.Ayarlar.yetkiliSıralamaKanalı)
      yetkiliSıralaması(guild);
    if (Data.Ayarlar && Data.Ayarlar.taglıSıralamaKanalı)
      taglıSıralaması(guild);
    if (Data.Ayarlar && Data.Ayarlar.teyitSıralamaKanalı)
      teyitSıralaması(guild);
  }, 600000 * 3);

  setInterval(() => {
    if (Data.Ayarlar && Data.Ayarlar.yayınSıralamaKanalı)
      yayınSıralaması(guild);
    if (Data.Ayarlar && Data.Ayarlar.davetSıralamaKanalı)
      davetSıralaması(guild);
  }, 600000 * 4);
});
...
async function yayınSıralaması(guild) {
  ...
  Data = Data.filter((x) => guild.members.cache.has(x._id) && x.Streaming)
    .sort((a, b) => {
      let uye2Toplam2 = 0;
      b.Streaming.map((x) => {
  ...
  if (data && data.leaderboardStream) {
    try {
      ...
      LeaderBoard = await kanal.send(
        `Yeni bir yayın açma sıralama mesajı oluşturuluyor...`,
      );
      await GUILD_SETTINGS.updateOne(
        { guildID: guild.id },
        { $set: { [`Caches.leaderboardStream`]: LeaderBoard.id } },
        { upsert: true },
      );
    }
  } else {
  ...
  let embed = new genEmbed().setColor("Random").setAuthor({
    name: guild.name,
  ...
  LeaderBoard.edit({
    content: null,
    embeds: [
      embed.setDescription(`Aşağı da **${ayarlar.serverName ? ayarlar.serverName : guild.name}** sunucusuna ait en iyi yayın açan sıralaması listelenmektedir.\n\n${Data.slice(0, 30).join("\n")}\n
Bu sıralama <t:${Number(String(Date.now()).substring(0, 10))}:R> düzenlendi.`),
    ],
  });
}
...
async function davetSıralaması(guild) {
  ...
  Data = Data.filter((x) => guild.members.cache.has(x.userID) && x.total)
    .sort((a, b) => {
  ...
  if (data && data.leaderboardInvite) {
    try {
      ...
      LeaderBoard = await kanal.send(
        `Yeni bir davet sıralama mesajı oluşturuluyor...`,
      );
      await GUILD_SETTINGS.updateOne(
        { guildID: guild.id },
        { $set: { [`Caches.leaderboardInvite`]: LeaderBoard.id } },
        { upsert: true },
      );
    }
  } else {
  ...
  LeaderBoard.edit({
    content: null,
    embeds: [
      embed.setDescription(`Aşağı da **${ayarlar.serverName ? ayarlar.serverName : guild.name}** sunucusuna ait en iyi davet sıralaması listelenmektedir.\n\n${Data.slice(0, 30).join("\n")}\n
Bu sıralama <t:${Number(String(Date.now()).substring(0, 10))}:R> düzenlendi.`),
    ],
  });
}
...
async function taglıSıralaması(guild) {
  ...
  if (!kanal) return;
  let Data = await Users.find();
  Data = Data.filter((x) => guild.members.cache.has(x._id) && x.Taggeds)
    .sort((a, b) => {
  ...
  if (data && data.leaderboardTagged) {
    try {
      ...
      LeaderBoard = await kanal.send(
        `Yeni bir taglı sıralama mesajı oluşturuluyor...`,
      );
      await GUILD_SETTINGS.updateOne(
        { guildID: guild.id },
        { $set: { [`Caches.leaderboardTagged`]: LeaderBoard.id } },
        { upsert: true },
      );
    }
  } else {
  ...
  LeaderBoard.edit({
    content: null,
    embeds: [
      embed.setDescription(`Aşağı da **${ayarlar.serverName ? ayarlar.serverName : guild.name}** sunucusuna ait en iyi taglı çeken sıralaması listelenmektedir.\n\n${Data.slice(0, 30).join("\n")}\n
Bu sıralama <t:${Number(String(Date.now()).substring(0, 10))}:R> düzenlendi.`),
    ],
  });
}
...
async function yetkiliSıralaması(guild) {
  ...
  if (!kanal) return;
  let Data = await Users.find();
  Data = Data.filter((x) => guild.members.cache.has(x._id) && x.Staffs)
    .sort((a, b) => {
  ...
  if (data && data.leaderboardStaff) {
    try {
      ...
      LeaderBoard = await kanal.send(
        `Yeni bir yetkili sıralama mesajı oluşturuluyor...`,
      );
      await GUILD_SETTINGS.updateOne(
        { guildID: guild.id },
        { $set: { [`Caches.leaderboardStaff`]: LeaderBoard.id } },
        { upsert: true },
      );
    }
  } else {
  ...
  LeaderBoard.edit({
    content: null,
    embeds: [
      embed.setDescription(`Aşağı da **${ayarlar.serverName ? ayarlar.serverName : guild.name}** sunucusuna ait en iyi yetkili çeken sıralaması listelenmektedir.\n\n${Data.slice(0, 30).join("\n")}\n
Bu sıralama <t:${Number(String(Date.now()).substring(0, 10))}:R> düzenlendi.`),
    ],
  });
}
...
async function teyitSıralaması(guild) {
  ...
  if (!kanal) return;
  let Data = await Users.find();
  Data = Data.filter((x) => guild.members.cache.has(x._id) && x.Records)
    .sort((a, b) => {
  ...
  if (data && data.leaderboardRegister) {
    try {
      ...
      LeaderBoard = await kanal.send(
        `Yeni bir teyit sıralama mesajı oluşturuluyor...`,
      );
      await GUILD_SETTINGS.updateOne(
        { guildID: guild.id },
        { $set: { [`Caches.leaderboardRegister`]: LeaderBoard.id } },
        { upsert: true },
      );
    }
  } else {
  ...
  LeaderBoard.edit({
    content: null,
    embeds: [
      embed.setDescription(`Aşağı da **${ayarlar.serverName ? ayarlar.serverName : guild.name}** sunucusuna ait genel teyit sıralaması listelenmektedir.\n\n${Data.slice(0, 30).join("\n")}\n
Bu sıralama <t:${Number(String(Date.now()).substring(0, 10))}:R> düzenlendi.`),
    ],
  });
}
