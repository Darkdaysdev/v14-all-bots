...
client.on("ready", async () => {
  client.logger.log(
    "İstatistik, davet, yetki sistemi verileri güncellendi.",
    "stat",
  );
  client.guilds.cache
    .get(sistem.SERVER.ID)
    .channels.cache.filter(
      (e) => e.type == ChannelType.GuildVoice && e.members.size > 0,
    )
    .forEach((channel) => {
      channel.members
...
module.exports = (oldState, newState) => {
  ...
  if (!statRecods.has(oldState.id)) {
    if (
      kanallar.ayrıkKanallar.some((x) => newState.channelId == x) &&
      kanallar.ayrıkKanallar.some((x) => oldState.channelId == x)
    ) {
      statRecods.set(oldState.id, {
        channel: newState.channelId,
        duration: Date.now(),
...
async function voiceInit(memberID, categoryID, duraction) {
  Stats.findOne(
    { guildID: sistem.SERVER.ID, userID: memberID },
    async (err, data) => {
      if (!data) {
        ...
        let uye = client.guilds.cache
          .get(sistem.SERVER.ID)
          .members.cache.get(memberID);
        if (uye) {
          let datacikcik = await GUILD_SETTINGS.findOne({
            guildID: sistem.SERVER.ID,
          });
          ...
          if (guildData && guildData.seviyeSistemi)
            await voiceXP(
              uye.id,
              duraction / 1000,
              uye.voice ? uye.voice.channel : null,
            );
          if (_statSystem.system) {
            if (_statSystem.staffs.some((x) => uye.roles.cache.has(x.rol))) {
              data.upstaffVoiceStats.set(
                categoryID,
                Number(lastStaffData) + duraction,
              );
              datacikcik = await GUILD_SETTINGS.findOne({
                guildID: sistem.SERVER.ID,
              });
              if (datacikcik && datacikcik.Ayarlar) {
                ...
                if (
                  guildData &&
                  guildData.Etkinlik == true &&
                  guildData.etkinlikIzinliler &&
                  guildData.etkinlikIzinliler.some((x) => x == categoryID)
                )
                  await client.Upstaffs.addPoint(
                    uye.id,
                    guildData.etkinlikPuan ? guildData.etkinlikPuan : "0.001",
                    "Etkinlik",
                  );
              }
            }
            ...
            if (data2 && data2.Mission)
              data.taskVoiceStats.set(
                categoryID,
                Number(lastTaskData) + duraction,
              ),
                checkTasks(uye, data2);
          }
        }
...
async function dayStatsUpdate(id, channel, value, type, ver) {
  ...
  if (
    ver &&
    kategori &&
    kanallar.publicKategorisi &&
    kanallar.publicKategorisi == kategori.parentId
  ) {
    let uye = kategori.guild.members.cache.get(id);
    if (uye && uye.voice && (uye.voice.selfMute || uye.voice.selfDeaf)) {
      uye.Leaders("pub", (value / 1000 / 60 / 2) * 2, {
        type: "VOICE",
        channel: kategori,
      });
    } else {
      uye.Leaders("pub", (value / 1000 / 60 / 2) * 10, {
        type: "VOICE",
        channel: kategori,
      });
    }
  ...
  if (
    ver &&
    kategori &&
    kanallar.registerKategorisi &&
    kanallar.registerKategorisi == kategori.parentId
  ) {
    let uye = kategori.guild.members.cache.get(id);
    if (uye && uye.voice && (uye.voice.selfMute || uye.voice.selfDeaf)) {
      uye.Leaders("reg", (value / 1000 / 60 / 2) * 2, {
        type: "VOICE",
        channel: kategori,
      });
      uye.Leaders("teyit", (value / 1000 / 60 / 2) * 2, {
        type: "VOICE",
        channel: kategori,
      });
      uye.Leaders("kayıt", (value / 1000 / 60 / 2) * 2, {
        type: "VOICE",
        channel: kategori,
      });
    } else {
      uye.Leaders("reg", (value / 1000 / 60 / 2) * 10, {
        type: "VOICE",
        channel: kategori,
      });
      uye.Leaders("teyit", (value / 1000 / 60 / 2) * 10, {
        type: "VOICE",
        channel: kategori,
      });
      uye.Leaders("kayıt", (value / 1000 / 60 / 2) * 10, {
        type: "VOICE",
        channel: kategori,
      });
    }
  ...
  if (
    ver &&
    kategori &&
    kanallar.streamerKategorisi &&
    kanallar.streamerKategorisi == kategori.parentId
  ) {
    let uye = kategori.guild.members.cache.get(id);
    if (uye && uye.voice && (uye.voice.selfMute || uye.voice.selfDeaf)) {
      uye.Leaders("stream", (value / 1000 / 60 / 2) * 2, {
        type: "VOICE",
        channel: kategori,
      });
      uye.Leaders("cam", (value / 1000 / 60 / 2) * 2, {
        type: "VOICE",
        channel: kategori,
      });
      uye.Leaders("yayın", (value / 1000 / 60 / 2) * 2, {
        type: "VOICE",
        channel: kategori,
      });
    } else {
      uye.Leaders("stream", (value / 1000 / 60 / 2) * 10, {
        type: "VOICE",
        channel: kategori,
      });
      uye.Leaders("yayın", (value / 1000 / 60 / 2) * 10, {
        type: "VOICE",
        channel: kategori,
      });
      uye.Leaders("cam", (value / 1000 / 60 / 2) * 10, {
        type: "VOICE",
        channel: kategori,
      });
    }
  ...
  if (
    ver &&
    kategori &&
    kanallar.sorunCozmeKategorisi &&
    kanallar.sorunCozmeKategorisi == kategori.parentId
  ) {
    let uye = kategori.guild.members.cache.get(id);
    if (uye && uye.voice && (uye.voice.selfMute || uye.voice.selfDeaf)) {
      uye.Leaders("sorun", (value / 1000 / 60 / 2) * 2, {
        type: "VOICE",
        channel: kategori,
      });
      uye.Leaders("criminal", (value / 1000 / 60 / 2) * 2, {
        type: "VOICE",
        channel: kategori,
      });
      uye.Leaders("ceza", (value / 1000 / 60 / 2) * 2, {
        type: "VOICE",
        channel: kategori,
      });
    } else {
      uye.Leaders("sorun", (value / 1000 / 60 / 2) * 10, {
        type: "VOICE",
        channel: kategori,
      });
      uye.Leaders("criminal", (value / 1000 / 60 / 2) * 10, {
        type: "VOICE",
        channel: kategori,
      });
      uye.Leaders("ceza", (value / 1000 / 60 / 2) * 10, {
        type: "VOICE",
        channel: kategori,
      });
    }
  }
  let kategoriget = client.channels.cache.get(
    kategori ? kategori.parentId : "123",
  );
  if (
    ver &&
    kategori &&
    kategoriget &&
    (kategoriget.name.includes("Etkinlik") ||
      kategoriget.name.includes("etkinlik"))
  ) {
    let uye = kategori.guild.members.cache.get(id);
    if (uye && uye.voice && (uye.voice.selfMute || uye.voice.selfDeaf)) {
      uye.Leaders("etkinlik", (value / 1000 / 60 / 2) * 2, {
        type: "VOICE",
        channel: kategori,
      });
    } else {
      uye.Leaders("etkinlik", (value / 1000 / 60 / 2) * 10, {
        type: "VOICE",
        channel: kategori,
      });
    }
  ...
  if (
    ...
    kategori &&
    kategoriget &&
    (kategoriget.name.includes("oyun") ||
      kategoriget.name.includes("Oyun") ||
      kategoriget.name.includes("OYUN"))
  ) {
    let uye = kategori.guild.members.cache.get(id);
    if (uye && uye.voice && (uye.voice.selfMute || uye.voice.selfDeaf)) {
      uye.Leaders("oyun", (value / 1000 / 60 / 2) * 2, {
        type: "VOICE",
        channel: kategori,
      });
    } else {
      uye.Leaders("oyun", (value / 1000 / 60 / 2) * 10, {
        type: "VOICE",
        channel: kategori,
      });
    }
  ...
  if (kategori && kategori.parentId) {
    await Stats.updateOne(
      { userID: id, guildID: global.sistem.SERVER.ID },
      {
        $inc: {
          [`allVoice.${days}.${channel}`]: value,
          [`allCategory.${days}.${kategori.parentId}`]: value,
        },
      },
    );
  } else {
...
async function checkTasks(uye, data2) {
  ...
  let logEmbed = new genEmbed().setThumbnail(
    "https://preview.redd.it/qlne8pqzf0301.png?auto=webp&s=70da263d61a6d13378ff2b4aaab10e0bfa233fcf",
  );
  let exRoleYetki = _statSystem.staffs.find((x) => x.No == data2.staffExNo);
  let MissionData =
    (await Tasks.findOne({
      roleID: exRoleYetki ? exRoleYetki.rol : 0,
      Active: true,
    })) ||
    (await Tasks.findOne({
      roleID: uye.roles.hoist ? uye.roles.hoist.id : 0,
    })) ||
    (await Tasks.findOne({
      roleID: uye.roles.highest ? uye.roles.highest.id : 0,
    })) ||
    (await Tasks.findOne({ Users: uye.id }));
        ...
        amınakodumunKanalı.send({
          ...
          embeds: [
            new genEmbed()
              ...
              .setDescription(
                `Görev tamamlama zamanınız <t:${String(Date.now()).slice(0, 10)}:R> doldu!
...
`,
              ...
              .setFooter({
                text: `Anlık güncelleme ile "${rolGetir.name}" rolüne ait görev tamamiyle bitmiştir.`,
              }),
          ],
        });
    }
    await Tasks.updateOne(
      { roleID: MissionData.roleID },
      { $set: { Active: false } },
      { upsert: true },
    );
    return;
  ...
  if (data) {
    ...
    if (!data2.Mission.CompletedAllVoice) {
      if (Number(client.takachiSaatYap(genelses)) >= MissionData.AllVoice) {
        if (MissionData.AllVoice <= 0) return;
        if (görevLog)
          görevLog.send({
            embeds: [
              logEmbed.setDescription(
                `${uye} isimli görev sahibi, **Genel Ses** görevini <t:${String(Date.now()).slice(0, 10)}:R> tamamlayarak \`${MissionData.Reward ? MissionData.Reward : uPConf.points.tasks}  Görev Puanı\` kazandı.`,
              ),
            ],
          });
        ...
        await Upstaffs.updateOne(
          { _id: uye.id },
          { $set: { "Mission.CompletedAllVoice": true } },
          { upsert: true },
        );
        ...
        await Upstaffs.updateOne(
          { _id: uye.id },
          {
            $inc: {
              ...
              Point: MissionData.Reward
                ? MissionData.Reward
                : uPConf.points.tasks,
              ToplamPuan: MissionData.Reward
                ? MissionData.Reward
                : uPConf.points.tasks,
              Görev: MissionData.Reward
                ? MissionData.Reward
                : uPConf.points.tasks,
            },
          },
        ...
        await client.Economy.updateBalance(
          uye.id,
          Number(MissionData.Reward),
          "add",
          1,
        );
        return;
    ...
    if (!data2.Mission.CompletedPublicVoice) {
      if (Number(client.takachiSaatYap(public)) >= MissionData.publicVoice) {
        if (MissionData.public <= 0) return;
        if (görevLog)
          görevLog.send({
            embeds: [
              logEmbed.setDescription(
                `${uye} isimli görev sahibi, Ses **(**\`Sohbet, Streamer ve Kayıt\`**)**  görevini <t:${String(Date.now()).slice(0, 10)}:R> tamamlayarak \`${MissionData.Reward ? MissionData.Reward : uPConf.points.tasks}  Görev Puanı\` kazandı.`,
              ),
            ],
          });
        await Users.updateOne(
        ...
        await Upstaffs.updateOne(
          { _id: uye.id },
          { $set: { "Mission.CompletedPublicVoice": true } },
          { upsert: true },
        );
        //await Stats.updateOne({guildID: sistem.SERVER.ID, userID: uye.id}, {$set: {"taskVoiceStats": new Map()}}, {upsert: true})
        await Upstaffs.updateOne(
    ...
    if (
      MissionData.countTasks &&
      Number(MissionData.countTasks) > 0 &&
      Number(data2.Mission.completedMission) &&
      Number(data2.Mission.completedMission) >= 1
    ) {
      if (
        Number(data2.Mission.completedMission) >= Number(MissionData.countTasks)
      ) {
        if (
          MissionData.Completed &&
          MissionData.Completed.length > 0 &&
          MissionData.Completed.includes(uye.id)
        )
          return;
        if (görevLog)
          görevLog.send({
            content: `${uye}`,
            embeds: [
              new genEmbed().setThumbnail(
                "https://cdn.discordapp.com/attachments/985356878034911313/985361738813808700/unknown.png",
              )
...
async function voiceXP(id, xp, channel) {
  ...
  await Stats.updateOne(
    { guildID: sistem.SERVER.ID, userID: id },
    { $inc: { voiceXP: xp * voiceXP } },
    { upsert: true },
  );
  ...
  if (_getStat) {
    ...
    if (yeniLevel <= _getStat.voiceXP) {
      ...
      const image = new Canvas(740, 128)
        ...
        .setColor("#fff")
        .printText(
          `+${2500 * (_getStat.voiceLevel + 1)} ${ayarlar.serverName} Parası`,
          350,
          70,
          350,
        )
      ...
      let attach = new AttachmentBuilder(image.toBuffer(), {
        name: "takachi-seviye.png",
      });
...
Sesli sohbet seviyeniz yükseldi ve ödülleri kaptınız. (**\` ${_getStat.voiceLevel} -> ${_getStat.voiceLevel + 1} \`**)`,
        ...
        .then(async (msg) => {
          await client.Economy.updateBalance(
            uye.id,
            Number(2500 * (_getStat.voiceLevel + 1)),
            "add",
            1,
          );
