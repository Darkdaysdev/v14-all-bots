...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    if (!message.channel.name.includes("ship"))
      return message
        .reply(
          `${cevaplar.prefix} Sadece ${message.guild.channels.cache
            .filter((x) => x.name.includes("ship"))
            .map((x) => x)
            .join(", ")} kanallarında kullanabilirsiniz.`,
        )
        .then((x) => {
          message.react(
    ...
    if (cooldown.get(message.member.id))
      return message
        .reply({
          content: `Bu komutu **5** saniyede bir kullanabilirsiniz. ${cevaplar.prefix}`,
        })
        .then((x) =>
          setTimeout(() => {
            x.delete().catch((err) => {});
      ...
      person = message.guild.members.cache
        .filter(
          (m) =>
            ...
            !roller.kayıtsızRolleri.some((x) => m.roles.cache.get(x)),
        )
        .random();
        ...
        person = message.guild.members.cache
          .filter(
            (m) =>
              m.id !== message.author.id &&
              !roller.kayıtsızRolleri.some((x) => m.roles.cache.get(x)) &&
              roller.kadınRolleri.some((x) => m.roles.cache.get(x)),
          )
          .random();
      if (roller.kadınRolleri.some((x) => message.member.roles.cache.has(x)))
        person = message.guild.members.cache
          .filter(
            (m) =>
              m.id !== message.author.id &&
              !roller.kayıtsızRolleri.some((x) => m.roles.cache.get(x)) &&
              roller.erkekRolleri.some((x) => m.roles.cache.get(x)),
          )
          .random();
    }

    person = message.guild.members.cache.get(person.id);
    let özel = ["817463869487185980"];
    person._views();
    if (özel.includes(person.id))
      return message
        .reply({
          content: `${message.guild.emojiGöster(emojiler.Iptal)} Tabi Efendim Ship Atarsın.`,
        })
        .then((x) => {
    ...
    if (emoti <= 44 && emoti >= 23) {
      emoticon =
        "https://cdn.glitch.com/00963c7e-8e86-4a55-8d85-36add9e330d7%2Femoji_2.png?v=1593651528429";
    } else if (emoti < 23 && emoti >= 12) {
      emoticon =
        "https://cdn.glitch.com/00963c7e-8e86-4a55-8d85-36add9e330d7%2Femoji_3-1.png?v=1593652255529";
    } else if (emoti < 11) {
      emoticon =
        "https://cdn.glitch.com/00963c7e-8e86-4a55-8d85-36add9e330d7%2Femoji_1.png?v=1593651511900";
    }
    ...
    let background = await Canvas.loadImage(
      client.guilds.cache.get(sistem.SERVER.ID).bannerURL({ extension: "png" })
        ? client.guilds.cache
            .get(sistem.SERVER.ID)
            .bannerURL({ extension: "png" }) + `?size=4096`
        : "https://cdn.discordapp.com/attachments/1019290688136958028/1023941333716566046/thumb-1920-1146731.jpg",
    );
    ...
    message
      .reply({
        components: [Row],
        content: `${person}`,
        content: `[ **${person.displayName}** & **${message.member.displayName}** ]
Uyumluğu musun? **${love}**`,
        files: [img],
      })
      .then(async (msg) => {
        ...
        collector.on("collect", async (i) => {
          if (i.customId == "test") {
            i.reply({
              content: `Şuanlık **Tanışma Sistemi** Devre-dışı olduğundan dolayı sadece **DM** üzerinden iletişime geçebilirsin.`,
              ephemeral: true,
            });
          }
        });
