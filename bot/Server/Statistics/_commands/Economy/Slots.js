...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    if (
      !kanallar.coinChat.some((x) => message.channel.id == x) &&
      !ayarlar.staff.includes(message.member.id)
    )
      return message
        .reply(
          `${cevaplar.prefix} Sadece ${kanallar.coinChat.map((x) => message.guild.channels.cache.get(x)).join(",")} kanalların da oynayabilirsin.`,
        )
        .then((x) => {
          message.react(
    ...
    if (Beklet.has(message.author.id))
      return message
        .reply(
          `${cevaplar.prefix} \`Flood!\` Lütfen bir kaç saniye sonra tekrar oynamayı deneyin.`,
        )
        .then((x) => {
          message.react(
            message.guild.emojiGöster(emojiler.Iptal)
    ...
    if (isNaN(Miktar))
      return message
        .reply(
          `${cevaplar.prefix} Miktar yerine harf kullanmamayı tavsiye ederim.`,
        )
        .then((x) => {
          message.react(
            message.guild.emojiGöster(emojiler.Iptal)
    ...
    if (Miktar <= 0)
      return message
        .reply(
          `${cevaplar.prefix} Göndermek istediğiniz miktar, birden küçük olamaz.`,
        )
        .then((x) => {
          message.react(
            message.guild.emojiGöster(emojiler.Iptal)
    ...
    if (Miktar > 500000)
      return message
        .reply(
          `${cevaplar.prefix} Bahise en fazla \`250.000\` ${ayarlar.serverName} Parası ile girilebilir.`,
        )
        .then((x) => {
          message.react(
            message.guild.emojiGöster(emojiler.Iptal)
    ...
    if (Miktar < 10)
      return message
        .reply(
          `${cevaplar.prefix} Bahise en az \`10\` ${ayarlar.serverName} Parası ile girebilirsiniz.`,
        )
        .then((x) => {
          message.react(
            message.guild.emojiGöster(emojiler.Iptal)
    ...
    if (Coin < Miktar)
      return message
        .reply(
          `${cevaplar.prefix} Belirtiğiniz miktar kadar yeterince ${ayarlar.serverName} Parası olmadığından dolayı bahse giremezsiniz.`,
        )
        .then((x) => {
          message.react(
            message.guild.emojiGöster(emojiler.Iptal)
    ...
    let Kıbrıstakachi = [
      message.guild.emojiGöster("takachi_pat"),
      message.guild.emojiGöster("takachi_heart"),
      message.guild.emojiGöster("takachi_vis"),
    ];
    var SlotOne = Kıbrıstakachi[Math.floor(Math.random() * 3)];
    var SlotTwo = Kıbrıstakachi[Math.floor(Math.random() * 3)];
    var SlotThree = Kıbrıstakachi[Math.floor(Math.random() * 3)];
    ...
    message
      .reply({
        content: `
\`___SLOTS___\`
  ${message.guild.emojiGöster("takachi_slot")} ${message.guild.emojiGöster("takachi_slot")} ${message.guild.emojiGöster("takachi_slot")}
\`|         |\`
\`|         |\`
