...
module.exports = {
  ...
  Kullanim: "taşkağıtmakas <@takachi/ID> <Miktar>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if (!uye)
      return message
        .reply(`${cevaplar.prefix} Savaşmak istediğiniz bir üyeyi belirtin!`)
        .then((x) => {
          message.react(
            message.guild.emojiGöster(emojiler.Iptal)
              ? message.guild.emojiGöster(emojiler.Iptal).id
    ...
    if (uye.id === message.author.id)
      return message
        .reply(`${cevaplar.prefix} Kendi pipinle oynayamazsın :)`)
        .then((x) => {
          message.react(
            message.guild.emojiGöster(emojiler.Iptal)
              ? message.guild.emojiGöster(emojiler.Iptal).id
    ...
    if (!args[1])
      return message
        .reply(
          `${cevaplar.prefix} Lütfen bir miktar belirtin aksi taktirde oyuna başlayamazsınız.`,
        )
        .then((x) => {
          message.react(
            message.guild.emojiGöster(emojiler.Iptal)
    ...
    if (args[1] <= 0)
      returnmessage
        .reply(
          `${cevaplar.prefix} Girdiğiniz miktar lütfen sıfırdan büyük olmalıdır.`,
        )
        .then((x) => {
          message.react(
            message.guild.emojiGöster(emojiler.Iptal)
    ...
    if (data_1 > args[1] && data_2 > args[1]) {
      await Game({
        ...
        embed: {
          title: "xd",
          description:
            "Aşağıda ki elementlerden birini seçin! Taş mı? Kağıt mı? Makas mı?",
          color: "#5865F2",
          footer: "xd",
          timestamp: false,
        },
        ...
        time: 60000,
        acceptMessage: `<@{{opponent}}> Merhaba!
...
Berabere kalındığından dolayı ${message.member} ve ${uye} üyelerinin \`${args[1]} ${ayarlar.serverName} Parası\` her ikisinede tekrardan verildi.`,
        endMessage:
          "<@{{opponent}}> zamanında cevap vermedi. Oyun bitirildi ve paralar iade edildi!",
        timeEndMessage:
      ...
      return message
        .reply(
          `${cevaplar.prefix} Belirtiğiniz üyenin veya sizin belirttiğiniz miktarda parası bulunamadı.`,
        )
        .then((x) => {
          message.react(
            message.guild.emojiGöster(emojiler.Iptal)
