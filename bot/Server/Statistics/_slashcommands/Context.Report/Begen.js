...
module.exports = {
  ...
  run: async (client, int, args) => {
    ...
    if (member.id == author.id)
      return int.followUp({
        content: `${message.guild.emojiGöster(emojiler.Iptal)} Kendinizi beğenemezsiniz.`,
        ephemeral: true,
      });
    ...
    if (member_data.Likes.includes(author.id)) {
      ...
      await int.followUp({
        content: `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${member} isimli üyeyin beğenisini kaldırdınız.`,
        ephemeral: true,
      });
    } else {
      await Users.updateOne(
      ...
      await int.followUp({
        content: `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${member} isimli üyeyi beğendiniz.`,
        ephemeral: true,
      });
    }
  },
