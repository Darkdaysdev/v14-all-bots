...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if (!args[0] || isNaN(args[0]))
      return message.reply("Lütfen bir rol belirtin!");
    await roleBackup
      .findOne({ roleID: args[0] }, async (err, data) => {
        if (!data)
          return (
            message.channel.send(
              `${cevaplar.prefix} Belirtilen rol geçmişte bulunamadığından işlem iptal edildi `,
            ),
            message.react(
        ...
        await message.channel.send({
          embeds: [
            embed.setDescription(`${message.guild.emojiGöster(emojiler.Onay)} <@&${newRole.id}> (\`${newRole.id}\`) isimli rol oluşturuldu ve gereken ayarları yapıldı.
