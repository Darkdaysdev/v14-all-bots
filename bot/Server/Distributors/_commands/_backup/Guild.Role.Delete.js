...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if (!rol)
      message
        .reply({
          content: `${cevaplar.prefix} Lütfen geçerli bir rol belirtin!`,
        })
        .then((x) =>
          setTimeout(() => {
            x.delete().catch((err) => {});
        ...
        message
          .reply(message.guild.emojiGöster(emojiler.Iptal))
          .catch((err) => {});
    message
      .react(
      ...
      .send({
        embeds: [
          embed.setDescription(
            `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla **${rol.name}** (\`${rol.id}\`) isimli rol \`${message.guild.name}\` sunucusundan silindi.`,
          ),
        ],
      })
      .then((x) => {
