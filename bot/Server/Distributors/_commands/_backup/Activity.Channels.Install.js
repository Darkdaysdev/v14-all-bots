...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if (!args[0] || isNaN(args[0]))
      return message.reply("Lütfen bir kategori ID'si belirtin.");
    CategoryChannels.findOne({ channelID: args[0] }, async (err, data) => {
      if (!data)
        return (
          message.channel.send(
            `${cevaplar.prefix} Belirtilen kategori kanalı geçmişte bulunamadığından işlem iptal edildi `,
          ),
          message.react(
      ...
      await message.channel.send({
        embeds: [
          embed.setDescription(
            `**${newChannel.name}** isimli kategorinin, \`${tarihsel(Date.now())}\` tarihli kategori yedeği kurulmaya başladı.`,
          ),
        ],
      });
...
async function voiceKur(idcik, message, embed, parentID) {
  VoiceChannels.findOne({ channelID: idcik }, async (err, data) => {
    if (!data)
      return (
        message.channel.send(
          `${cevaplar.prefix} Belirtilen ses kanal geçmişte bulunamadığından işlem iptal edildi `,
        ),
        message.react(
          message.guild.emojiGöster(emojiler.Iptal)
            ? message.guild.emojiGöster(emojiler.Iptal).id
    ...
    await message.channel.send({
      embeds: [
        embed.setDescription(
          `**${newChannel.name}** isimli ses kanalının, \`${tarihsel(Date.now())}\` tarihli ses kanalı kurulmaya başladı.`,
        ),
      ],
    });
    const newOverwrite = [];
...
async function textKur(idcik, message, embed, parentID) {
  TextChannels.findOne({ channelID: idcik }, async (err, data) => {
    if (!data)
      return (
        message.channel.send(
          `${cevaplar.prefix} Belirtilen metin kanal geçmişte bulunamadığından işlem iptal edildi `,
        ),
        message.react(
          message.guild.emojiGöster(emojiler.Iptal)
            ? message.guild.emojiGöster(emojiler.Iptal).id
    ...
    await message.channel.send({
      embeds: [
        embed.setDescription(
          `**${newChannel.name}** isimli metin kanalının, \`${tarihsel(Date.now())}\` tarihli metin kanalı kurulmaya başladı.`,
        ),
      ],
    });
    const newOverwrite = [];
