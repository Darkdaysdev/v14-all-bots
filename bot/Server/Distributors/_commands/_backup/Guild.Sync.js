...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
      ...
      .send({
        embeds: [
          embed.setDescription(
            `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla **${message.guild.name}** sunucusunun rol ve kanal senkronizasyonu güncellendi.`,
          ),
        ],
      })
      .then((x) => {
    ...
    client.logger.log(
      `ROL => Manuel olarak senkronizasyon işlemi gerçekleştirildi. (${message.author.username})`,
      "backup",
    );
  },
};
