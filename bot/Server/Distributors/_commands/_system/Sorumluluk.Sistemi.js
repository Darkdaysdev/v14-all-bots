...
module.exports = {
          ...
          .addComponents(
            ...
            new ActionRowBuilder().addComponents(
              new TextInputBuilder()
                ...
                .setMinLength(5)
                .setMaxLength(100)
                .setPlaceholder("Örn: takachi#0001/ID")
                .setRequired(false),
            ),
            new ActionRowBuilder().addComponents(
              new TextInputBuilder()
    ...
    client.on("interactionCreate", async (modal) => {
      ...
      if (modal.customId == "sorumlulukBasvuru") {
        ...
        if (!getir) {
          await modal.deferReply({ ephemeral: true });
          return await modal.followUp({
            content: `Sistemsel bir hata oluştu. ${cevaplar.prefix}`,
            ephemeral: true,
          });
        }
        ...
        if (uye.roles.cache.has(sorumlulukRolü)) {
          await modal.deferReply({ ephemeral: true });
          return await modal.followUp({
            content: `Üzerinizde bu sorumluluk rolü bulunduğu için işlem iptal edildi. ${cevaplar.prefix}`,
            ephemeral: true,
          });
        }
        ...
        let embed = new genEmbed()
          ...
          .setDescription(
            `${uye} isimli üye (**${sorumlulukIsmi}**) ${guild.roles.cache.get(sorumlulukRolü)} rolü için  <t:${Number(String(Date.now()).substring(0, 10))}:F> tarihinde başvurdu.`,
          );
        embed.addFields({
              ...
              refernas
                ? `${
                    guild.members.cache.find(
                      (x) =>
                        x.user.username === refernas ||
                        x.user.globalName.includes(refernas) ||
                        x.id === refernas,
                    )
                      ? guild.members.cache.find(
                          (x) =>
        ...
        let yetkiler = uye.roles.cache.filter(
          (rol) => altYetki.position <= rol.position,
        );
        if (yetkiler)
          embed.addFields({
        ...
        logKanalı.send({
          content: `${Selector.map((x) => guild.roles.cache.get(x)).join(", ")}`,
          embeds: [embed],
        });
        await modal.deferReply({ ephemeral: true });
        Select.delete(uye.id);
        return await modal.followUp({
          content: `Başarıyla ${guild.roles.cache.get(sorumlulukRolü)} rolüne başvurunuz iletilmiştir! Gerekli kontrollerden ve denetimlerden sonra ${Selector.map((x) => guild.roles.cache.get(x)).join(", ")} rolüne sahip yöneticiler seninle ilgilenecektir. Tebrikler!`,
          ephemeral: true,
        });
      }
      ...
      if (modal.customId == "sorumluluk-ekle") {
        ...
        if (role && !guild.roles.cache.get(role)) {
          await modal.deferReply({ ephemeral: true });
          return await modal.followUp({
            content: `Belirtilen sorumluluk rolü sunucuda bulunamadı.`,
            ephemeral: true,
          });
        }
        ...
        if (
          leaders &&
          !leaders.split(" ").every((x) => guild.roles.cache.get(x))
        ) {
          await modal.deferReply({ ephemeral: true });
          return await modal.followUp({
            content: `Belirtilen sorumluluk lider rolleri sunucuda bulunamadı.`,
            ephemeral: true,
          });
        }
        ...
        await modal.deferReply({ ephemeral: true });
        return await modal.followUp({
          content: `${guild.emojiGöster(emojiler.Onay)} Başarıyla ${name} sorumluluğu başarıyla <t:${String(Date.now()).slice(0, 10)}:R> eklendi.`,
          ephemeral: true,
        });
      }
    });
  ...
  onRequest: async function (client, message, args) {
    ...
    let msg = await message.reply({
      content: `Sorumluluk sistemi yüklenirken bekleyin...`,
    });
    let Row = new ActionRowBuilder().addComponents(Buttons);
    msg.edit({
      embeds: [
        new genEmbed().setDescription(
          `Aşağıda bulunan düğmeler ile sorumluluk sistemini yönetebilirsin.`,
        ),
      ],
      content: null,
      components: [Row],
...
Oluşturan: ${message.guild.members.cache.get(x.created) ? message.guild.members.cache.get(x.created) : `<@${x.created}>`} (<t:${String(x.date).slice(0, 10)}:R>)`,
        ...
        msg.edit({
          embeds: [
            new genEmbed()
              .setDescription(`Aşağı da ${message.guild.name} sunucusuna ait sorumluluklar listelenmektedir.
...
${_data.join("\n──────────────────────────\n")}`),
      ...
      if (i.customId == "reset") {
        ...
        msg.edit({
          embeds: [
            new genEmbed().setDescription(
              `Başarıyla tüm sorumluluklar sıfırlandı. ${message.guild.emojiGöster(emojiler.Onay)}`,
            ),
          ],
          content: null,
          components: [],
      ...
      if (i.customId == "silSorumluluk") {
        ...
        msg.edit({
          embeds: [
            new genEmbed().setDescription(
              `Başarıyla **${_get ? _get.name : "@"}** sorumluluğu silindi. ${message.guild.emojiGöster(emojiler.Onay)}`,
            ),
          ],
          content: null,
          components: [],
        ...
        if (_data && _data.length > 0) {
          ...
          msg.edit({
            embeds: [
              new genEmbed()
                .setDescription(
                  `Aşağıda bulunan ve listelenmekte olan sorumluluklardan hangisi silmek istersin?`,
                )
                .setFooter({
                  text: "Minimum 1 tane rol kalana kadar rahatlıkla silebilirsiniz.",
                }),
            ],
            content: null,
            components: [Roww],
          ...
          i.reply({
            content: `Silinebilcek bir sorumluluk bulunamadığından işlem iptal edildi.`,
            ephemeral: true,
          });
        }
      }
      if (i.customId == "install") {
        ...
        if (_data && _data.length > 0) {
          ...
          msg.edit({
            embeds: [
              new genEmbed().setDescription(
                `Başarıyla sorumluluk sistemi kurulumu tamamlandı. ${message.guild.emojiGöster(emojiler.Onay)}`,
              ),
            ],
            content: null,
            components: [],
...
Aşağıda bulunan başvurusu açık sorumluluk rollerinden başvurmak istediğinizi seçiniz.`,
          ...
          i.reply({
            content: `Başarıyla ayarlamış olduğunuz sorumluluk başvuru sistemi açıldı. ${message.guild.emojiGöster(emojiler.Onay)}`,
            ephemeral: true,
          });
        } else {
          i.reply({
            content: `Kurulum yapabilmeniz için en az bir sorumluluk oluşturmalısınız.`,
            ephemeral: true,
          });
        }
      }
          ...
          .addComponents(
            ...
            new ActionRowBuilder().addComponents(
              new TextInputBuilder()
                ...
                .setMaxLength(1024)
                .setPlaceholder(
                  "Rol ID'si verilmelidir. Birden fazla için aralarına boşluk koyunuz.",
                )
                .setRequired(true),
            ),
          );
