...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    let load = await message.reply({
      content: `Hoş geldin botları yeniden başlatılırken. Lütfen bekleyin.`,
    });
    const ls = children.exec(`pm2 restart Voices`);
    ls.stdout.on("data", function (data) {
      load
        .edit({
          content: `Başarıyla hoş geldin ses botları yeniden başlatıldı. ${message.guild.emojiGöster(emojiler.Onay)}`,
        })
        .then((x) => {
          message
            .react(
