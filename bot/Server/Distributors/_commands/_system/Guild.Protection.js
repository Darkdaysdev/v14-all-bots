  ...
  onRequest: async function (client, message, args) {
    ...
    switch (type) {
      case "kapat": {
        ...
        let roller = message.guild.roles.cache
          .filter((rol) => rol.editable)
          .filter((rol) => perms.some((yetki) => rol.permissions.has(yetki)));
        message.channel
          .send({
            embeds: [
              new genEmbed().setDescription(
                `${message.guild.emojiGöster(emojiler.Onay)} ${roller.map((x) => x).join(", ")} rolün(lerin), koruması başarıyla <t:${String(Date.now()).slice(0, 10)}:R> açıldı ve izinleri kapatıldı.`,
              ),
            ],
          })
          .then((x) => {
          ...
          await Roles.updateOne(
            { Role: rol.id },
            {
              $set: {
                ...
                Reason: "Koruma Komutu Çalıştırıldı!",
      ...
      case "aç": {
        ...
        Roller.filter((x) => message.guild.roles.cache.get(x.Role)).forEach(
          async (data) => {
        ...
        return message.channel
          .send({
            embeds: [
              new genEmbed().setDescription(
                `${message.guild.emojiGöster(emojiler.Onay)} ${Roller.map((x, key) => message.guild.roles.cache.get(x.Role)).join(", ")} rolün(lerin), koruması başarıyla <t:${String(Date.now()).slice(0, 10)}:R> kapatıldı ve izinleri tekrardan açıldı.`,
              ),
            ],
          })
          .then((x) => {
