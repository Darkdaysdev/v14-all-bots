...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let yetkiler = data.filter((m) => {
      ...
      return (
        uyeToplam2 <= ms("12h") &&
        message.guild.members.cache.has(m.userID) &&
        !uye.permissions.has(Discord.PermissionFlagsBits.Administrator) &&
        !uye.permissions.has(Discord.PermissionFlagsBits.ManageRoles) &&
    ...
    let yetkiliListesi = yetkiler.forEach((m, index) => {
      let yetkiliCheck = new Promise(async function (yetkili, yetkilidegil) {
        ...
        if (get && get.Staff) {
          if (
            staffget &&
            staffget.Baslama &&
            staffget.Baslama >= Date.now() - ms("10h")
          ) {
      ...
      let uye = message.guild.members.cache.get(m.userID);
      if (yetkiliCheck) {
        yetkiliCheck.then(function (value) {
          {
            mesaj += `<@${m.userID}> (${çevirSüre(m.lifeTotalVoiceStats)}) [**Yetkisi Çekildi**] (${value})\n`;
            message.channel.send(
              `<@${m.userID}> (${çevirSüre(m.lifeTotalVoiceStats)}) [**${value == "yeni yetkili" ? "Yeni Yetkili" : value ? "Yetkisi Çekildi" : "Yetkisi Çekilmedi"}**] (<t:${Number(String(Date.parse(uye.joinedAt)).substring(0, 10))}:R>)\n`,
            );
          }
        });
      }
      ...
      .send({
        embeds: [
          embed.setDescription(
            `${mesaj.length >= 1 ? mesaj : "İşleminiz biraz sonra başlayacak, lütfen bekleyin."}`,
          ),
        ],
      })
      .catch((err) => {
        const arr = splitMessages(
          `${mesaj.length >= 1 ? mesaj : "Tebrikler! 12 Saat Altı Bir Yetkili Bulunamadı!"}`,
          { maxLength: 1950, char: "\n" },
        );
        arr.forEach((element) => {
          message.channel.send({
            embeds: [embed.setDescription(`${element}`)],
          });
        });
      });
  },
