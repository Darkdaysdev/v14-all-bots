...
module.exports = {
  ...
  onLoad: async function (client) {
    ...
    let guardSettings = await GUARDS_SETTINGS.findOne({
      guildID: sistem.SERVER.ID,
    });
    if (!guardSettings)
      await GUARDS_SETTINGS.updateOne(
        { guildID: sistem.SERVER.ID },
    ...
    allTokens.forEach(async (token) => {
      ...
      botClient.on("ready", async () => {
        ...
        if (!callbacks.TOKENS.WELCOME.tokens.includes(botClient.token)) {
          if (
            guardSettings &&
            guardSettings.BOTS &&
            !guardSettings.BOTS.includes(botClient.user.id)
          ) {
            await GUARDS_SETTINGS.updateOne(
              { guildID: sistem.SERVER.ID },
              { $push: { BOTS: botClient.user.id } },
              { upsert: true },
            );
          }
        }
    ...
    let msg = await message.channel.send({
      embeds: [
        new genEmbed()
          ...
          .setDescription(
            `Aşağıda sıralanmakta olan botların ismini, profil fotoğrafını, durumunu ve hakkındasını değişmesini istediğiniz bir botu seçiniz.`,
          ),
      ],
      components: [Row],
    });
      ...
      if (i.customId == "selectBot") {
        ...
        if (!type)
          return await i.reply({
            content: "Bir bot veya işlem bulunamadı!",
            ephemeral: true,
          });
          ...
          .send({
            embeds: [
              new genEmbed()
                .setColor("White")
                .setDescription(
                  `${botClient.user} (**${botClient.user.username}**) isimli bot üzerinde yapmak istediğiniz değişikliliği seçiniz?`,
                ),
            ],
                ...
                msg.edit({
                  embeds: [
                    new genEmbed()
                      .setColor("Random")
                      .setDescription(
                        `${message.guild.emojiGöster(emojiler.Icon)} ${botClient.user} isimli botun yeni profil resmini yükleyin veya bağlantısını girin. İşlemi iptal etmek için (**iptal**) yazabilirsiniz. (**Süre**: \`60 Saniye\`)`,
                      ),
                  ],
                  components: [],
                });
                ...
                col.on("collect", async (m) => {
                  if (m.content == ("iptal" || "i")) {
                    ...
                    message
                      ...
                      .catch((err) => {});
                    await i.reply({
                      content: `${cevaplar.prefix} Profil resmi değiştirme işlemi iptal edildi.`,
                      ephemeral: true,
                    });
                    return;
                  }
                  let eskinick = botClient.user.avatarURL({ extension: "png" });
                  let bekle = await message.reply(
                  ...
                  if (!isim) {
                    ...
                    msg.delete().catch((err) => {});
                    await i.reply({
                      content: `${cevaplar.prefix} Profil resmi belirtilmediği için işlem iptal edildi.`,
                      ephemeral: true,
                    });
                    return;
                  }
                  botClient.user
                      ...
                      if (logChannel)
                        logChannel.send({
                          embeds: [
                            new genEmbed()
                              .setDescription(
                                `${message.member} tarafından ${botClient.user} isimli botun profil resmi değiştirildi.`,
                              )
                      ...
                      message.channel
                        .send({
                          embeds: [
                            new genEmbed()
                              .setDescription(
                                `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla! ${botClient.user} isimli botun profil resmi güncellendi!`,
                              )
                              .setThumbnail(botClient.user.avatarURL()),
                          ],
                        })
                    ...
                    .catch((err) => {
                      ...
                      message.channel
                        .send(
                          `${cevaplar.prefix} **${botClient.user.username}**, Başarısız! profil resmi güncelleyebilmem için biraz beklemem gerek!`,
                        )
                        .then((x) => {
                          message
                            .react(
                ...
                msg.edit({
                  embeds: [
                    new genEmbed()
                      .setColor("Random")
                      .setDescription(
                        `${message.guild.emojiGöster(emojiler.Icon)} ${botClient.user} isimli botun yeni ismini belirtin. İşlemi iptal etmek için (**iptal**) yazabilirsiniz. (**Süre**: \`60 Saniye\`)`,
                      ),
                  ],
                  components: [],
                });
                ...
                col.on("collect", async (m) => {
                  if (m.content == ("iptal" || "i")) {
                    ...
                    message
                      ...
                      .catch((err) => {});
                    await i.reply({
                      content: `${cevaplar.prefix} İsim değiştirme işlemi iptal edildi.`,
                      ephemeral: true,
                    });
                    return;
                  }
                  ...
                  botClient.user
                        ...
                        logChannel.send({
                          embeds: [
                            new genEmbed().setDescription(
                              `${message.member} tarafından ${botClient.user} isimli botun ismi değiştirildi.\n**${eskinick}** \` ••❯ \` **${botClient.user.username}** olarak güncellendi.`,
                            ),
                          ],
                        });
                      message.channel
                        .send({
                          embeds: [
                            new genEmbed().setDescription(
                              `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla! **${eskinick}** \` ••❯ \` **${botClient.user.username}** olarak değiştirildi.`,
                            ),
                          ],
                        })
                        .then((x) => {
                    ...
                    .catch((err) => {
                      ...
                      message.channel
                        .send(
                          `${cevaplar.prefix} **${botClient.user.username}**, Başarısız! isim değiştirebilmem için biraz beklemem gerek!`,
                        )
                        .then((x) => {
                          message
                            .react(
