    ...
    if (sunucuData) {
      if (eski === sunucuData.tag) {
        await guildSettings.updateOne(
          ...
          { $set: { "Ayarlar.tag": yeni } },
          { upsert: true },
        );
        message.channel.send({
          embeds: [
            new genEmbed().setDescription(
              `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla sunucunun yeni tag'ı \`${yeni}\` olarak belirlendi.`,
            ),
          ],
        });
      }
