...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    const takachi = message.content.split(" ");
    if (!takachi[1])
      return message
        .react(
          message.guild.emojis.cache.find((emoji) => emoji.name === "Iptal")
            ? message.guild.emojis.cache.find((emoji) => emoji.name === "Iptal")
                .id
            : undefined,
        )
        .catch((err) => message.reply("Kod belirtmedin."));
    if (takachi[1] === "client")
      return message
        .react(
          message.guild.emojis.cache.find((emoji) => emoji.name === "Iptal")
    ...
    try {
      const code = takachi.slice(1).join(" ");
