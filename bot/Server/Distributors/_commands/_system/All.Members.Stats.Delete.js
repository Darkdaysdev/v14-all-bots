      ...
      .send({
        embeds: [
          new genEmbed().setDescription(
            `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla **${tarihsel(Date.now() - ms("7d"))}** tarihinden itibaren olan tüm veriler temizlendi.`,
          ),
        ],
      })
      .then(async (x) => {
        await StatsSchema.updateMany(
        ...
        stats
          .filter((s) => !message.guild.members.cache.has(s.userID))
          .forEach((s) => StatsSchema.findByIdAndDelete(s._id));
        await StatsSchema.updateMany(
          { guildID: message.guild.id },
          {
            voiceStats: new Map(),
