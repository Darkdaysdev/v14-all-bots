...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let guardSettings = await GUARD_SETTINGS.findOne({
      guildID: message.guild.id,
    });
    let guildSettings = await GUILD_SETTINGS.findOne({
    ...
    let whiteListRow = new ActionRowBuilder().addComponents([
      new ButtonBuilder()
        ...
        .setStyle(
          guardSettings.unManageable.includes(id)
            ? ButtonStyle.Success
      ...
      new ButtonBuilder()
        ...
        .setStyle(
          guardSettings.fullAccess.includes(id)
            ? ButtonStyle.Success
            : ButtonStyle.Secondary,
        ),
      new ButtonBuilder()
        ...
        .setStyle(
          guardSettings.guildAccess.includes(id)
            ? ButtonStyle.Success
            : ButtonStyle.Secondary,
        ),
    ]);
    let whiteListRowTwo = new ActionRowBuilder().addComponents([
      new ButtonBuilder()
        ...
        .setStyle(
          guardSettings.rolesAcess.includes(id)
            ? ButtonStyle.Success
            : ButtonStyle.Secondary,
        ),
      new ButtonBuilder()
        ...
        .setStyle(
          guardSettings.channelsAccess.includes(id)
            ? ButtonStyle.Success
            : ButtonStyle.Secondary,
        ),
      new ButtonBuilder()
        ...
        .setStyle(
          guardSettings.memberAccess.includes(id)
            ? ButtonStyle.Success
            : ButtonStyle.Secondary,
        ),
      new ButtonBuilder()
        ...
        .setStyle(
          guardSettings.emojiAccess.includes(id)
            ? ButtonStyle.Success
            : ButtonStyle.Secondary,
        ),
      new ButtonBuilder()
        ...
        .setStyle(
          guardSettings.botAccess.includes(id)
            ? ButtonStyle.Success
            : ButtonStyle.Secondary,
        ),
    ]);
    let whiteListRowThree = new ActionRowBuilder().addComponents([
      new ButtonBuilder()
        ...
        .setStyle(
          ayarlar.chatİzinliler.includes(id)
            ? ButtonStyle.Success
            : ButtonStyle.Secondary,
        ),
    ]);
    ...
    let anaMenu = new genEmbed()
      ...
      .setDescription(
        `Aşağı da \`${ayarlar ? (ayarlar.serverName ? ayarlar.serverName : message.guild.name) : message.guild.name}\` sunucusunun güvenlik sistemi ve güvenliğe takılmış kayıtlar bulunmaktadır. Ayarları güncellemek istiyorsanız aşağıda ki düğmeleri kullanabilirsiniz.
...
${
  guardSettings.Process
    ? guardSettings.Process.length > 0
      ? guardSettings.Process.sort((a, b) => b.date - a.date)
          .slice(0, 10)
          .map(
            (x, index) =>
              `\` ${index + 1} \` <@!${x.target}> [**${x.type}**] ${x.member ? `(<@!${x.member}>) ` : ""}(<t:${String(x.date).slice(0, 10)}:R>)`,
          )
          .join("\n")
      : "Yakın zamanda bir kayıt bulunamadı."
    : "Yakın zamanda bir kayıt bulunamadı."
}`,
      )
      .setFooter({
        text: `Bir üyeyi beyaz listeye ekleyip/çıkartmak için ${global.sistem.botSettings.Prefixs[0]}guard <@takachi/ID> komutunu kullanabilirsiniz.`,
      });
    if (!uye) {
      message.channel
        .send({ embeds: [anaMenu], components: [oneRows] })
        .then(async (msg) => {
          ...
          collector.on("collect", async (i) => {
            ...
            if (i.customId == "backup") {
              const {
                guildBackup,
              } = require("../../../../Global/Init/Guild.Backup");
              await guildBackup.guildChannels();
              await guildBackup.guildRoles();
              oneRows.components[3]
                .setDisabled(true)
                .setLabel("Yedekleme Başarıyla Alındı!")
            ...
            if (i.customId == "guard_manager") {
              ...
              let takachiRow = new ActionRowBuilder().addComponents([
                new ButtonBuilder()
                  .setCustomId("guardState")
                  .setLabel(
                    `Güvenliği ${guardSettings.guildProtection ? "Kapat" : "Aç"}`,
                  )
                  .setEmoji("943285868368633886")
                  .setStyle(
                    guardSettings.guildProtection
                ...
                new ButtonBuilder()
                  ...
                  .setEmoji("943290446329835570")
                  .setStyle(
                    ayarlar.SMS ? ButtonStyle.Secondary : ButtonStyle.Danger,
                  ),
                new ButtonBuilder()
                  .setCustomId("wpState")
                  .setLabel(
                    ayarlar.SMS
                      ? `WhatsApp/Telegram ${ayarlar.WHATSAPP || ayarlar.TELEGRAM ? "Kapat" : "Aç"}`
                      : "SMS/MMS Gerekli",
                  )
                  ...
                  .setStyle(
                    ayarlar.SMS
                      ? ayarlar.WHATSAPP || ayarlar.TELEGRAM
                        ? ButtonStyle.Secondary
                        : ButtonStyle.Danger
                      : ButtonStyle.Success,
                  ),
              ...
              msg.delete().catch((err) => {});
              message.channel
                .send({ embeds: [anaMenu], components: [takachiRow, backMenu] })
                .then(async (gmsg) => {
                  var filter = (i) => i.user.id == message.member.id;
                  let collector = gmsg.createMessageComponentCollector({
                    filter: filter,
                    time: 120000,
                  });
                  ...
                  collector.on("collect", async (i) => {
                    ...
                    if (i.customId == "guardState") {
                      ...
                      if (guardSettings.guildProtection) {
                        takachiRow.components[2].setDisabled(true);
                        takachiRow.components[3].setDisabled(true);
                        takachiRow.components[1].setDisabled(true);
                        takachiRow.components[4].setDisabled(true);
                        takachiRow.components[0]
                          .setStyle(ButtonStyle.Danger)
                          .setLabel(`Güvenliği Aç`);
                        await GUARD_SETTINGS.updateOne(
                          { guildID: sistem.SERVER.ID },
                          { $set: { guildProtection: false } },
                          { upsert: true },
                        );
                        ...
                        takachiRow.components[1].setDisabled(false);
                        if (guardSettings.limit)
                          takachiRow.components[2].setDisabled(false);
                        takachiRow.components[3].setDisabled(false);
                        if (ayarlar.SMS)
                          takachiRow.components[4].setDisabled(false);
                        takachiRow.components[0]
                          .setStyle(ButtonStyle.Secondary)
                          .setLabel(`Güvenliği Kapat`);
                        await GUARD_SETTINGS.updateOne(
                          { guildID: sistem.SERVER.ID },
                          { $set: { guildProtection: true } },
                          { upsert: true },
                        );
                      }
                    }
                        ...
                        .send({
                          embeds: [
                            new genEmbed()
                              .setColor("White")
                              .setDescription(
                                `Aşağı da \`${ayarlar ? (ayarlar.serverName ? ayarlar.serverName : message.guild.name) : message.guild.name}\` sunucusunun limit ayarları belirtilmektedir.
...
Olarak ayarlanmış. Ayarı güncellemek için dakika cinsinde işlem süresi belirtin.`,
                              ...
                              .setFooter({
                                text: `Ana menüye dönmek için "iptal" yazabilirsiniz.`,
                              }),
                          ],
                        })
                        .then(async (ymsg) => {
                          var filter = (m) => m.author.id == message.member.id;
                          let collector = ymsg.channel.createMessageCollector({
                            filter: filter,
                            max: 1,
                            time: 30000,
                          });
                          collector.on("collect", async (m) => {
                            ...
                            if (!sure)
                              return m
                                .reply(
                                  `${cevaplar.prefix} Lütfen rakam girilmeli veyada boş bırakılmamalıdır.`,
                                )
                                .then((x) =>
                                  setTimeout(() => {
                                    x.delete().catch((err) => {});
                              ...
                              .send({
                                embeds: [
                                  new genEmbed()
                                    .setColor("White")
                                    .setDescription(
                                      `Başarıyla Limit Zamanlaması Ayarlandı!

\` ••❯ \` **Limit Ayarları**: \` ${moment.duration(ms(surecik)).format("h [Saatde,] m [Dakikada]")} ${guardSettings.auditLimit} İşlem Hakkı Bulunmakta! \`

Olarak ayarlanmış fakat işlem sayısı ayarlanmamış. Ayarlamak için lütfen kaç işlem olacağını belirtin.`,
                                    )
                                    .setFooter({
                                      text: `Ana menüye dönmek için "iptal" yazabilirsiniz.`,
                                    }),
                              ...
                              .then(async (yarramm) => {
                                var filter = (m) =>
                                  m.author.id == message.member.id;
                                let collector =
                                  yarramm.channel.createMessageCollector({
                                    filter: filter,
                                    max: 1,
                                    time: 30000,
                                  });
                                collector.on("collect", async (m) => {
                                  ...
                                  yarramm
                                    .edit({
                                      embeds: [
                                        new genEmbed()
                                          .setDescription(`${message.guild.emojiGöster(emojiler.Onay)} Başarıyla sunucunun limit ayarları güncellendi.
...
Olarak ayarlandı.`),
                                    ...
                                    .then(async (takachi) => {
                                      var filter = (i) =>
                                        ...
                                        i.customId == "anamenucuk";
                                      let collector =
                                        yarramm.createMessageComponentCollector(
                                          {
                                            filter: filter,
                                            max: 1,
                                            time: 30000,
                    ...
                    if (i.customId == "smsState") {
                      ...
                      if (guildSettings) ayarlar = guildSettings.Ayarlar;
                      if (ayarlar.SMS) {
                        takachiRow.components[3]
                          .setStyle(ButtonStyle.Danger)
                          .setLabel(`SMS/MMS Aç`);
                        takachiRow.components[4]
                          .setDisabled(true)
                          .setStyle(ButtonStyle.Success)
                          .setLabel(`SMS/MMS Gerekli`);
                        await GUILD_SETTINGS.updateOne(
                          { guildID: sistem.SERVER.ID },
                          { $set: { "Ayarlar.SMS": false } },
                          { upsert: true },
                        );
                        await GUILD_SETTINGS.updateOne(
                          { guildID: sistem.SERVER.ID },
                          {
                            $set: {
                              "Ayarlar.WHATSAPP": false,
                              "Ayarlar.TELEGRAM": false,
                            },
                          },
                          { upsert: true },
                        );
                        ...
                        takachiRow.components[4]
                          .setDisabled(false)
                          .setLabel(`WhatsApp/Telegram Aç`)
                          .setStyle(ButtonStyle.Danger);
                        takachiRow.components[3]
                          .setStyle(ButtonStyle.Secondary)
                          .setLabel(`SMS/MMS Kapat`);
                        await GUILD_SETTINGS.updateOne(
                          { guildID: sistem.SERVER.ID },
                          { $set: { "Ayarlar.SMS": true } },
                          { upsert: true },
                        );
                      }
                    }
                    if (i.customId == "wpState") {
                      ...
                      if (ayarlar.WHATSAPP | ayarlar.TELEGRAM) {
                        ...
                        takachiRow.components[4]
                          .setStyle(ButtonStyle.Secondary)
                          .setLabel(`WhatsApp/Telegram Kapat`);
                        await GUILD_SETTINGS.updateOne(
                          { guildID: sistem.SERVER.ID },
                          {
                            $set: {
                              "Ayarlar.WHATSAPP": true,
                              "Ayarlar.TELEGRAM": true,
                            },
                          },
                          { upsert: true },
                        );
  ...
  guardSettings.Process
    ...
    : "Yakın zamanda bir kayıt bulunamadı."
                    ...
                    gmsg
                      .edit({
                        embeds: [anaMenu],
                        components: [takachiRow, backMenu],
                      })
                      .catch((err) => {});
                    i.deferUpdate().catch((err) => {});
                  });
            ...
            if (i.customId == "guard_manage") {
              ...
              let listEmbed = new genEmbed()
                ...
                .setDescription(
                  `Aşağı da \`${ayarlar ? (ayarlar.serverName ? ayarlar.serverName : message.guild.name) : message.guild.name}\` sunucusunun beyaz listede ki üyeleri ve rolleri listelenmektedir.`,
                )
                .addFields({
                  name: "Dokunulmaz",
                  value: `${
                    guardSettings.unManageable.filter(
                      (x) =>
                        message.guild.members.cache.get(x) ||
                        message.guild.channels.cache.get(x) ||
                        message.guild.roles.cache.get(x),
                    ).length >= 1
                      ? guardSettings.unManageable
                          .filter(
                            (x) =>
                              message.guild.members.cache.get(x) ||
                              message.guild.channels.cache.get(x) ||
                          ...
                          .map(
                            (x, index) =>
                              `\` ${index + 1} \` ${message.guild.roles.cache.get(x) || message.guild.members.cache.get(x) || x}`,
                          )
                          .join("\n")
                ...
                .addFields({
                  name: "Limitli Full Yönetim",
                  value: `${
                    guardSettings.fullAccess.filter(
                      (x) =>
                        message.guild.members.cache.get(x) ||
                        message.guild.channels.cache.get(x) ||
                        message.guild.roles.cache.get(x),
                    ).length >= 1
                      ? guardSettings.fullAccess
                          .filter(
                            (x) =>
                              message.guild.members.cache.get(x) ||
                              message.guild.channels.cache.get(x) ||
                ...
                .addFields({
                  name: "Sunucu (Düzenleme)",
                  value: `${
                    guardSettings.guildAccess.filter(
                      (x) =>
                        message.guild.members.cache.get(x) ||
                        message.guild.channels.cache.get(x) ||
                        message.guild.roles.cache.get(x),
                    ).length >= 1
                      ? guardSettings.guildAccess
                          .filter(
                            (x) =>
                              message.guild.members.cache.get(x) ||
                              message.guild.channels.cache.get(x) ||
                ...
                .addFields({
                  name: "Bot (Ekleme/Çıkarma)",
                  value: `${
                    guardSettings.botAccess.filter(
                      (x) =>
                        message.guild.members.cache.get(x) ||
                        message.guild.channels.cache.get(x) ||
                        message.guild.roles.cache.get(x),
                    ).length >= 1
                      ? guardSettings.botAccess
                          .filter(
                            (x) =>
                              message.guild.members.cache.get(x) ||
                              message.guild.channels.cache.get(x) ||
                ...
                .addFields({
                  name: "Rol (Oluşturma/Kaldırma/Düzenleme)",
                  value: `${
                    guardSettings.rolesAcess.filter(
                      (x) =>
                        message.guild.members.cache.get(x) ||
                        message.guild.channels.cache.get(x) ||
                        message.guild.roles.cache.get(x),
                    ).length >= 1
                      ? guardSettings.rolesAcess
                          .filter(
                            (x) =>
                              message.guild.members.cache.get(x) ||
                              message.guild.channels.cache.get(x) ||
                ...
                .addFields({
                  name: "Kanal (Oluşturma/Kaldırma/Düzenleme)",
                  value: `${
                    guardSettings.channelsAccess.filter(
                      (x) =>
                        message.guild.members.cache.get(x) ||
                        message.guild.channels.cache.get(x) ||
                        message.guild.roles.cache.get(x),
                    ).length >= 1
                      ? guardSettings.channelsAccess
                          .filter(
                            (x) =>
                              message.guild.members.cache.get(x) ||
                              message.guild.channels.cache.get(x) ||
                ...
                .addFields({
                  name: "Sağ-Tık (Rol/Ban/Kick/Sustur/Bağlantı)",
                  value: `${
                    guardSettings.memberAccess.filter(
                      (x) =>
                        message.guild.members.cache.get(x) ||
                        message.guild.channels.cache.get(x) ||
                        message.guild.roles.cache.get(x),
                    ).length >= 1
                      ? guardSettings.memberAccess
                          .filter(
                            (x) =>
                              message.guild.members.cache.get(x) ||
                              message.guild.channels.cache.get(x) ||
                ...
                .addFields({
                  name: "Emoji/Sticker (Oluştur/Kaldır/Düzenle)",
                  value: `${
                    guardSettings.emojiAccess.filter(
                      (x) =>
                        message.guild.members.cache.get(x) ||
                        message.guild.channels.cache.get(x) ||
                        message.guild.roles.cache.get(x),
                    ).length >= 1
                      ? guardSettings.emojiAccess
                          .filter(
                            (x) =>
                              message.guild.members.cache.get(x) ||
                              message.guild.channels.cache.get(x) ||
      ...
      message.channel
        .send({
          embeds: [
            new genEmbed()
              .setDescription(
                `**Merhaba!** ${message.member.user.username}
...
**Bilgilendirme**:
Belirtilen üye, rol veya kanal ekli bir beyaz listede bulunuyorsa düğmenin rengi yeşile dönmektedir, bulunmuyor ise düğme füme rengine dönmektedir. :tada:`,
              )
              .setColor("Yellow"),
          ],
          components: [whiteListRow, whiteListRowTwo, whiteListRowThree],
        })
        .then(async (msg) => {
          var filter = (i) => i.user.id == message.member.id;
          let collector = msg.createMessageComponentCollector({
          ...
          collector.on("collect", async (i) => {
            if (i.customId == "dokunulmaz") {
              ...
              if (guardSettings.unManageable.includes(id)) {
                await GUARD_SETTINGS.updateOne(
                  { guildID: sistem.SERVER.ID },
                  { $pull: { unManageable: id } },
                  { upsert: true },
                );
                await whiteListRow.components[0].setStyle(
                  ButtonStyle.Secondary,
                );
              } else if (!guardSettings.unManageable.includes(id)) {
                await GUARD_SETTINGS.updateOne(
                  { guildID: sistem.SERVER.ID },
                  { $push: { unManageable: id } },
                  { upsert: true },
                );
              ...
              if (guardSettings.fullAccess.includes(id)) {
                ...
                await whiteListRow.components[1].setStyle(
                  ButtonStyle.Secondary,
                );
              ...
              if (guardSettings.guildAccess.includes(id)) {
                ...
                await whiteListRow.components[2].setStyle(
                  ButtonStyle.Secondary,
                );
              ...
              if (guardSettings.rolesAcess.includes(id)) {
                ...
                await whiteListRowTwo.components[0].setStyle(
                  ButtonStyle.Secondary,
                );
              ...
              if (guardSettings.channelsAccess.includes(id)) {
                await GUARD_SETTINGS.updateOne(
                  { guildID: sistem.SERVER.ID },
                  { $pull: { channelsAccess: id } },
                  { upsert: true },
                );

                await whiteListRowTwo.components[1].setStyle(
                  ButtonStyle.Secondary,
                );
              } else if (!guardSettings.channelsAccess.includes(id)) {
                await GUARD_SETTINGS.updateOne(
                  { guildID: sistem.SERVER.ID },
                  { $push: { channelsAccess: id } },
                  { upsert: true },
                );
                await whiteListRowTwo.components[1].setStyle(
                  ButtonStyle.Success,
              ...
              if (guardSettings.memberAccess.includes(id)) {
                await GUARD_SETTINGS.updateOne(
                  { guildID: sistem.SERVER.ID },
                  { $pull: { memberAccess: id } },
                  { upsert: true },
                );

                await whiteListRowTwo.components[2].setStyle(
                  ButtonStyle.Secondary,
                );
              } else if (!guardSettings.memberAccess.includes(id)) {
                await GUARD_SETTINGS.updateOne(
                  { guildID: sistem.SERVER.ID },
                  { $push: { memberAccess: id } },
                  { upsert: true },
                );
                await whiteListRowTwo.components[2].setStyle(
                  ButtonStyle.Success,
              ...
              if (guardSettings.emojiAccess.includes(id)) {
                ...
                await whiteListRowTwo.components[3].setStyle(
                  ButtonStyle.Secondary,
                );
              ...
              if (guardSettings.botAccess.includes(id)) {
                ...
                await whiteListRowTwo.components[4].setStyle(
                  ButtonStyle.Secondary,
                );
              ...
              if (ayarlar.chatİzinliler.includes(id)) {
                await GUILD_SETTINGS.updateOne(
                  { guildID: sistem.SERVER.ID },
                  { $pull: { "Ayarlar.chatİzinliler": id } },
                  { upsert: true },
                );
                await whiteListRowThree.components[0].setStyle(
                  ButtonStyle.Secondary,
                );
              } else if (!ayarlar.chatİzinliler.includes(id)) {
                await GUILD_SETTINGS.updateOne(
                  { guildID: sistem.SERVER.ID },
                  { $push: { "Ayarlar.chatİzinliler": id } },
                  { upsert: true },
                );
                await whiteListRowThree.components[0].setStyle(
                  ButtonStyle.Success,
