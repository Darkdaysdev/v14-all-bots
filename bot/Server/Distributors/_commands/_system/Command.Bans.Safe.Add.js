  ...
  onRequest: async function (client, message, args) {
    ...
    message.channel.send({
      embeds: [
        new genEmbed().setDescription(
          `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye} isimli üyenin \`${message.guild.name}\` sunucusunda ki komut yasağı \`${tarihsel(Date.now())}\` tarihinde kaldırıldı.`,
        ),
      ],
    });
    ...
    if (logLa)
      logLa.send({
        embeds: [
          new genEmbed().setDescription(
            `${uye} isimli üyenin \`${message.guild.name}\` sunucusunda ki komut yasağı ${message.member} tarafından \`${tarihsel(Date.now())}\` tarihinde kaldırıldı.`,
          ),
        ],
      });
  },
