...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    let msg = await message.reply({
      content: `Yapılandırma hizmeti yükleniyor... Lütfen bekleyin!`,
    });
    ...
    collector.on("collect", async (i) => {
      if (i.customId == "isimyapilandirma") {
          ...
          .forEach(async (uye) => {
            ...
            if (data && data.Name && data.Gender == "Kayıtsız") {
              if (uye && uye.manageable && ayarlar.type && ayarlar.isimyas)
                await uye
                  .setNickname(
                    `${uye.user.globalName.includes(ayarlar.tag) ? ayarlar.tag : ayarlar.tagsiz ? ayarlar.tagsiz : ayarlar.tag || ""} ${data.Name}`,
                  )
                  .catch((err) => {});
              if (uye && uye.manageable && !ayarlar.type && ayarlar.isimyas)
                await uye.setNickname(`${data.Name}`).catch((err) => {});
              if (uye && uye.manageable && ayarlar.type && !ayarlar.isimyas)
                await uye
                  .setNickname(
                    `${uye.user.globalName.includes(ayarlar.tag) ? ayarlar.tag : ayarlar.tagsiz ? ayarlar.tagsiz : ayarlar.tag || ""} ${data.Name}`,
              ...
              if (uye && uye.manageable && ayarlar.type && ayarlar.isimyas)
                await uye
                  ...
                  .catch((err) => {});
              if (uye && uye.manageable && !ayarlar.type && ayarlar.isimyas)
                await uye.setNickname(`İsim | Yaş`).catch((err) => {});
              if (uye && uye.manageable && !ayarlar.type && !ayarlar.isimyas)
                await uye.setNickname(`Kayıtsız`).catch((err) => {});
              if (uye && uye.manageable && ayarlar.type && !ayarlar.isimyas)
                await uye
                  .setNickname(
        ...
        i.reply({
          content: `**Başarıyla!** ${message.guild.name} sunucusunun isimleri yapılandırıldı! ${message.guild.emojiGöster(emojiler.Onay)}`,
          ephemeral: true,
        });
      }
    });
