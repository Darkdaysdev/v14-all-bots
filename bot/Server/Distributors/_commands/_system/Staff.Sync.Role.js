...
module.exports = {
  ...
  Kullanim: "y u <@takachi/ID> <Yetki S.Kodu>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if (işlem === "u") {
      ...
      if (yetkiKodu > _statSystem.staffs.length)
        return message.react(
          message.guild.emojiGöster(emojiler.Iptal)
            ? message.guild.emojiGöster(emojiler.Iptal).id
            : undefined,
      ...
      if (yeniYetki) {
        ...
        if (!uye.roles.cache.has(roller.altilkyetki))
          await uye.roles.add(roller.altilkyetki);
      }
      message.react(
      ...
      message.guild.kanalBul("terfi-log").send({
        embeds: [
          embed.setDescription(
            `${message.member} isimli yetkili ${uye} isimli üyeyi <t:${String(Date.now()).slice(0, 10)}:R> ${yeniYetki.rol ? message.guild.roles.cache.get(yeniYetki.rol) : "Bulunamadı!"} role senkronize etti.`,
          ),
        ],
      });
    } else if (işlem === "r") {
      ...
      rol.members.forEach(async (uye) => {
        ...
        if (_statSystem.staffs.some((x) => uye.roles.cache.has(x.rol))) {
          let takachi = _statSystem.staffs.find((x) =>
            uye.roles.cache.has(x.rol),
          );
          let No = Number(takachi.No);
          if (!uye.roles.cache.has(roller.altilkyetki))
            await uye.roles.add(roller.altilkyetki);
          await Upstaffs.updateOne(
            { _id: uye.id },
