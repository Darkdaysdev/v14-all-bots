    ...
    client.on("interactionCreate", async (modal) => {
      ...
      if (modal.customId == "tp-kaldır") {
        ...
        let cmd = check.talentPerms.find(
          (takachi) => takachi.Commands == cmdName,
        );
        if (!cmd) {
          await modal.deferReply({ ephemeral: true });
          return await modal.followUp({
            content: `Belirtilen isimde aktif bir komut bulunmakta. ${cevaplar.prefix}`,
            ephemeral: true,
          });
        }
        await TalentPerms.updateOne(
          { guildID: guild.id },
        ...
        await modal.deferReply({ ephemeral: true });
        return await modal.followUp({
          content: `Başarıyla ${cmdName} komutu <t:${String(Date.now()).slice(0, 10)}:R> kaldırıldı. ${guild.emojiGöster(emojiler.Onay)}`,
          ephemeral: true,
        });
      }
      if (modal.customId == "tp-detay") {
        ...
        return await modal.followUp({
          embeds: [
            new genEmbed().setThumbnail(guild.iconURL({ extension: "png" }))
...
**Kullanacak rol(ler)**:
${cmd.Permission ? cmd.Permission.map((x) => guild.roles.cache.get(x)).join(", ") : "@rol bulanamadı"} rol veya rolleri kullanabilir.`),
          ],
          ephemeral: true,
        });
      }
      if (modal.customId == "tp-ekle") {
        let cmdName = modal.fields.getTextInputValue("tp_isim");
        let cmdType =
          modal.fields.getTextInputValue("tp_kullancakroller") || [];
        let cmdContent =
          modal.fields.getTextInputValue("tp_vericekroller") || [];
        ...
        let cmd = check.talentPerms
          ? check.talentPerms.find((takachi) => takachi.Commands == cmdName)
          : undefined;
        if (cmd) {
          await modal.deferReply({ ephemeral: true });
          return await modal.followUp({
        ...
        if (
          (_Roles && !_Roles.some((x) => guild.roles.cache.has(x))) ||
          (_Permission && !_Permission.some((x) => guild.roles.cache.has(x)))
        ) {
          await modal.deferReply({ ephemeral: true });
          return await modal.followUp({
            content: `Belirtilen rol veya roller ${guild.name} sunucusunda bulunamadı. ${cevaplar.prefix}`,
            ephemeral: true,
          });
        }
        await TalentPerms.updateOne(
        ...
        await modal.deferReply({ ephemeral: true });
        return await modal.followUp({
          content: `Başarıyla ${cmdName} komutu <t:${String(Date.now()).slice(0, 10)}:R> eklendi. ${guild.emojiGöster(emojiler.Onay)}`,
          ephemeral: true,
        });
      }
    });
  ...
  onRequest: async function (client, message, args) {
    ...
    let komutListe = [];
    if (Tp && TalentPerm) {
      TalentPerm.filter((x) => !Array.isArray(x.Commands)).forEach((x) =>
        komutlar.push({
          name: x.Commands,
          roles: `${x.Roles.map((a) => (message.guild.roles.cache.get(a) ? message.guild.roles.cache.get(a) : "@rol bulunamadı")).join(", ")}`,
        }),
      );
      TalentPerm.filter((x) => !Array.isArray(x.Commands)).forEach((data) => {
        komutListe.push([
          {
            label: başHarfBüyült(data.Commands),
            value: data.Commands,
            emoji: { id: "1023821496025612359" },
            description: `${data.Roles.map((x) => (message.guild.roles.cache.get(x) ? message.guild.roles.cache.get(x).name : "@rol bulunamadı")).join(", ")} veriyor.`,
          },
        ]);
      });
    }
    ...
    load.edit({
      ...
      embeds: [
        new genEmbed()
          .setDescription(
            `Aşağıda ${message.guild.name} sunucusuna ait rol (ver/al) komut oluşturma, görüntüleme ve kaldırma işlemi yapabilirsiniz.\n
...
${
  komutlar.length > 0
    ? `Aşağıda sunucuda bulunan alt komut veya rol (ver/al) komutları listelenmekte:
${komutlar.map((x) => `${message.guild.emojiGöster("takachi_arrow")} **${x.name}** (${x.roles})`).join("\n")}`
    : ``
}`,
          )
        ...
        load.edit({
          ...
          embeds: [
            new genEmbed().setDescription(
              `Zaman aşımına uğradığı için işleminiz sonlandırıldı. ${cevaplar.prefix}`,
            ),
          ],
        });
        setTimeout(() => {
