      ...
      if (ayar.staff.includes(uye.id)) {
          ...
          .send({
            embeds: [
              embed.setDescription(
                `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye} isimli üye, <t:${String(Date.now()).slice(0, 10)}:R> bottan kaldırıldı.`,
              ),
            ],
          })
          .then((x) => {
        ...
        message.channel
          .send({
            embeds: [
              embed.setDescription(
                `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye} isimli üye, <t:${String(Date.now()).slice(0, 10)}:R> bot'a \`Tester\` olarak eklendi.`,
              ),
            ],
          })
          .then((x) => {
...
Tester (all)\`\`\`${
        ayar.staff
          ? ayar.staff
              .filter((x) => x != "317518644705755138")
              .map(
                (x, index) =>
                  `\` ${index + 1} \` ${message.guild.members.cache.get(x) ? message.guild.members.cache.get(x) : x}`,
              )
              .join("\n")
