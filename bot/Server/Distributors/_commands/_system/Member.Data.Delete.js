  ...
  onRequest: async function (client, message, args) {
    ...
    if (!uye)
      return message.channel.send(
        `${message.guild.emojiGöster(emojiler.Iptal)} Bir üye belirtmelisin.`,
      );
    if (!(await Kullanıcı.findOne({ _id: uye.id })))
      return message.channel.send(
        `${cevaplar.prefix} ${uye} profiline sahip üyenin sunucu üzerinde verisi bulunamadı.`,
      );
    await uye.Delete();
    ...
    await message.channel.send({
      embeds: [
        new genEmbed().setDescription(
          `${message.guild.emojiGöster(emojiler.Onay)} ${uye} üyesinin tüm verileri ve tüm kayıt bilgileri sunucudan temizlendi.`,
        ),
      ],
    });
    await message.react(
