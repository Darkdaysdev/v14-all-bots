      ...
      if (Database && Database.length >= 1) {
        for (let index = 0; index < Database.length; index++) {
          ...
          if (Data.Secret == menu) {
            ...
            if (!check)
              interaction.reply({
                content: "Bu menü sistemden kaldırıldığı için kullanılamıyor.",
                ephemeral: true,
              });
            ...
            if (interaction.values[0] === "rolsil") {
              ...
              if (
                check.Access &&
                check.Access.length > 0 &&
                !check.Access.some((r) => member.roles.cache.has(r)) &&
                ...
                return interaction.reply({
                  content: `Üzerinizde ${check.Access.filter((x) =>
                    member.guild.roles.cache.has(x),
                  )
                    .map((x) => member.guild.roles.cache.get(x))
                    .join(
                      ", ",
                    )} rol(ler) bulunmadığından bu seçeneği seçemezsiniz.`,
                  ephemeral: true,
                });
  ...
  onRequest: async function (client, message, args) {
    ...
    if (Data && Data.length >= 1) {
      ...
      Data.forEach(async (x) => {
        listele.push({
          label: x.Name,
          description: `${tarihsel(x.Date)} ${message.guild.members.cache.get(x.Author) ? `- ${message.guild.members.cache.get(x.Author).user.username}` : ""}`,
          value: x.Name,
        });
      });
      comp = [
    ...
    message.channel
      .send({
        embeds: [
          new genEmbed()
            .setDescription(`**Merhaba!** ${message.member.user.username} (${message.member}),
...
\` ❯ \` Düzenleme işlemi yaparken tekrardan aşağıda ki düğmeye basarak, düzenlenmesini istediğiniz rol seçim menüsü ismini girerek tekrardan ayarlarını güncelleyebilirsiniz.`),
        ],
        components: comp,
      })
      .then(async (msg) => {
        ...
        collector.on("collect", async (i) => {
          if (i.customId == "sil") {
            ...
            i.reply({
              content: `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla **${i.values}** isimli rol seçim menüsü silindi.`,
              ephemeral: true,
            });
          ...
          if (i.customId == "kur") {
            ...
            if (kurulcak) {
              ...
              kurulcak.Roles.forEach((r) => {
                Opt.push({
                  label: message.guild.roles.cache.get(r)
              ...
              i.reply({
                content: `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla **${kurulcak.Name}** isimli rol seçim menüsü kuruldu.`,
                ephemeral: true,
              });
              msg.delete().catch((err) => {});
              message.react(
          ...
          if (i.customId == "ekle") {
            ...
            message.channel
              .send({
                content: `:tada: **${ayarlar.serverName ? ayarlar.serverName : message.guild.name}**
Yeni bir rol seçim menüsü oluşturuluyor...`,
                embeds: [
...
Yeni oluşturulmakta olan rol seçim menünüze bir isim belirleyin.`),
              ...
              .then(async (isimbelirleme) => {
                ...
                let collector = isimbelirleme.channel.createMessageCollector({
                  filter: filt,
                  time: 60000,
                  max: 1,
                  errors: ["time"],
                ...
                collector.on("collect", async (m) => {
                  ...
                  if (mesaj == "iptal" || mesaj == "ıptal") {
                    return isimbelirleme
                      .edit({
                        content: null,
                        embeds: [
                          new genEmbed().setDescription(
                            `${message.guild.emojiGöster(emojiler.Iptal)} Başarıyla rol seçim menü oluşturma aracı iptal edildi.`,
                          ),
                        ],
                      })
                      .then((x) => {
...
Yeni oluşturulmakta olan rol seçim menünüze bir açıklama belirtin. Örn: \`Aşağıda ki rollerden istediğiniz rolü alabilirsiniz!\``),
                    ...
                    .then(async (açıklamabelirleme) => {
                      var filt = (m) => m.author.id == message.member.id;
                      let collector =
                        açıklamabelirleme.channel.createMessageCollector({
                          filter: filt,
                          time: 60000,
                          max: 1,
                          errors: ["time"],
...
Yeni oluşturulmakta olan rol seçim menünüzde listelenecek rolleri belirtin.`,
                                )
                                .setFooter({
                                  text: `En az 3 tane, en fazla 25 tane rol ekleyebilirsiniz.`,
                                }),
                            ],
                          })
                          .then(async (rolbelirleme) => {
                            ...
                            collector.on("collect", async (m) => {
                              ...
                              if (m.mentions.roles.size >= 1) {
                                rolPushing = m.mentions.roles.map(
                                  (role) => role.id,
                                );
                              } else {
                                let argss = m.content.split(" ");
                                argss = argss.splice(0);
                                let rolVerAbime = argss.filter((role) =>
...
${message.guild.emojiGöster(emojiler.Onay)} Başarıyla **${rolSeçim.Name}** isimli rol seçim menüsü \`${tarihsel(Date.now())}\` tarihinde oluşturuldu.`),
                                  ],
                                })
