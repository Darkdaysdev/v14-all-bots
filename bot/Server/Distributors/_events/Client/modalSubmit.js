...
module.exports = async (modal) => {
  ...
  if (modal.customId == "isimDüzenleme") {
    if (
      ...
      roller.özelRoller &&
      !roller.özelRoller.some((x) => uye.roles.cache.has(x)) &&
      !uye.permissions.has(Discord.PermissionFlagsBits.Administrator)
    ) {
      await modal.deferReply({ ephemeral: true });
      return await modal.followUp({
    ...
    if (ayarlar.type && ayarlar.isimyas) {
      if (
        ...
        let Nickname = uye.nickname
          .replace(ayarlar.tagsiz, "")
          .replace(ayarlar.tag, "")
          .replace(" ", "")
          .split(" | ")[0];
        if (Nickname && uye.manageable) {
          uye
            .setNickname(uye.displayName.replace(Nickname, isim))
            .catch((err) => {});
          await modal.deferReply({ ephemeral: true });
          return await modal.followUp({
            content: `Başarılı şekilde sunucuda isminiz güncellendi.`,
          });
        } else {
          await modal.deferReply({ ephemeral: true });
          return await modal.followUp({
            content: `İsim değiştirirken bir hata oluştu veya botun yetkisi size yetmiyor.`,
          });
        }
      }
    }
    ...
    let isim = modal.fields.getTextInputValue("isim");
    if (ayarlar.type)
      yazilacakIsim = `${uye.user.globalName.includes(ayarlar.tag) ? ayarlar.tag : ayarlar.tagsiz ? ayarlar.tagsiz : ayarlar.tag || ""} ${isim}`;
    if (!ayarlar.type) yazilacakIsim = `${isim}`;
    if (uye.manageable) {
      uye
        .setNickname(`${yazilacakIsim}`)
        .then(async (devam) => {
          await modal.deferReply({ ephemeral: true });
          return await modal.followUp({
            content: `Başarılı şekilde sunucuda isminiz güncellendi.`,
        ...
        logKanal.send({
          embeds: [
            new genEmbed()
              .setColor("Random")
              .setFooter({
                text: uye.user.username + " tarafından güncellendi.",
                iconURL: uye.user.avatarURL({ extension: "png" }),
              })
              .setDescription(
                `${guild.name} sunucusunda güncelleme ${uye} tarafından yapıldı.
...
`,
      ...
      return await modal.followUp({
        embeds: [
          new genEmbed().setColor("Random")
            .setDescription(`Başarıyla ${guild.name} sunucusu üzerinde işlem(ler) <t:${String(Date.now()).slice(0, 10)}:F> tarihinde gerçekleştirildi/güncelleştirildi.
...
\`\`\``),
      ...
      await modal.deferReply({ ephemeral: true });
      return await modal.followUp({
        content: `Sunucu üzerinde bir değişiklik yapılmadı.`,
        ephemeral: true,
      });
    }
  }
