      ...
      await findChannel.send({
        embeds: [
          new genEmbed().setDescription(
            `${ban.user} uyesinin sunucudaki ${res ? `\`#${res.No}\` ceza numaralı yasaklaması` : "yasaklaması"}, <t:${String(Date.now()).slice(0, 10)}:R> ${entry.executor} tarafından kaldırıldı.`,
          ),
        ],
      });
  });
