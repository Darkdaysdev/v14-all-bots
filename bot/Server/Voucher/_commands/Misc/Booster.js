...
module.exports = {
  ...
  onRequest: async (client, message, args) => {
    ...
    if (
      ...
      roller.özelRoller &&
      !roller.özelRoller.some((x) => message.member.roles.cache.has(x)) &&
    ...
    if (zaman.get(message.author.id) >= 1)
      return (
        message.reply({
          content: `${message.guild.emojiGöster(emojiler.Iptal)} Sunucu takviyecisi özellik komutunu sadece **15 Dakika** ara ile kullanabilirsin.`,
          ephemeral: true,
        }),
        message.react(
    ...
    if (ayarlar.type && ayarlar.isimyas) {
      if (
        ...
        if (Nickname && message.member.manageable) {
          message.member
            .setNickname(message.member.displayName.replace(Nickname, isim))
            .catch((err) => {});
    ...
    if (!isim)
      return message
        ...
        .then((s) => setTimeout(() => s.delete().catch((err) => {}), 5000));
    if (ayarlar.type)
      yazilacakIsim = `${message.member.user.globalName.includes(ayarlar.tag) ? ayarlar.tag : ayarlar.tagsiz ? ayarlar.tagsiz : ayarlar.tag || ""} ${isim}`;
