...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if (!role)
      return message
        .reply({
          content: `Belirtilen argümanlarda rol'e ait herhangi bir bilgi bulunamadı.`,
        })
        .then((x) => {
          message
            .react(
    ...
    let load = await message.reply({
      content: `Belirtilen ${role.name} rolünün bilgileri getirilirken lütfen bekleyin.`,
    });
    let offlinemembers = role.members.filter((x) => !x.presence);
    let sestemembers = role.members.filter(
      (x) => x.presence && x.presence !== "offline" && x.voice.channel,
    );
    let sesteolmayanaktif = role.members.filter(
      (x) => x.presence && x.presence !== "offline" && !x.voice.channel,
    );
    let offlineamaseste = role.members.filter(
      (x) => !x.presence && x.voice.channel,
    );
...
─────────────────────`;
    ...
    load.edit({ content: `${text}` }).catch((err) => {
      ...
      load.edit({
        content: `Belirtilen ${role.name} rolünde 2048 karakteri aşan listeleme bulunduğundan aşağıya yeni mesaj şeklinde listeleyecektir.`,
      });
      arr.forEach((element) => {
