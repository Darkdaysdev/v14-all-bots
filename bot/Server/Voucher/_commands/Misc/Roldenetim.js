...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    const role =
      ...
      message.guild.roles.cache.find((x) => x.name.includes(args[0])) ||
      message.guild.rolBul(args[0]);
    if (!role)
    ...
    let list = 1;
    let veri =
      `${tarihsel(Date.now())} Tarihinde ${message.member.user.username} tarafından istenmiştir!\n` +
      role.members
        .map((e) =>
          e
            ? `#${list++} ••❯ ID: ${e.id} - İsim: ${e.displayName} - ${e.voice.channel ? "Seste" : "Seste Değil"}`
            : "sa",
        )
        .join("\n");
...
${Discord.Formatters.codeBlock("fix", "Rol: " + role.name + " | " + role.id + " | " + role.members.size + " Toplam Üye | " + unVoice.size + " Seste Olmayan Üye")}`,
    ...
    message.channel
      .send(
        `Aşağıda **${role.name}** (\`${role.id}\`) isimli rolünün seste olmayan üyeleri sıralandırılmıştır.`,
      )
      .then((xx) => {
        const arr = splitMessages(
          `${unVoice.map((e) => `<@${e.id}>`).join(", ")}`,
          { maxLength: 1950, char: "," },
        );
        arr.forEach((element) => {
