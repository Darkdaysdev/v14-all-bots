...
module.exports = {
  ...
  Kullanim: "komutdenetim @takachi/ID",
    ...
    Users.findOne({ _id: uye.id }, async (err, res) => {
      if (!res)
        return message.channel
          .send(`${cevaplar.prefix} Komut bilgisi bulunamadı.`)
          .then((x) =>
            setTimeout(() => {
              x.delete();
            }, 7500),
      ...
      if ((!pages && !pages.length) || !pages[currentPage - 1])
        return message
          .reply(`${cevaplar.prefix} Komut bilgisi bulunamadı.`)
          .then((x) =>
            setTimeout(() => {
              x.delete();
            }, 7500),
        ...
        .send({
          embeds: [
            embed.setDescription(
              `${uye}, üyesin komut bilgileri yükleniyor... Lütfen bekleyin...`,
            ),
          ],
          components: [row],
          fetchReply: true,
      ...
      await curPage
        .edit({
          embeds: [
            embed.setDescription(`${uye} isimli üyesinin toplamda \`${res.CommandsLogs.length || 0}\` adet komut kullanımı mevcut.
          ...
          .edit({
            embeds: [
              embed.setFooter({
                ...
                .setDescription(`${uye} isimli üyesinin toplamda \`${res.CommandsLogs.length || 0}\` adet komut kullanımı mevcut.
            ...
            .edit({
              embeds: [
                embed
                  ...
                  .setDescription(
                    `${uye} isimli üyesinin toplamda \`${res.CommandsLogs.length || 0}\` adet komut kullanımı mevcut.`,
                  ),
              ],
              components: [],
            })
