...
module.exports = {
  Isim: "seskontrol",
  Komut: ["sesk", "n"],
  Kullanim: "seskontrol @takachi/ID",
  Aciklama:
    "Belirlenen üyenin seste aktif veya haporleri ve kulaklığının açık veya kapalı olduğunu gösterir.",
  Kategori: "yönetim",
  Extend: true,
  /**
   * @param {Client} client
  ...
  onLoad: function (client) {
    client.on("voiceStateUpdate", async (oldState, newState) => {
      ...
      if (!oldState.channelId && newState.channelId)
        await joinedAt.findOneAndUpdate(
          { _id: newState.id },
          { $set: { date: Date.now() } },
          { upsert: true },
        );
  ...
  onRequest: async (client, message, args) => {
      ...
      return message.channel.send({
        embeds: [
          embed.setDescription(
            `${member} adlı kullanıcı herhangi bir ses kanalında değil.`,
          ),
        ],
      });
    ...
    message.channel.send({
      embeds: [
        embed.setDescription(`${member}, isimli üye şuan da ${member.voice.channel} kanalında bulunuyor.
