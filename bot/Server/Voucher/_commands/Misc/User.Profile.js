...
module.exports = {
  Isim: "profil",
  Komut: ["me", "info"],
  Kullanim: "profil <@takachi/ID>",
  Aciklama:
    "Belirlenen kişinin veya kullanan kişinin sunucu içerisindeki detaylarını ve discord içerisindeki bilgilerini aktarır.",
  Kategori: "diğer",
  Extend: true,
  ...
  onRequest: async function (client, message, args) {
    ...
    if (uye.presence && uye.presence.status !== "offline") {
      bilgi = `\`•\` Bağlandığı Cihaz: ${platform[Object.keys(uye.presence.clientStatus)[0]]}`;
    } else {
      bilgi = "`•` Bağlandığı Cihaz: Çevrimdışı `🔻`";
    }
    let takipçi = yetkiliKullanim.Follower
      ? yetkiliKullanim.Follower.filter((x) =>
          message.guild.members.cache.get(x),
        ).length
      : 0;
    let arkadaşş = yetkiliKullanim.Friends
      ? yetkiliKullanim.Friends.filter((x) =>
          message.guild.members.cache.get(x),
        ).length
      : 0;
    let goruntulenme = yetkiliKullanim.Views || 0;
    let begeni = yetkiliKullanim.Likes
      ? yetkiliKullanim.Likes.filter((x) => message.guild.members.cache.get(x))
          .length
      : 0;
    let takipçiPuan =
      Number(takipçi * 3.5) +
    ...
    const embed = new genEmbed()
      .setAuthor({
        ...
        iconURL: kullanici.user.avatarURL({ extension: "png", size: 2048 }),
      })
      .setThumbnail(kullanici.user.avatarURL({ extension: "png", size: 2048 }))
      .addFields([
        {
...
\`•\` Rolleri (\`${uye.roles.cache.size - 1 >= 0 ? uye.roles.cache.size - 1 : 0}\`): ${
            uye.roles.cache.size <= 5
              ? uye.roles.cache
...
\`•\` Görütülenme: \`${yetkiliKullanim.Views || 0}\` Beğeni: \`${yetkiliKullanim.Likes ? yetkiliKullanim.Likes.filter((x) => message.guild.members.cache.get(x)).length : 0}\``,
    ...
    if (await uye.voice.channel) {
      ...
      embed.addFields({
        name: `${message.guild.emojiGöster("support")} **Sesli Kanal Bilgisi**`,
        value: uyesesdurum,
      });
    }
    if (
      roller.Yetkiler.some((x) => uye.roles.cache.has(x)) ||
      roller.kurucuRolleri.some((oku) => uye.roles.cache.has(oku)) ||
      uye.permissions.has(Discord.PermissionFlagsBits.Adminisrator)
    ) {
        ...
        .setOptions(
          {
            ...
            description: `${uye.user.username} üyesinin sunucu içerisinde aktifliğini gösterir.`,
            emoji: { id: "948674910425853993" },
            value: "statcim",
          },
          {
            label: "Ekonomi Durumu",
            description: `${uye.user.username} üyesinin ekonomi durumunu gösterir.`,
            emoji: { id: "948674949567111248" },
            value: "coincim",
          },
          {
            label: "Ceza Geçmişi",
            description: `${uye.user.username} üyesinin ceza geçmişini listelenir.`,
            emoji: { id: "948677924561752104" },
            value: "cezalarim",
          },
          {
            label: "Ses Geçmişi",
            description: `${uye.user.username} üyesinin ses kayıtlarını gösterir.`,
            emoji: { id: "948679866562277456" },
            value: "sesgecmisim",
          },
          {
            label: "Arkadaş Listesi",
            description: `${uye.user.username} üyesinin arkadaş listesini görüntüler.`,
            emoji: { id: "943286130357444608" },
            value: "arkadaşListesi",
          },
          {
            label: "Takipçi Listesi",
            description: `${uye.user.username} üyesinin takipçi listesini görüntüler.`,
            emoji: { id: "943286130357444608" },
            value: "takipçiListesi",
          },
          {
            label: "Takip Edilen Listesi",
            description: `${uye.user.username} üyesinin takip edilen listesini görüntüler.`,
            emoji: { id: "951514358515638362" },
            value: "takipEdilenListesi",
          },
          {
            label: "Beğeni Listesi",
            description: `${uye.user.username} üyesinin beğenilme listesini görüntüler.`,
            emoji: { id: "629785162649174016" },
            value: "beğeniListesi",
          },
          {
            label: "Profil Fotoğrafı",
            description: `${uye.user.username} üyesinin profil resmini büyütür.`,
            emoji: { id: "926954863647150140" },
            value: "pp",
          },
          {
            label: "Profil Kapağı",
            description: `${uye.user.username} üyesinin profil arkaplanını büyütür.`,
            emoji: { id: "926954863647150140" },
            value: "banner",
          },
        ),
    ...
    if (
      uye &&
      uye.presence &&
      uye.presence.activities &&
      uye.presence.activities.some(
        (x) => x.name == "Spotify" && x.type == "LISTENING",
      )
    ) {
      ...
      const spotify = await new Spotify()
        ...
        .setImage(
          `https://i.scdn.co/image/${presence.assets.largeImage.slice(8)}`,
        )
    ...
    let x = await message.reply({
      embeds: [
        new genEmbed().setDescription(
          `${uye} isimli üyenin detaylı bilgileri yükleniyor...`,
        ),
      ],
    });
    x.edit(obj).then((x) => {
      ...
      collector.on("collect", async (i) => {
        if (i.values[0] == "beğeniListesi") {
          ...
          Users.findOne({ _id: uye.id }, async (err, res) => {
                  ...
                  .reply({
                    ...
                    embeds: [
                      new genEmbed()
                        ...
                        .setDescription(
                          `${uye} isimli üyenin hiç beğenisi bulunamadı.`,
                        ),
                    ],
                  })
                  .then((x) =>
            ...
            const curPage = await i
              .reply({
                embeds: [
                  embed.setDescription(
                    `${uye} üyesinin beğenileri yükleniyor. Lütfen bekleyin...`,
                  ),
                ],
                components: [row],
                fetchReply: true,
            ...
            await curPage
              .edit({
                embeds: [
                  embed.setDescription(`Aşağıda ${uye} isimli üyenin beğeni listesi listelenmektedir.
...
${pages[currentPage - 1].map((x, index) => `\` ${index + 1} \` ${message.guild.members.cache.get(x)} (${message.guild.members.cache.get(x).user.username})`).join("\n")}`),
            ...
            collector.on("collect", async (i) => {
              ...
              await curPage
                .edit({
                  embeds: [
                    embed.setDescription(`Aşağıda ${uye} isimli üyenin arkadaş listesi listelenmektedir.
...
${pages[currentPage - 1].map((x, index) => `\` ${index + 1} \` ${message.guild.members.cache.get(x)} (${message.guild.members.cache.get(x).user.username})`).join("\n")}`),
              ...
              if (curPage)
                curPage
                  .edit({
                    embeds: [
                      embed.setDescription(
                        `${uye} isimli üyesinin toplamda \`${res.Likes.length || 0}\` beğenisi mevcut.`,
                      ),
                    ],
                    components: [],
                  })
        ...
        if (i.values[0] == "arkadaşListesi") {
          ...
          Users.findOne({ _id: uye.id }, async (err, res) => {
                  ...
                  .reply({
                    ...
                    embeds: [
                      new genEmbed()
                        ...
                        .setDescription(
                          `${uye} isimli üyenin hiç arkadaşı bulunamadı.`,
                        ),
                    ],
                  })
                  .then((x) =>
            ...
            let pages = res.Friends.filter((x) =>
              message.guild.members.cache.get(x),
            ).chunk(10);
            var currentPage = 1;
            if ((!pages && !pages.length) || !pages[currentPage - 1])
            ...
            const curPage = await i
              .reply({
                embeds: [
                  embed.setDescription(
                    `${uye} üyesinin arkadaş listesi yükleniyor. Lütfen bekleyin...`,
                  ),
                ],
                components: [row],
                fetchReply: true,
...
${pages[currentPage - 1].map((x, index) => `\` ${index + 1} \` ${message.guild.members.cache.get(x)} (${message.guild.members.cache.get(x).user.username})`).join("\n")}`),
              ...
              if (curPage)
                curPage
                  .edit({
                    embeds: [
                      embed.setDescription(
                        `${uye} isimli üyesinin toplamda \`${res.Friends.length || 0}\` arkadaşı mevcut.`,
                      ),
                    ],
                    components: [],
                  })
        ...
        if (i.values[0] == "takipEdilenListesi") {
          ...
          Users.findOne({ _id: uye.id }, async (err, res) => {
                  ...
                  .reply({
                    ...
                    embeds: [
                      new genEmbed()
                        ...
                        .setDescription(
                          `${uye} isimli üyenin takip edilen bilgisi bulunamadı.`,
                        ),
                    ],
                  })
                  .then((x) =>
            ...
            let pages = res.FollowUp.filter((x) =>
              message.guild.members.cache.get(x),
            ).chunk(10);
            var currentPage = 1;
            if ((!pages && !pages.length) || !pages[currentPage - 1])
            ...
            const curPage = await i
              .reply({
                embeds: [
                  embed.setDescription(
                    `${uye} üyesinin takip edilen bilgisi yükleniyor. Lütfen bekleyin...`,
                  ),
                ],
                components: [row],
                fetchReply: true,
            ...
            await curPage
              .edit({
                embeds: [
                  embed.setDescription(`Aşağıda ${uye} isimli üyenin takip edilen listesi listelenmektedir.
...
${pages[currentPage - 1].map((x, index) => `\` ${index + 1} \` ${message.guild.members.cache.get(x)} (${message.guild.members.cache.get(x).user.username})`).join("\n")}`),
              ...
              if (curPage)
                curPage
                  .edit({
                    embeds: [
                      embed.setDescription(
                        `${uye} isimli üyesinin toplamda \`${res.FollowUp.length || 0}\` adet takip ettiği mevcut.`,
                      ),
                    ],
                    components: [],
                  })
        ...
        if (i.values[0] == "takipçiListesi") {
          ...
          Users.findOne({ _id: uye.id }, async (err, res) => {
                  ...
                  .reply({
                    ...
                    embeds: [
                      new genEmbed()
                        ...
                        .setDescription(
                          `${uye} isimli üyenin takipçi bilgisi bulunamadı.`,
                        ),
                    ],
                  })
                  .then((x) =>
            ...
            let pages = res.Follower.filter((x) =>
              message.guild.members.cache.get(x),
            ).chunk(10);
            var currentPage = 1;
            if ((!pages && !pages.length) || !pages[currentPage - 1])
            ...
            const curPage = await i
              .reply({
                embeds: [
                  embed.setDescription(
                    `${uye} üyesinin takipçi bilgisi yükleniyor. Lütfen bekleyin...`,
                  ),
                ],
                components: [row],
                fetchReply: true,
            ...
            await curPage
              .edit({
                embeds: [
                  embed.setDescription(`Aşağıda ${uye} isimli üyenin takipçi listesi listelenmektedir.
              ...
              if (curPage)
                curPage
                  .edit({
                    embeds: [
                      embed.setDescription(
                        `${uye} isimli üyesinin toplamda \`${res.Follower.length || 0}\` adet takipçisi mevcut.`,
                      ),
                    ],
                    components: [],
                  })
