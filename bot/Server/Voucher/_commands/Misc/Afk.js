...
module.exports = {
  ...
  Kullanim: "afk <Sebep>",
  Aciklama:
    "Klavyeden uzak iseniz gitmeden önce bu komutu girdiğinizde sizi etiketleyenlere sizin klavye başında olmadığınızı açıklar.",
  Kategori: "diğer",
  Extend: true,
  ...
  onLoad: function (client) {
    client.on("messageCreate", async (message) => {
              ...
              embeds: [
                new EmbedBuilder()
                  ...
                  .setAuthor({
                    ...
                    iconURL: victim.displayAvatarURL({ extension: "png" }),
                  })
                  .setDescription(
                    `${victim} kullanıcısı \`${victimData.sebep ? `${victimData.sebep}\` sebebiyle ` : ""} ${tarih} AFK moduna geçti!`,
                  ),
              ],
            })
            .then((x) => {
      ...
      await Afks.findByIdAndDelete(message.member.id);
      message
        .reply(`Merhaba **${message.author.username}** Tekrardan Hoş Geldin.`)
        .then((x) => {
          setTimeout(() => {
            x.delete();
          }, 7500);
  ...
  onRequest: async function (client, message, args) {
    ...
    if (GetAfk)
      return message
        .reply(
          `${message.guild.emojiGöster(emojiler.Iptal)} AFK durumdayken tekrardan AFK olamazsın ${message.member}!`,
        )
        .then((x) => {
          setTimeout(() => {
            x.delete();
    ...
    await Afks.updateOne(
      { _id: message.member.id },
      { $set: { sure: new Date(), sebep: sebep } },
      { upsert: true },
    );
    message.react(
