    ...
    Users.findOne({ _id: uye.id }, async (err, res) => {
          ...
          .send({
            embeds: [
              new genEmbed()
                ...
                .setDescription(
                  `${uye} isimli üyenin geçmiş ses bilgileri bulunamadı.`,
                ),
            ],
          })
          .then((x) =>
        ...
        .send({
          embeds: [
            embed.setDescription(
              `${uye}, üyesinin rol bilgisi yükleniyor... Lütfen bekleyin...`,
            ),
          ],
          components: [row],
          fetchReply: true,
      ...
      await curPage
        .edit({
          embeds: [
            embed.setDescription(
              `${pages[currentPage - 1].map((x, index) => `\` ${index + 1} \` ${message.guild.channels.cache.get(x.channel) ? message.guild.channels.cache.get(x.channel) : "#silinmiş-kanal"} <t:${String(x.date).slice(0, 10)}:R> [**${x.state}**] ${x.entry ? "(<@" + x.entry + ">)" : ""}`).join("\n")}`,
            ),
          ],
        })
        .catch((err) => {});
            ...
            .edit({
              embeds: [
                embed
                  ...
                  .setDescription(
                    `${uye} isimli üyesinin toplamda \`${res.Voices.length || 0}\` adet ses bilgisi mevcut.`,
                  ),
              ],
              components: [],
            })
