...
module.exports = {
  ...
  onLoad: function (client) {
    client.on("interactionCreate", async (i) => {
      ...
      if (i.customId === "takachi_yardim") {
        if (i.values == "talent") {
          i.reply({
            embeds: [
              new genEmbed().setDescription(
                `${Data ? (Data.talentPerms ? Data.talentPerms.map((x) => `\`${sistem.botSettings.Prefixs[0] + x.Commands + " <@takachi/ID>"}\``).join("\n") : "") : ""}`,
              ),
            ],
            ephemeral: true,
          });
                ...
                `${client.commands
                  .filter(
                    (x) =>
                      x.Extend != false &&
                      x.Kategori == `${i.values}` &&
                      (ayarlar.dugmeliKayit ? x.Isim != "erkek" : true),
                  )
                  .map(
                    (x) =>
                      `\`${sistem.botSettings.Prefixs[0] + (ayarlar.dugmeliKayit ? x.Kullanim.replace("kadın", "kayıt") : x.Kullanim)}\``,
                  )
                  .join("\n")}`,
              ),
            ],
  ...
  onRequest: async function (client, message, args) {
    ...
    const takachi = await message.reply({
      components: [Row],
      embeds: [
        new genEmbed().setDescription(
          `:tada: Aşağıdaki kategorilerden komut yardımı almak istediğiniz kategoriyi seçin!`,
        ),
      ],
    });
    var filter = (i) => i.user.id == message.member.id;
    let collector = takachi.createMessageComponentCollector({
      filter: filter,
      time: 60000,
      error: ["time"],
    });
