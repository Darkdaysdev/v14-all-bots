...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let kanal =
      ...
      message.guild.kanalBul(args[0]);
    if (!kanal)
      return message
        .reply(cevaplar.argümandoldur)
        .then((s) => setTimeout(() => s.delete().catch((err) => {}), 5000));
    let _data = await Users.find({
      "Voices.channel": kanal.id,
      "Voices.state": "KATILDI",
    });
    ...
    _data = joined
      .filter((x) => message.guild.members.cache.get(x.id))
      .slice(0, 15)
    ...
    let load = await message.reply({
      content: `${kanal} isimli kanalın giriş çıkışları kontrol ediliyor...`,
    });
    if (joined.length <= 0)
      return load.edit(
        `${kanal} isimli kanalın giriş çıkışları kontrol edildi fakat daha önce bu kanala giriş yapan bulunamadı. ${cevaplar.prefix}`,
      );
    await load.edit({
      content: null,
      embeds: [
        new genEmbed()
          .setDescription(`Aşağıda ${kanal} kanalına son giriş yapan 15 üye listelenmektedir. Toplam da bu kanala giriş yapan ${joined.length} üye bulundu.
