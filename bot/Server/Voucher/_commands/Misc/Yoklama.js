...
module.exports = {
  ...
  Kullanim: "toplantı <[Bireysel: toplantı <@takachi/ID>]>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if (!toplantiKanal)
      return message
        .reply(
          `Toplantı sistemi için herhangi bir ses kanalında bulunmalısınız. ${cevaplar.prefix}`,
        )
        .then((x) =>
          setTimeout(() => {
            x.delete().catch((err) => {});
    ...
    if (
      ...
      let buttons = [
        new ButtonBuilder()
          .setLabel(
            `Bireysel ${_getBireysel ? "Katıldı Olarak Bitir!" : "Başlat!"}`,
          )
      ...
      let bmsg = await message.reply({
        content: `Bireysel toplantı ${uye.user.username} için yükleniyor...`,
      });
      ...
      bmsg.edit({
        ...
        embeds: [
          new genEmbed().setDescription(
            `Belirtilen ${uye} isimli üye için bireysel toplantı yönetimi aşağıda bulunan düğmeler ile yönetilebilir.`,
          ),
        ],
        components: [_row],
      });
      ...
      let _collector = bmsg.createMessageComponentCollector({
        filter: filter,
        time: 30000,
      });
      ...
      _collector.on("collect", async (i) => {
        if (i.customId == "b_katilmadi") {
          ...
          bmsg.edit({
            ...
            embeds: [
              new genEmbed().setDescription(
                `${uye} isimli üyenin bireysel toplantısı katılmadı olarak kaydedildi.`,
              ),
            ],
          });
          i.reply({
            content: `Başarıyla ${uye} isimli üyenin toplantısı katılmadı olarak kaydedildi.`,
            ephemeral: true,
          });
          setTimeout(() => {
        ...
        if (i.customId == "b_izinli") {
          ...
          bmsg.edit({
            ...
            embeds: [
              new genEmbed().setDescription(
                `${uye} isimli üyenin bireysel toplantısı mazeretli olarak kaydedildi.`,
              ),
            ],
          });
          i.reply({
            content: `Başarıyla ${uye} isimli üyenin toplantısı mazeretli olarak kaydedildi.`,
            ephemeral: true,
          });
          setTimeout(() => {
            bmsg.delete().catch((err) => {});
        ...
        if (i.customId == "b_start") {
          ...
          if (_get && _get.state == "START")
            return i.reply({
              content: `Belirtilen ${uye} isimli üyenin aktif bireysel toplantısı bulunmaktadır.`,
              ephemeral: true,
            });
          ...
          if (uye.voice && !uye.voice.channel) {
            uye
              .send({
                content: `Merhaba! **${uye.user.username}**
Şuan da ${toplantiKanal} kanalında bireysel toplantınız başladı. 
Lütfen sese katılın aksi taktirde sizin yetkinize yaptırım olacaktır.`,
              })
              .catch((err) => {
                i.reply({
                  ephemeral: true,
                  content: `Başarıyla ${uye} isimli üyenin bireysel toplantısı başladı fakat **DM** kutusu kapalı olduğundan bildiremedim. Benim yerime sen bildirir misin?`,
                  ephemeral: true,
                });
              });
            i.reply({
              content: `Başarıyla ${uye} isimli üye ile bireysel toplantıyı başlattın.`,
              ephemeral: true,
            });
          } else {
            uye
...
Şuan da ${toplantiKanal} kanalında bireysel toplantınız başladı.`,
              ...
              .catch((err) => {});
            i.reply({
              content: `Başarıyla ${uye} isimli üyenin bireysel toplantısı başlattın. Belirtilen üye ${uye.voice.channel} kanalında seste bulunuyor.`,
              ephemeral: true,
            });
          }
        }
        if (i.customId == "b_end") {
          ...
          if (!_get)
            return i.reply({
              content: `Belirtilen ${uye} isimli üyenin aktif bireysel toplantısı bulunmamaktadır.`,
              ephemeral: true,
            });
          await Users.updateOne(
            { _id: uye.id },
          ...
          if (staffCheck && staffCheck.Staff && Upstaff)
            await client.Upstaffs.addPoint(uye.id, 50, "Toplantı");
          bmsg.edit({
            content: `${uye.user.username} Kişisinin bireysel toplantısı kapatılıyor.`,
            embeds: [],
            components: [],
          });
          setTimeout(() => {
            client._cached.delete(uye.id);
            bmsg.edit({
              content: `${uye.user.username} Kişisinin bireysel toplantısı kapatıldı.`,
              embeds: [
...
Toplantı verileri yetkisine yansıyacak şekilde kaydedildi ve ${message.member} tarafından sonlandırıldı.`),
    ...
    if (toplantiKanal && toplantiKanal.members.size < 1)
      return message
        .reply(
          `Toplantı sistemi için ses kanalınızda en az bir kişi olmalı. ${cevaplar.prefix}`,
        )
        .then((x) =>
          setTimeout(() => {
            x.delete().catch((err) => {});
    ...
    if (
      MeetingData &&
      MeetingData.authorId &&
      MeetingData.channelId &&
      Data.Ayarlar.Toplantı &&
      message.guild.channels.cache.get(MeetingData.channelId) &&
      message.member.voice.channel.id != MeetingData.channelId
    )
      return message
        .reply(
          `Şuan da aktif bir toplantı var. ${message.guild.channels.cache.get(MeetingData.channelId) ? message.guild.channels.cache.get(MeetingData.channelId) : "#silinen-kanal"} kanalına girerek toplantıyı yönetebilirsin. ${cevaplar.prefix}`,
        )
        .then((x) =>
          setTimeout(() => {
            x.delete().catch((err) => {});
    ...
    if (
      ...
      MeetingData.channelId &&
      MeetingData.Date &&
      MeetingData.endAuthorId &&
      MeetingData.endDate &&
      MeetingData.Joining &&
      MeetingData.Leaving
    ) {
      embed.addFields({
        name: `Son Toplantı Bilgisi`,
        value: `
...
> Toplantı **\`${meetingTime(MeetingData.endDate - MeetingData.Date)}\`** sürmüş.`,
    ...
    message
      .reply({
        components: [Row],
        embeds: [
          embed.setDescription(`**Merhaba** ${message.member.user.username}
          ...
          if (i.customId == "toplantıAç") {
            ...
            if (meetingStatus) {
              await GUILDS_SETTINGS.updateOne(
                { _id: 1 },
                { $set: { "Ayarlar.Toplantı": false } },
                { upsert: true },
              );
              await MEETING_INFO.updateOne(
              ...
              let KatilimData = (await MEETING_INFO.findOne({
                guildID: message.guild.id,
              })) || { Joining: [], Leaving: [] };
              ...
              await message.channel.send({
                embeds: [
                  new genEmbed()
                    .setDescription(`Başarıyla ${message.member.voice.channel} kanalında bulunan toplandı bitirildi.
...
> Bu toplantı  **\`${meetingTime(KatilimData.endDate - KatilimData.Date)}\`** sürmüş.`),
              ...
              if (KatilimData && KatilimData.Joining.length > 0) {
                KatilimData.Joining.forEach(async (id) => {
                  ...
                  let Upstaff = await Upstaffs.findOne({ _id: uye.id });
                  if (staffCheck && staffCheck.Staff && Upstaff)
                    await client.Upstaffs.addPoint(uye.id, 75, "Toplantı");
                });
              }
              if (KatilimData && KatilimData.Leaving.length > 0) {
                KatilimData.Leaving.forEach(async (id) => {
                  let uye = message.guild.members.cache.get(id);
                  if (
                    roller.mazeretliRolü &&
                    uye.roles.cache.has(roller.mazeretliRolü)
                  ) {
                    await Users.updateOne(
                      { _id: id },
                      {
              ...
              if (katildiRolü)
                katildiRolü.members.array().map((x) => {
                  x.roles.remove(katildiRolü.id).catch((err) => {});
                });
              await message.channel.send({
                embeds: [
                  new genEmbed().setDescription(
                    `Başarıyla ${message.member.voice.channel} kanalında toplantı başladı.`,
                  ),
                ],
              });
              await i.deferUpdate().catch((err) => {});
          ...
          if (i.customId == "Yoklama") {
            ...
            let enAltYetkiliRolü = await message.guild.roles.cache.get(
              roller.altilkyetki,
            );
            await msg.edit({
              embeds: [
                new genEmbed().setDescription(
                  `Şuan da ${message.member.voice.channel} toplantı kanalında, yoklama alınmaya başlandı.`,
                ),
              ],
            });
            let uyeler = message.guild.members.cache.array();
            let filteruye = uyeler.filter(
              (uye) =>
                !uye.user.bot &&
                uye.roles.highest.position >= enAltYetkiliRolü.position,
            );
            let sestekiYetkililer = filteruye.filter(
              (uye) =>
                uye.voice.channel &&
                uye.voice.channel.id == KatilimData.channelId,
            );
            let sesteOlmayanYetkililer = filteruye.filter(
              (uye) => !uye.voice.channel,
            );
            ...
            await msg.edit({
              embeds: [
                new genEmbed().setDescription(
                  `Başarıyla ${message.member.voice.channel} toplantı kanalında, yoklama alındı. ${message.guild.emojiGöster(emojiler.Onay)}`,
                ),
              ],
            });
            await i.deferUpdate().catch((err) => {});
