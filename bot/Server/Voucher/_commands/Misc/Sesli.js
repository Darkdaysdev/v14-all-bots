...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    let tagsız = message.guild.members.cache.filter((x) => {
      return !x.roles.cache.has(roller.tagRolü) && x.voice.channel;
    }).size;
    let yetkili = message.guild.members.cache.filter((x) => {
      return (
        x.roles.cache.has(roller.tagRolü) &&
        x.voice.channel &&
        roller.Yetkiler.some((yetkili) => x.roles.cache.has(yetkili))
      );
    }).size;
    let kontrol;
    if (args[0] == "tüm") {
...
${message.guild.emojiGöster(emojiler.Tag)} Ses kanallarında taglı **${global.sayılıEmoji(taglı)}** üye bulunmakta.\n`
    ...
    message.channel
      .send(
        `${message.guild.emojiGöster(emojiler.Tag)} Ses kanallarında **${global.sayılıEmoji(ses)}** üye bulunmakta.${kontrol}`,
      )
      .then((x) => {
        setTimeout(() => {
          x.delete();
