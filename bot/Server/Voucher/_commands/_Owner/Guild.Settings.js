...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if (!process.some((x) => args[0] == x))
      return message.channel.send(
        cevaplar.argümandoldur +
          ` \`${sistem.botSettings.Prefixs[0]}${module.exports.Isim} <${process.map((x) => x).join("/")}> <Yeni İçerik>\``,
      );

    if (args[0] == "isim") {
      if (!args[1])
        return message.channel
          .send(`${cevaplar.prefix} Lütfen bir sunucu ismi belirleyin.`)
          .then((x) =>
            setTimeout(() => {
              x.delete();
            }, 7500),
    ...
    if (args[0] == "afiş") {
      ...
      return setBanner(
        ...
        message.member,
        message.channel,
        args.slice(1).join(" ") || message.attachments.first().url,
        message.guild.bannerURL({ extension: "png" }),
        embed,
      );
    }
    if (args[0] == "resim") {
...
async function setIcon(guild, member, channel, newIcon, oldIcon, embed) {
  embed
    .setDescription(
      `${member} (\`${member.id}\`) isimli yönetici sunucu resmini değiştirdi.`,
    )
...
async function setBanner(guild, member, channel, newBanner, oldBanner, embed) {
  embed
    .setDescription(
      `${member} (\`${member.id}\`)  isimli yönetici sunucu afişini değiştirdi.`,
    )
...
async function setName(guild, member, channel, newName, oldName, embed) {
  embed.setDescription(
    `${member} (\`${member.id}\`) isimli yönetici sunucu ismini \`${oldName}\` => \`${newName}\` olarak değiştirdi.`,
  );
