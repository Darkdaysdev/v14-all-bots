...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let load = await message.reply({
      content: `${uye.user.username} üyesinin çıkmadan önceki yetki rolleri listeleniyor. Lütfen bekleyin...`,
    });
    ...
    load.edit({
      ...
      embeds: [
        new genEmbed()
          .setDescription(`Aşağı da ${uye} isimli üyenin sunucudan atılmadan/çıkmadan önceki rol veya rolleri listelenmektedir. 

**Rol(ler) şunlardır**:
${data._roles
  .filter((x) => message.guild.roles.cache.get(x))
  ...
  .join("\n")}`),
    ...
    collector.on("collect", async (i) => {
      if (i.customId == "tanımla") {
        i.reply({
          content: `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye} isimli üyenin rollerine eski rolleri tanımlandı ve verisi temizlendi.`,
          ephemeral: true,
        });
        let rol = [];
        data._roles
