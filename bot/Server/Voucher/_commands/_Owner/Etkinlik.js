...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let etkinlikDurumu = ayarlar
      ? ayarlar.Etkinlik
        ? ayarlar.Etkinlik
        : false
      : false;
    let etkinlikKanalları = ayarlar
      ? ayarlar.etkinlikIzinliler
        ? ayarlar.etkinlikIzinliler
        : []
      : [];
    let etkinlikSaniyelikPuan = ayarlar
      ? ayarlar.etkinlikPuan
        ? ayarlar.etkinlikPuan
        : 0.001
      : 0.001;
    message.channel
      .send({
        embeds: [
          new genEmbed().setFooter({
            text: "Etkinlik puanlamasını websitesi üzerinden veya da kurulum panelinden değiştirebilirsiniz.",
          })
...
\` ❯ \` Etkinlik Kategorileri: \`${etkinlikKanalları.length >= 1 ? etkinlikKanalları.map((x) => message.guild.channels.cache.get(x).name).join(", ") : " Ayarlanmamış! "}\``),
        ],
        ...
        collector.on("collect", async (i) => {
          if (i.customId == "aktif") {
            ...
            await GUILDS_SETTINGS.updateOne(
              ...
              { $set: { "Ayarlar.Etkinlik": true } },
              { upsert: true },
            );
            msg.delete().catch((err) => {});
            i.reply({
              content: `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla <t:${String(Date.now()).slice(0, 10)}:R> ${etkinlikKanalları.length >= 1 ? etkinlikKanalları.map((x) => message.guild.channels.cache.get(x).name).join(", ") + " kategorilerin de" : ``} etkinlik **${etkinlikSaniyelikPuan}** saniyelik puan ile başlatıldı.`,
              ephemeral: true,
            });
            ...
            await GUILDS_SETTINGS.updateOne(
              { guildID: sistem.SERVER.ID },
              { $set: { "Ayarlar.Etkinlik": false } },
              { upsert: true },
            );
            msg.delete().catch((err) => {});
            i.reply({
              content: `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla <t:${String(Date.now()).slice(0, 10)}:R>${etkinlikKanalları.length >= 1 ? etkinlikKanalları.map((x) => message.guild.channels.cache.get(x).name).join(", ") + " kategorilerinde ki" : ``} etkinlik sonlandırıldı.`,
              ephemeral: true,
            });
          }
        });
