  ...
  onRequest: async function (client, message, args) {
    ...
    if (!ayarlar.taglıalım)
      return message.channel.send(
        `${cevaplar.prefix} \`Taglı-Alım\` modu kapalı olduğundan dolayı işlem iptal edildi.`,
      );
      ...
      .send({
        embeds: [
          embed.setDescription(
            `Sunucuda kayıtlı olup tagı olmayan \`${tagsızlar.size}\` üye başarıyla kayıtsız'a atıldı!`,
          ),
        ],
      })
      .then((x) => {
