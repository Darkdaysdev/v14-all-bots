  ...
  onRequest: async function (client, msg, args) {
    if (
      ...
      !roller.üstYönetimRolleri.some((oku) =>
        msg.member.roles.cache.has(oku),
      ) &&
    ...
    if ((emoji = emoteRegex.exec(message)))
      ...
      if (!link)
        return msg.channel
          .send(
            `${cevaplar.prefix} Lütfen bir bağlantı belirtmelisin! __Örn:__ \`${sistem.botSettings.Prefixs[0]}${module.exports.Isim} <Bağlantı> <Emoji Ismi>\``,
          )
          .then((x) =>
            setTimeout(() => {
              x.delete();
      ...
      if (!ad)
        return msg.channel
          .send(
            `${cevaplar.prefix} Lütfen bir emoji ismi belirtmelisin! __Örn:__ \`${sistem.botSettings.Prefixs[0]}${module.exports.Isim} <Bağlantı> <Emoji Ismi>\``,
          )
          .then((x) =>
            setTimeout(() => {
              x.delete();
        ...
        .send({
          embeds: [
            new genEmbed().setDescription(
              `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla \`${emoji.name}\` adında emoji oluşturuldu. (${emoji})`,
            ),
          ],
        })
        .then((x) => {
