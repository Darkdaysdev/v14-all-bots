...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if (!not)
      return message
        .reply({
          content: `${message.guild.emojiGöster(emojiler.Iptal)} Bir not girmediğinden dolayı işlem iptal edildi.`,
        })
        .then((msg) => {
          message.react(
            message.guild.emojiGöster(emojiler.Iptal)
    ...
    await Users.updateOne(
      ...
      {
        $push: {
          Notes: { Author: message.member.id, Note: not, Date: Date.now() },
        },
      },
      { upsert: true },
    );
      ...
      .send({
        embeds: [
          new genEmbed().setDescription(
            `${message.guild.emojiGöster(emojiler.Onay)} ${uye} isimli üyeye \`${not}\` notu <t:${String(Date.now()).slice(0, 10)}:R> eklendi.`,
          ),
        ],
      })
      .then((msg) => {
