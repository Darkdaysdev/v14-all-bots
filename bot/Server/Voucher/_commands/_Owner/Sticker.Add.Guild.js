...
module.exports = {
  ...
  Kullanim:
    "stickeroluştur <[Sticker URL/Ek]> <[Sticker Adı]> <[Sticker Tagı]> <[Sticker Açıklaması]>",
  Aciklama: "",
  Kategori: "kurucu",
  Extend: true,
...
function stickerOluştur(link, ad, tag, açıklama, message) {
  message.guild.stickers
    .create(link, ad, tag, { description: açıklama })
    .then((sticker) =>
      message
        .reply({
          embeds: [
            new genEmbed().setDescription(
              `Başarıyla **${ad}** isimli çıkartma \`${tag}\` tagı ile oluşturuldu. ${message.guild.emojiGöster(emojiler.Onay)}`,
            ),
          ],
        })
        .then((x) => {
