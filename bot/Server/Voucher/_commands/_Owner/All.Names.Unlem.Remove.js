...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let ünlemliler = message.guild.members.cache.filter((x) =>
      x.displayName.includes("!"),
    );
    ünlemliler.forEach(async (uye) => {
      await uye
        .setNickname(uye.displayName.replace("!", ""))
        .catch((err) => {});
      if (uye.displayName.includes("."))
        await uye
          .setNickname(uye.displayName.replace(".", ""))
          .catch((err) => {});
    });
      ...
      .send({
        embeds: [
          new genEmbed().setDescription(
            `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla \`${ünlemliler.size}\` üyenin isminde ki __ünlem, özel karakter veya boşluk__ kaldırıldı.`,
          ),
        ],
      })
      .then((x) => {
