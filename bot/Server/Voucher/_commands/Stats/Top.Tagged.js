...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let load = await message.reply({
      content: `${message.guild.name} sunucusuna ait taglı çekme sıralaması yükleniyor. Lütfen bekleyin!`,
    });
    let findedIndex = "";
    ...
    let topTagli = data
      ...
      .map((m, index) => {
        ...
        if (m._id == message.author.id && index + 1 > 20)
          findedIndex = `\`${index + 1}.\` <@${m._id}>: \` ${uyeToplam2} Taglı \` **(Siz)**`;
      ...
      .edit({
        ...
        embeds: [
          embed.setDescription(
            `Aşağı da \`${message.guild.name}\` sunucusunun en iyi taglı çekenlerin sıralaması belirtilmiştir.\n\n${topTagli ? `${topTagli}\n${findedIndex}` : `\`${message.guild.name}\` sunucusun da taglı bilgileri bulunamadı.`}`,
          ),
        ],
      })
      .then((x) => {
