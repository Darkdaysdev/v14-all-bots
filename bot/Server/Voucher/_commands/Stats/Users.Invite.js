...
module.exports = {
  ...
  Kullanim: "invite <@takachi/ID>",
  Aciklama:
    "Belirtilen üye veya komutu kullanan üyenin davet bilgilerini görüntüler.",
  Kategori: "stat",
  Extend: true,
  ...
  onRequest: async function (client, message, args) {
    ...
    const weekly = invMember
      ...
      : 0;
    const tagged = invMember
      ? message.guild.members.cache.filter(
          (usr) =>
            invMember.some((x) => x.userID === usr.user.id) &&
            usr.user.globalName.includes(ayarlar.tag),
        ).size
      : 0;
    message.channel.send({
      embeds: [
        new genEmbed()
          .setDescription(`${member} isimli üye toplam **${total + bonus}** (**Bonus**: \` +${bonus} \`) davete sahip. (**${regular}** giren, ${ayarlar.type ? ` **${tagged}** taglı, ` : ``}**${leave}** ayrılmış, **${fake}** sahte, **${daily}** günlük, **${weekly}** haftalık)
