...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    let load = await message.reply({
      content: `${message.guild.name} sunucusuna ait görev puanı sıralaması yükleniyor. Lütfen bekleyin!`,
    });
    ...
    let topTagli = data
      ...
      .map((m, index) => {
        ...
        if (m._id == message.author.id && index + 1 > 20)
          findedIndex = `\`${index + 1}.\` <@${m._id}>: **\`${uyeToplam2} Görev Puanı\`** **(Siz)**`;
      ...
      .edit({
        ...
        embeds: [
          embed.setDescription(
            `Aşağı da **${message.guild.name}** sunucusunun en iyi görev yapanların sıralaması belirtilmiştir.\n\n${topTagli ? `${topTagli}\n${findedIndex}` : `${cevaplar.prefix} ${message.guild.name} Sunucusunda da görev sıralama bilgileri bulunamadı.`}`,
          ),
        ],
      })
      .then((x) => {
