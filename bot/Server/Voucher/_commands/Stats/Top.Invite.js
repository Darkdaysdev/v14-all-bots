  ...
  onRequest: async function (client, message, args) {
    ...
    let topTagli = data
      ...
      .map((m, index) => {
        ...
        uyeToplam2 = m.total;
        if (index + 1 > 20)
          findedIndex = `\`${index + 1}.\` <@${m.userID}> toplam **${m.total}** üye davet etmiş. ${m.userID == message.member.id ? `**(Siz)**` : ``}`;
      ...
      .edit({
        ...
        embeds: [
          embed.setDescription(
            `Aşağı da **${message.guild.name}** sunucusunun en iyi davet yapanların sıralaması belirtilmiştir.\n\n${topTagli ? `${topTagli}\n${findedIndex}` : `${cevaplar.prefix} ${message.guild.name} Sunucusun da davet bilgileri bulunamadı.`}`,
          ),
        ],
      })
      .then((x) => {
