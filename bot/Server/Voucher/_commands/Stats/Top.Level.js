  ...
  onRequest: async function (client, message, args) {
    ...
    Stat.filter(
      (x) =>
        ...
        !message.guild.members.cache.get(x.userID).user.bot,
    ).map((x) =>
      data.push({
        id: x.userID,
        ses: x.voiceLevel,
        mesaj: x.messageLevel,
        xp: Number(x.messageXP + x.voiceXP),
      }),
    );
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let avatar = await loadImage(
        uye.user.avatarURL({ format: "jpg" })
          ? uye.user.avatarURL({ format: "jpg" })
          : "https://cdn.discordapp.com/attachments/990322473750917120/1005299390921064498/aotNervousEren.png",
      );
      image
        .printRoundedImage(back, 0, Number(bstart), 678, 56, 25)
      ...
      if (uye.user.username.toString().length > 12) {
        image.setTextFont("18px Arial Black"),
          image.printText(`${uye.user.username}`, 130, Number(start + 5), 850);
      } else {
        image.setTextFont("20px Arial Black"),
          image.printText(`${uye.user.username}`, 130, Number(start + 2), 850);
      }

      if (i != 0 && i != 1 && i != 2)
        image
          .setTextFont("20px Arial Black")
          .printText(`#${i + 1}`, 20, Number(start + 2), 850);
      image
        .setTextFont("18px Arial Black")
        .printText(`${data[i].ses} Sv.`, 510, Number(start + 5), 850);
      image
        .setTextFont("18px Arial Black")
        .printText(`${data[i].mesaj} Sv.`, 610, Number(start + 5), 850);
      bstart += 57;
      start += 57;
    }

    let attachment = new AttachmentBuilder(image.toBuffer(), {
      name: "takachi-siralama.png",
    });

    let ses = new genEmbed()
      .setDescription(
        `Aşağıda gösterilen tabloda \`${message.guild.name}\` sunucusuna ait en iyi genel ses ve mesaj seviye sıralaması gösterilmektedir. Bu sıralama ses ve mesaj xp'inizin toplamını baz alır.`,
      )
      .setThumbnail(message.guild.iconURL({ extension: "png" }))
      .setImage("attachment://takachi-siralama.png");
