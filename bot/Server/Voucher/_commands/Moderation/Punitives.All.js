...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
        ...
        .reply({
          embeds: [
            new genEmbed().setDescription(
              `${message.guild.name} sunucusunda ceza uygulanmadı.`,
            ),
          ],
        })
        .then((x) =>
    ...
    let embed = new genEmbed().setColor("Random").setFooter({
      ...
      .reply({
        embeds: [
          embed.setDescription(
            `${message.guild.name} sunucusunun son atılan ceza listesi yükleniyor...`,
          ),
        ],
        components: [row],
        fetchReply: true,
    ...
    await curPage
      .edit({
        embeds: [
          embed.setDescription(
            `${pages[currentPage - 1].map((x, index) => `\` ${x.No} \` <@${x.Member}> <t:${String(Date.parse(x.Date)).slice(0, 10)}:R> [**${x.Type}**] (<@${x.Staff}>)`).join("\n")}`,
          ),
        ],
      })
      .catch((err) => {});
          ...
          .edit({
            embeds: [
              embed
                ...
                .setDescription(
                  `${message.guild.name} sunucusunda toplamda \`${res.length || 0}\` adet ceza uygulandı.`,
                ),
            ],
            components: [],
          })
