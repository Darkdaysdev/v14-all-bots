...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    if (
      (!ayarlar && !roller && !roller.jailHammer) ||
    ...
    const sebeps = [
      ...
      {
        label: "Sunucu Düzeni Ve Huzursuzluk Yaratmak",
        description: "4 Gün",
        emoji: { name: "4️⃣" },
        value: "4",
        date: "4d",
      ...
      {
        label: "Kayıt Odalarında Gereksiz Trol Yapmak",
        description: "3 Gün",
        emoji: { name: "5️⃣" },
        value: "5",
        date: "3d",
    ...
    let jailButton = new ButtonBuilder()
      .setCustomId(`onayla`)
      .setLabel(
        (await Jail.findById(uye.id))
          ? `Aktif Cezalandırılması Mevcut!`
          : getLimit.get(message.member.id) >= ayarlar.jailLimit
            ? `Limit Doldu (${getLimit.get(message.member.id) || 0} / ${ayarlar.jailLimit})`
            : "Cezalandırmayı Onaylıyorum!",
      )
      ...
      .reply({
        embeds: [
          new genEmbed()
            ...
            .setDescription(
              `Belirtilen ${uye} isimli üyeyi cezalandırmak istediğinize emin misiniz?`,
            ),
        ],
    ...
    collector.on("collect", async (i) => {
        ...
        i.update({
          embeds: [
            new genEmbed()
              ...
              .setDescription(
                `Belirtilen ${uye} isimli üyesini hangi sebep ile cezalandırmak istiyorsun?\n${!roller.kurucuRolleri.some((x) => message.member.roles.cache.has(x)) && !ayarlar.staff.includes(message.member.id) && !message.member.permissions.has(Discord.PermissionFlagsBits.Adminisrator) ? (Number(ayarlar.jailLimit) ? `Kullanılabilir Limit: \`${getLimit.get(message.member.id) || 0} / ${ayarlar.jailLimit}\`` : ``) : ``}`,
              ),
          ],
          components: [
      ...
      if (i.customId === `sebep`) {
        ...
        if (seçilenSebep) {
          if (getLimit.get(message.member.id) >= ayarlar.jailLimit)
            return i.update({
              content: `${message.guild.emojiGöster(emojiler.Iptal)} ${uye} isimli üyenin cezalandırılma işlemi limit dolumundan dolayı iptal edildi.`,
              components: [],
              embeds: [],
              ephemeral: true,
            });
