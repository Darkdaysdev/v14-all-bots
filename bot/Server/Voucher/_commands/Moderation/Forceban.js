...
module.exports = {
  Isim: "kalkmazban",
  Komut: ["takachiban", "uzaoç", "forceban", "xox", "siktirgitamınoğlu"],
  Kullanim: "kalkmazban <@takachi/ID> <Sebep>",
          ...
          embeds: [
            new genEmbed()
              .setFooter({
                ...
                iconURL: uye.avatarURL({ extension: "png" }),
              })
              .setDescription(
                `${uye.toString()} üyesine, <t:${String(Date.now()).slice(0, 10)}:R> \`${sebep}\` nedeniyle işlem uygulandı.`,
              ),
          ],
        });
      await message.channel.send(
        `${message.guild.emojiGöster(emojiler.Yasaklandı)} ${uye.toString()} isimli üyeye \`${sebep}\` sebebiyle "__Kalkmaz(**BOT**) Yasaklama__" türünde ceza-i işlem uygulandı. (\`Ceza Numarası: #${cezano}\`)`,
      );
