...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if (kalkmazYasaklama) {
      buttons.push([
        new ButtonBuilder()
          ...
          .setLabel(
            toplamBan > 0 ? "📛 Kalkmaz Yasaklama" : "🔓 Kalkmaz Yasaklama",
          )
          .setDisabled(toplamBan > 0 ? false : true)
          .setStyle(
            toplamBan > 0 ? ButtonStyle.Primary : ButtonStyle.Secondary,
          ),
      ]);
    ...
    if (toplamBan > 0)
      await message.guild.bans.fetch().then(async (banned) => {
        banned.forEach(async (user) => {
          let cezaBul = await Punitives.findOne({
            Member: user.user.id,
            Type: "Yasaklama",
          });
    ...
    await message
      .reply({
        content: `:tada: Aşağı da \`${message.guild.name}\` sunucusuna ait kalkmaz yasaklama ve yasaklamalar listelenmektedir.`,
        components: [Row],
      })
      .then(async (msg) => {
        message
        ...
        collector.on("collect", async (i) => {
          if (i.customId == "bans") {
            msg
              .edit({
                content: `:x: Aşağıda \`${message.guild.name}\` sunucusuna ait yasaklı üyeler listelenmektedir (**${toplamBan}**).\n\`\`\`${banList}\`\`\``,
              })
              .catch((err) => {
                const arr = splitMessages(banList, {
                  maxLength: 2000,
                  char: "\n",
                });
          ...
          if (i.customId == "forcebans") {
            msg
              .edit({
                content: `:x: Aşağıda \`${message.guild.name}\` sunucusuna ait kalkmaz yasaklanan üyeler listelenmektedir (**${kalkmazYasaklama.length}**).\n\`\`\`${forceBanList}\`\`\``,
              })
              .catch((err) => {
                const arr = splitMessages(forceBanList, {
                  maxLength: 2000,
                  char: "\n",
                });
                for (const newText of arr) {
