...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    const sebeps = [
      {
        label: "Kışkırtma, Trol, Dalgacı ve Ortam Bozucu Davranış",
      ...
      {
        label: "Dizi, Film ve Hikayeler Hakkında Spoiler Vermek",
      ...
      {
        label: "Küçümseyici Ve Aşalayıcı Davranış",
      ...
      {
        label: "Ailevi Değerlere Küfür/Hakaret",
      ...
      {
        label: `Ortamı (${ayarlar.serverName}) Kötülemek`,
      ...
      {
        label: "Seste Yaşanan Olayları Chat'e Yansıtmak ve Uzatmak",
        description: "10 Dakika",
        emoji: { name: "7️⃣" },
        value: "7",
        date: "10m",
      ...
      {
        label: "Özel Odalara Uyarılmalara Rağmen İzinsiz Giriş",
        description: "30 Dakika",
        emoji: { name: "3️⃣" },
        value: "10",
        date: "30m",
      ...
      {
        label: "Soundpad, Efekt ve Ses Programları Kullanımı",
        description: "10 Dakika",
        emoji: { name: "5️⃣" },
        value: "12",
        date: "10m",
      ...
      .setDisabled(
        roller.muteHammer.some((oku) => message.member.roles.cache.has(oku)) ||
          ...
          ? (await Mute.findById(uye.id))
            ? true
            : getLimitMute.get(message.member.id) >= ayarlar.muteLimit
              ? true
              : false
          : true,
      );
      ...
      .setDisabled(
        roller.voiceMuteHammer.some((oku) =>
          ...
          ? (await voiceMute.findById(uye.id))
            ? true
            : getLimitVoiceMute.get(message.member.id) >= ayarlar.voiceMuteLimit
              ? true
              : false
          : true,
      );
      ...
      .reply({
        embeds: [
          new genEmbed()
            ...
            .setDescription(
              `Belirtilen ${uye} isimli üyeyi hangi türde susturmak istiyorsun?`,
            ),
        ],
        components: [muteOptions],
      })
      .catch((err) => {});
        ...
        i.update({
          embeds: [
            new genEmbed()
              ...
              .setDescription(
                `Belirlenen ${uye} isimli üyesini hangi sebep ile **metin kanallarından** susturmamı istiyorsun?`,
              ),
          ],
          components: [
            new ActionRowBuilder().addComponents(
              new Discord.StringSelectMenuBuilder()
                .setCustomId(`sebep`)
                .setPlaceholder(
                  `${uye.user.username} için ceza sebebi belirtin!`,
                )
                .addOptions(
                  sebeps
                    .filter((x) => x.type === 5)
        ...
        i.update({
          embeds: [
            new genEmbed()
              ...
              .setDescription(
                `Belirlenen ${uye} isimli üyesini hangi sebep ile **ses kanallarından** susturmamı istiyorsun?`,
              ),
          ],
          components: [
            new ActionRowBuilder().addComponents(
          ...
          if (selectMute == 4) {
            if (Number(ayarlar.voiceMuteLimit)) {
              ...
              if (voiceMuteCheck)
                return (
                  await i.reply({
                    content: `Belirtiğin ${uye} üyesinin, aktif bir susturulma cezası mevcut!`,
                    ephemeral: true,
                  }),
              ...
              if (
                ...
                getLimitVoiceMute.set(
                  `${message.member.id}`,
                  Number(getLimitVoiceMute.get(`${message.member.id}`) || 0) +
                    1,
                );
                setTimeout(
                  () => {
                    getLimitVoiceMute.set(
                      `${message.member.id}`,
                      Number(
                        getLimitVoiceMute.get(`${message.member.id}`) || 0,
                      ) - 1,
                    );
                  },
                  1000 * 60 * 5,
          ...
          if (selectMute == 5) {
            ...
            if (Number(ayarlar.muteLimit)) {
              if (
                ...
                getLimitMute.set(
                  `${message.member.id}`,
                  Number(getLimitMute.get(`${message.member.id}`) || 0) + 1,
                );
                setTimeout(
                  () => {
                    getLimitMute.set(
          ...
          return i.update({
            ...
            embeds: [
              new genEmbed().setDescription(
                `${message.guild.emojiGöster(emojiler.Iptal)} İşlem sırasında hata oluştu lütfen bot sahibine başvurun.`,
              ),
            ],
          });
        }
