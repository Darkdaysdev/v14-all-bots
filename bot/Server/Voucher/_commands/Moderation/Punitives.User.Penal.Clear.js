...
module.exports = {
  ...
  Kullanim: "cezalartemizle <@takachi/ID>",
  ...
  onRequest: async function (client, message, args) {
      ...
      return message.reply({
        embeds: [
          new genEmbed().setDescription(
            `${uye} isimli üyenin cezası bulunamadı.`,
          ),
        ],
      });
    if (await Punitives.findOne({ Member: uye.id, Active: true }))
      return message.reply({
        embeds: [
          new genEmbed().setDescription(
            `${uye} isimli üyenin aktif cezası bulunduğundan dolayı işlem iptal edildi.`,
          ),
        ],
      });
    ...
    await message.reply({
      embeds: [
        new genEmbed().setDescription(
          `${message.guild.emojiGöster(emojiler.Onay)} ${uye} üyesinin tüm cezaları başarıyla temizlendi.`,
        ),
      ],
    });
