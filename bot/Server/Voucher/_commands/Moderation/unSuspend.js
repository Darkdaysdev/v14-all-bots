...
module.exports = {
  ...
  Kullanim: "şüpheliçıkart <@takachi/ID> <Sebep>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if (cezakontrol) {
      message.channel
        .send(
          `${cevaplar.prefix} Belirtilen üye sistemsel tarafından cezalandırılmış, şüpheli çıkart komutu ile çıkartman münkün gözükmüyor.`,
        )
        .then((x) => {
          setTimeout(() => {
            x.delete();
      ...
      findChannel.send({
        embeds: [
          new genEmbed().setDescription(
            `${uye} uyesinin şüpheli durumu <t:${String(Date.now()).slice(0, 10)}:R> ${message.member} tarafından kaldırıldı.`,
          ),
        ],
      });
    ...
    await message
      .reply({
        embeds: [
          new genEmbed().setDescription(
            `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye} isimli üye şüpheli hesap konumundan çıkartıldı!`,
          ),
        ],
      })
      .then((x) => {
        ...
        .send({
          embeds: [
            new genEmbed()
              ...
              .setDescription(
                `${uye.user.username}, ${message.author} tarafından <t:${String(Date.now()).slice(0, 10)}:R> şüpheliden çıkartıldın.`,
              ),
          ],
        })
        .catch((x) => {});
