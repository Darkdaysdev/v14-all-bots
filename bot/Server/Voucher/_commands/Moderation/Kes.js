  ...
  onRequest: async function (client, message, args) {
    ...
    if (Beklet.has(message.author.id))
      return message.channel
        .send(
          `${cevaplar.prefix} \`Günlük Limit Aşıldı!\` ikiden fazla bağlantı kesme işlemi uygulandığı için.`,
        )
      ...
      findChannel.send({
        embeds: [
          new genEmbed().setDescription(
            `${uye} üyesi ${message.author} tarafından ${tarihsel(Date.now())} tarihinde ${uye.voice.channel ? uye.voice.channel : "#Kanal Bulunamadı"} belirtilen sesli kanalından atıldı.`,
          ),
        ],
      });
    ...
    uye
      .send({
        embeds: [
          new genEmbed().setDescription(
            `${message.author} tarafından \`${sebep}\` sebebi ile <t:${String(Date.now()).slice(0, 10)}:R> bulunduğun sesten atıldın.`,
          ),
        ],
      })
      .catch((x) => {});
