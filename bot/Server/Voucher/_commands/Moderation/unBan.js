...
module.exports = {
  ...
  Kullanim: "yargıkaldır <@takachi/ID>",
  ...
  onRequest: async function (client, message, args) {
    ...
    await Punitives.findOne({
      ...
      message.guild.bans.fetch().then(async (yasaklar) => {
        if (yasaklar.size == 0)
          return message.channel.send(cevaplar.yasaklamayok);
        let yasakliuye = yasaklar.find((yasakli) => yasakli.user.id == uye.id);
        if (!yasakliuye)
          return message.channel.send(
            `${cevaplar.prefix} \`Belirtilen Üye Yasaklı Değil!\` lütfen geçerli bir yasaklama giriniz.`,
          );
        if (res) {
          if (
          ...
          await findChannel.send({
            embeds: [
              new genEmbed().setDescription(
                `${uye} uyesinin sunucudaki ${res ? `\`#${res.No}\` ceza numaralı yasaklaması` : "yasaklaması"}, <t:${String(Date.now()).slice(0, 10)}:R> ${message.author} tarafından kaldırıldı.`,
              ),
            ],
          });
        await message.reply(
          `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye} üyesinin ${res ? `(\`#${res.No}\`) ceza numaralı` : "sunucudaki"} yasaklaması kaldırıldı!`,
        );
        message.react(
