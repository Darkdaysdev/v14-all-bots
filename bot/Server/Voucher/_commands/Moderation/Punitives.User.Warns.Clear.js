...
module.exports = {
  ...
  Kullanim: "uyarıtemizle <@takachi/ID>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if (await Punitives.findOne({ Member: uye.id, Type: "Uyarılma" })) {
      await message.reply({
        embeds: [
          new genEmbed().setDescription(
            `${message.guild.emojiGöster(emojiler.Onay)} ${uye} üyesinin tüm uyarılmaları başarıyla temizlendi.`,
          ),
        ],
      });
      await Punitives.updateMany(
        { Member: uye.id, Type: "Uyarılma" },
        {
          $set: {
      ...
      return message.reply({
        embeds: [
          new genEmbed().setDescription(
            `${uye} isimli üyenin uyarısı bulunamadığından dolayı işlem iptal edili.`,
          ),
        ],
      });
    }
