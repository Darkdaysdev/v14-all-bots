...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let jailButton = new ButtonBuilder()
      ...
      .setLabel(
        (await Jail.findById(uye.id))
          ...
          : getLimit.get(message.member.id) >= ayarlar.reklamLimit
            ? `Limit Doldu (${getLimit.get(message.member.id) || 0} / ${ayarlar.reklamLimit})`
    ...
    let msg = await message
      .reply({
        content: `Belirtilen ${uye} isimli üyeyi reklam gerekçesiyle kalıcı bir şekilde cezalandırmak istiyor musun?`,
    ...
    collector.on("collect", async (i) => {
      if (i.customId === `onayla`) {
        ...
        return uye.addPunitives(
          3,
          message.member,
          "Sunucu içerisinde reklam yapmak!",
          message,
        );
      }
      if (i.customId === `iptal`) {
        return await i.update({
          content: `${message.guild.emojiGöster(emojiler.Iptal)} ${uye} isimli üyenin cezalandırılma işlemi başarıyla iptal edildi.`,
          components: [],
          embeds: [],
        });
      }
