...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    await Punitives.find({ Member: uye.id }).exec(async (err, res) => {
      if (err)
        return message
          .reply("Hata: `Bazı hatalar oluştu :(`")
          .then((x) => x.delete({ timeout: 5000 }));
      if (!(await Punitives.findOne({ Member: uye.id })))
        return message
          .reply(`${uye} üyesinin ceza-i bilgilerine ulaşılamadı.`)
          .then((x) =>
            setTimeout(() => {
              x.delete();
            }, 7500),
      ...
      datachat = datachat.concat(
        res
          .filter((value) => value.Type === "Metin Susturulma")
          .map((value) => {
            return [
              `\`#${value.No}\``,
              `**[${value.Type}]**`,
      ...
      datases = datases.concat(
        res
          .filter((value) => value.Type === "Ses Susturulma")
          .map((value) => {
            return [
              `\`#${value.No}\``,
              `**[${value.Type}]**`,
      ...
      datauyari = datauyari.concat(
        res
          .filter((value) => value.Type === "Uyarılma")
          .map((value) => {
            return [
              `\`#${value.No}\``,
              `**[${value.Type}]**`,
      ...
      datajail = datajail.concat(
        res
          .filter((value) => value.Type === "Karantina")
          .map((value) => {
            return [
              `\`#${value.No}\``,
              `**[${value.Type}]**`,
      ...
      databan = databan.concat(
        res
          .filter((value) => value.Type === "Yasaklama")
          .map((value) => {
            return [
              `\`#${value.No}\``,
              `**[${value.Type}]**`,
      ...
      let buttonGroup = [
        {
          label: "Chat Susturulmaları",
          description: `Kullanıcının aldığı Chat Susturma Cezaları listelenmektedir.`,
          emoji: { id: "1230914512186773575" },
          value: "chatmute",
        },
        {
          label: "Ses Susturulmaları",
          description:
            "Kullanıcının aldığı Ses Susturma Cezaları listelenmektedir.",
          emoji: { id: "1230914644252561530" },
          value: "sesmute",
        },
        {
          label: "Uyarı Cezaları",
          description: `Kullanıcının aldığı Uyarı Cezaları listelenmektedir.`,
          emoji: { id: "1232038364052455435" },
          value: "uyariceza",
        },
        {
          label: "Karantina Cezaları",
          description:
            "Kullanıcının aldığı Karantina Cezaları listelenmektedir.",
          emoji: { id: "1232275857985503263" },
          value: "jailceza",
        },
        {
          label: "Yasaklama Cezaları",
          description:
            "Kullanıcının aldığı Yasaklama Cezaları listelenmektedir.",
          emoji: { id: "1232275804965044255" },
          value: "banceza",
        },
        {
          label: "Ceza Geçmişi",
      ...
      function progressBar(
        ...
        if (veri > 0)
          progressStart = `${message.guild.emojiGöster(emojiler.Terfi.başlamaBar)}`;
        const progressText =
          `${message.guild.emojiGöster(emojiler.Terfi.doluBar)}`.repeat(
            progress,
          );
        ...
        const bar =
          ...
          progressText +
          emptyProgressText +
          `${emptyProgress == 0 ? `${message.guild.emojiGöster(emojiler.Terfi.doluBitişBar)}` : `${message.guild.emojiGöster(emojiler.Terfi.boşBitişBar)}`}`;
        return bar;
      }
      ...
      let ses = new genEmbed()
        .setDescription(
          `${message.guild.emojiGöster("information")} Kullanıcının öğrenmek istediğiniz sicilin kategorisini aşağıda bulunan menüden seçiniz.
