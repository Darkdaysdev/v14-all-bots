...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    if (!Number(args[0]))
      return message
        .reply(
          `${cevaplar.prefix} Ceza numarası yerine başka birşey girmiş olmalısın! **Lütfen Doğru Ceza Numarası Giriniz.**`,
        )
        .then((x) =>
          setTimeout(() => {
            x.delete().catch((err) => {});
    ...
    if (!res)
      return message
        .reply(
          `${cevaplar.prefix} **Hatalı!** Lütfen Doğru Ceza Numarası Giriniz.`,
        )
        .then((x) =>
          setTimeout(() => {
            x.delete().catch((err) => {});
    ...
    let cezalananbilgi;
    if (cezalanan != `\`Bulunamayan Üye\`` && cezalanan.username)
      cezalananbilgi = `${cezalanan} (\`${cezalanan.id}\`)`;
    if (!cezalananbilgi)
      cezalananbilgi = "<@" + res.Member + ">" + `(\`${res.Member}\`)`;
    // Ceza Veren Üye
    let yetkili = await client.getUser(res.Staff);
    let yetkilibilgi;
    if (yetkili != `\`Bulunamayan Üye\`` && yetkili.username)
      yetkilibilgi = `${yetkili} (\`${yetkili.id}\`)`;
    ...
    if (res.Remover)
      kaldırılmadurumu =
        "• Ceza'yı Kaldıran: " +
        `${(await client.getUser(res.Remover)) ? (message.guild.members.cache.get(res.Remover) ? message.guild.members.cache.get(res.Remover) : `<@${res.Remover}> (\`${res.Remover}\`)`) : `<@${res.Remover}> (\`${res.Remover}\`)`}`;
    message.channel.send({
      embeds: [
        new genEmbed()
          .setDescription(
            `**Ceza Detayı** (\`#${res.No}/${res.Type}\`)
