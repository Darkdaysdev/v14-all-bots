...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    if (
      !roller.warnHammer.some((oku) => message.member.roles.cache.has(oku)) &&
    ...
    if (
      !uye &&
      message.member.roles.highest.position <= uye.roles.highest.position
    )
      return message
    ...
    if (
      !checkRoles.some((x) => uye.roles.cache.has(x)) &&
      ...
      if (
        ...
        if (
          ...
          client.fetchJailLimit.get(message.member.id) >= ayarlar.jailLimit
        )
          return (
        ...
        return uye.addPunitives(
          3,
          message.member,
          "Gereğinden fazla uyarı cezası bulunmak!" + ` (${sebep})`,
          message,
        );
      }
    }
