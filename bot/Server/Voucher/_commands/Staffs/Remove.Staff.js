      ...
      yetkiliLog.send({
        embeds: [
          embed.setDescription(
            `${message.author} isimli yetkili ${uye.toString()} isimli üyenin \`${tarihsel(Date.now())}\` tarihinde yetkisini aldı!`,
          ),
        ],
      });
    message
      .reply({
        embeds: [
          new genEmbed().setDescription(
            `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye.toString()} isimli üyenin yetkisi alındı.`,
          ),
        ],
      })
      .then((x) => {
