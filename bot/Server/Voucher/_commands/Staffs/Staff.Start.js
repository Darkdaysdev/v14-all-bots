...
module.exports = {
  ...
  Kullanim: "yetkibaşlat <@takachi/ID>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if (
      ...
      return (
        ...
        message.reply(cevaplar.yenihesap).then((x) => {
          setTimeout(() => {
            x.delete();
          }, 7500);
        })
    ...
    if (yetkiSalma) {
      if (yetkiSalma.unleashPoint && yetkiSalma.unleashPoint == 1) {
        embed.setFooter({
          text: `${uye.user.username} üyesi daha önce yetki salmış birdaha salarsa yetkili olamayacak.`,
        });
      } else {
        embed.setFooter({
          text: `${uye.user.username} üyesinin yetki salma hakkı ${yetkiSalma.unleashPoint ? yetkiSalma.unleashPoint : 0} adet bulunuyor.`,
        });
      }
      if (
    ...
    if (kontrol && kontrol.Staff)
      return message.reply(
        `${cevaplar.prefix} ${uye} isimli üye zaten yetkili olarak belirlenmiş.`,
      );
    if (
      ...
      message.reply({
        embeds: [
          embed.setDescription(
            `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye.toString()} üyesi ${message.author} tarafından <t:${String(Date.now()).slice(0, 10)}:R> başarıyla yetkili olarak başlatıldı!`,
          ),
        ],
        components: [],
      });
      ...
      if (yetkiliLog)
        yetkiliLog.send({
          embeds: [
            embed.setDescription(
              `${uye} isimli üye <t:${String(Date.now()).slice(0, 10)}:R> ${message.author} tarafından yetkili olarak başlatıldı.`,
            ),
          ],
        });
    ...
    embed.setDescription(
      `${message.member.toString()} isimli yetkili seni **${message.guild.name}** sunucusunda ${roller.başlangıçYetki ? (message.guild.roles.cache.get(roller.başlangıçYetki) ? `${message.guild.roles.cache.get(roller.başlangıçYetki)} yetkisinden yetkili` : "yetkili") : "yetkili"} yapmak istiyor. Kabul ediyor musun?`,
    );
    await message.channel
      .send({ content: uye.toString(), embeds: [embed], components: [Row] })
      .then(async (msg) => {
          ...
          if (i.customId == "OK") {
            ...
            message.channel
              .send({
                ...
                embeds: [
                  embed.setDescription(
                    `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye.toString()} üyesi ${message.author} tarafından <t:${String(Date.now()).slice(0, 10)}:R> başarıyla ${roller.başlangıçYetki ? (message.guild.roles.cache.get(roller.başlangıçYetki) ? `${message.guild.roles.cache.get(roller.başlangıçYetki)} yetkisinde yetkili` : "yetkili") : "yetkili"} olarak başlatıldı!`,
                  ),
                ],
                components: [],
              })
            ...
            uye.roles
              .add(verilcekRol)
              .then((x) => client.Upstaffs.addPoint(uye.id, "1", "Bonus"));
            message.member.Leaders("yetki", _statSystem.points.staff, {
              type: "STAFF",
              user: uye.id,
            });
              ...
              .edit({
                ...
                embeds: [
                  new genEmbed()
                    .setColor("Red")
                    .setDescription(
                      `${message.guild.emojiGöster(emojiler.Iptal)} ${uye.toString()} isimli üye, **${message.guild.name}** sunucusunda yetkili olma teklifini reddetti!`,
                    ),
                ],
                components: [],
              })
