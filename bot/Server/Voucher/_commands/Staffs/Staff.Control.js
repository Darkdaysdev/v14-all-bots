...
module.exports = {
  ...
  Kullanim: "yetki <@takachi/ID>",
  Aciklama:
    "Belirlenen yetkilinin sunucu içerisinde ki bilgileri gösterir ve yükseltir düşürür.",
  Kategori: "stat",
  Extend: true,
  ...
  onRequest: async function (client, message, args) {
    ...
    if (
      ...
      !roller.limitliYükselticiRolleri.some((x) =>
    ...
    let taglıUser = (await userData)
      ? userData.Taggeds
        ? `${userData.Taggeds.length}` || 0
        : 0
      : 0;
    let compUser = await ComponentUser.findOne({ _id: uye.id });
    let taglıGetirme = compUser
      ? compUser.Checks
        ? `${
            compUser.Checks.filter((x) => {
              let uye2 = message.guild.members.cache.get(x.target);
              return (
                uye2 &&
                uye2.user.globalName.includes(ayarlar.tag) &&
                x.type == "TAG"
              );
            }).length
          }` || 0
        : 0
      : 0;
    let taglıGetirmeToplam = compUser
      ? compUser.Checks.filter((x) => x.type == "TAG")
        ? `${compUser.Checks.filter((x) => x.type == "TAG").length}` || 0
        : 0
      : 0;
    let yetkiliGetirme = compUser
      ? compUser.Checks
        ? `${
            compUser.Checks.filter((x) => {
              let uye2 = message.guild.members.cache.get(x.target);
              return (
                uye2 &&
                roller.Yetkiler.some((b) => uye2.roles.cache.has(b)) &&
                x.type == "STAFF"
              );
            }).length
          }` || 0
        : 0
      : 0;
    let yetkiliGetirmeToplam = compUser
      ? compUser.Checks.filter((x) => x.type == "STAFF")
        ? `${compUser.Checks.filter((x) => x.type == "STAFF").length}` || 0
        : 0
      : 0;
    let teyitUser = (await userData)
      ? userData.Records
        ? `${userData.Records.length}` || 0
        : 0
      : 0;
    let yetkiliUser = (await userData)
      ? userData.Staffs
        ? `${userData.Staffs.length}` || 0
        : 0
      : 0;
    let davetUser = (await Invites.findOne({ userID: uye.id })) || {
      regular: 0,
      bonus: 0,
      fake: 0,
      total: 0,
    ...
    if (data) {
      ...
      data.lifeChatStats.forEach((value, key) => {
        if (key == _statSystem.generalChatCategory)
          haftalikChatListe = `\` • \` ${message.guild.channels.cache.has(key) ? (`${ayarlar.serverName} Chat` ? `${ayarlar.serverName} Chat` : message.guild.channels.cache.get(key).name) : "#Silinmiş"}: \`${value} mesaj\``;
      });
    }
    let rolBilgileri = [];
    let rolLer = [];
    ...
    if (
      ...
      rolkitle = _statSystem.staffs.filter(
        (x) => x.No <= Number(ayarlar.yükseltimSınırı),
      );
    ...
    rolkitle.forEach((rol, index) => {
      ...
      if (rolBilgi) {
        rolLer.push(rol.rol);
        rolBilgileri.push([{ label: rolBilgi.name, value: rol.rol }]);
      }
    });
    ...
    let satir2 = new ActionRowBuilder().addComponents(rolMenu);
    ...
    if (!Upstaffs) {
          ...
          return message
            .reply({
              embeds: [
                new genEmbed()
                  .setFooter({
                    text: `${yetkiSalma.unleashPoint} yetki salma hakkı bulunmakta.`,
                  })
                  .setDescription(
                    `${message.guild.emojiGöster(emojiler.Iptal)} ${uye} isimli üyesi birden fazla kez yetki saldığından dolayı işlem yapılamıyor.`,
                  ),
              ],
            })
            .then((x) => {
      ...
      msg = await message.reply({
        content: `Belirlenen **${uye.user.username}** üyesi için işlem yükleniyor. Lütfen bekleyin!`,
      });
      await msg.edit({
        content: `${uye} üyesi yetkili değil, yetki vermek istiyorsanız aşağıda ki menüden seçiniz!`,
        components: [satir2],
      });
    } else {
      let yetkibul =
        _statSystem.staffs[
          _statSystem.staffs.indexOf(
            _statSystem.staffs.find((x) => uye.roles.cache.has(x.rol)),
          )
        ];
      let altyetkisi =
        _statSystem.staffs[_statSystem.staffs.indexOf(yetkibul) - 1];
      let üstyetkisi =
        _statSystem.staffs[_statSystem.staffs.indexOf(yetkibul) + 1];
      let yetkiSalma = await Unleash.findOne({ _id: uye.id });
      if (yetkiSalma) {
      ...
      await msg.edit({
        content: null,
        embeds: [
          embed.setThumbnail(uye.user.avatarURL({ extension: "png" }))
            .setDescription(`Aşağıda ${uye} (\`${uye.id}\`) yetkilinin/yöneticinin analizleri detaylı bir şekilde belirtilmiştir.\n
**Yetkili Bilgileri**:
...
${
  yetkiSalma
    ? `\` ••❯ \` Yetki Salma Hakkı: \`${yetkiSalma.unleashPoint ? yetkiSalma.unleashPoint : 0} Hak\` ${
        yetkiSalma.unleashPoint >= 1
          ? `(${yetkiSalma.unleashPoint == 1 ? `**Tolerans:** \`+1 Hak => %50.0\`` : `\`Hak Doldu!\``})
...
${
  ...
  userData.Meetings.filter(
    (x) => Number(Date.parse(Upstaffs.Baslama)) < Number(x.Date),
  ).length > 0
    ? `Aşağı da son 5 toplantının bilgileri baza alınmıştır. Bu yükseltim/düşüm işlemleri sizin insafiyetinize kalmış.
${userData.Meetings.filter(
  (x) => Number(Date.parse(Upstaffs.Baslama)) < Number(x.Date),
)
    ...
    collector.on("collect", async (button) => {
      if (button.customId === "bilgilendirme") {
        ...
        Users.findOne({ _id: uye.id }, async (err, res) => {
                ...
                .edit({
                  embeds: [
                    new genEmbed()
                      ...
                      .setDescription(
                        `${uye} isimli yetkilinin yükseltim geçmiş bilgisi bulunamadı.`,
                      ),
                  ],
                })
                .then((x) =>
          ...
          const curPage = await msg
            .edit({
              embeds: [
                embed.setDescription(
                  `${uye} yetkilinin yükseltim geçmiş bilgisi yükleniyor. Lütfen bekleyin...`,
                ),
              ],
              components: [row],
              fetchReply: true,
          ...
          await curPage
            .edit({
              embeds: [
                embed.setDescription(
                  `${pages[currentPage - 1].map((x, index) => `\` ${index + 1} \` ${message.guild.roles.cache.get(x.Role) ? message.guild.roles.cache.get(x.Role) : "@Rol Bulunamadı"} <t:${Number(String(x.Date).substring(0, 10))}:R> [**${x.Process}**] (<@${x.Author}>)`).join("\n")}`,
                ),
              ],
            })
            .catch((err) => {});
                ...
                .edit({
                  embeds: [
                    embed
                      ...
                      .setDescription(
                        `${uye} isimli yetkilinin toplamda \`${res.StaffLogs.length || 0}\` adet yükseltim geçmiş bilgisi mevcut.`,
                      ),
                  ],
                  components: [],
                })
          ...
          .send({
            embeds: [
              new genEmbed()
                .setColor("Random")
                .setDescription(
                  `${uye} isimli yetkiliye vermek veya almak istediğiniz yetki puanını belirtiniz.`,
                ),
            ],
          })
          .then(async (mesaj) => {
            var filter = (i) => i.author.id == message.member.id;
            let collector = mesaj.channel.createMessageCollector({
              filter: filter,
              time: 60000,
              max: 1,
            });
            ...
            collector.on("collect", async (m) => {
              ...
              if (isNaN(puan))
                return (
                  ...
                  m.reply({
                    content: `${message.guild.emojiGöster(emojiler.Iptal)} Puanı rakam veya da puan şeklinde belirtmediğinizden veya da puan girmediğinizden dolayı işleminiz iptal edildi.`,
                    ephemeral: true,
                  })
                );
              mesaj.delete().catch((err) => {});
              let Rowcuk = new ActionRowBuilder().addComponents(
                ...
                new ButtonBuilder()
                  ...
                  .setDisabled(
                    Number(Upstaffs.Point) < Number(puan) ? true : false,
                  )
                  .setStyle(ButtonStyle.Secondary),
                new ButtonBuilder()
              ...
              message.channel
                .send({
                  embeds: [
                    new genEmbed()
                      .setColor("Random")
                      .setDescription(
                        `${uye} isimli yetkiliye \` ${puan} Yetki Puanı \` verilecek veya alınacak. Lütfen aşağıda ki düğmelerden seçiniz!`,
                      ),
                  ],
                  components: [Rowcuk],
                })
                .then(async (yarram) => {
                  ...
                  let collector = yarram.createMessageComponentCollector({
                    filter: filter,
                    max: 1,
                  });
                  collector.on("collect", async (i) => {
                    if (i.customId == "verabimeyarrak") {
                      ...
                      await client.Upstaffs.addPoint(
                        uye.id,
                        Number(puan),
                        "Bonus",
                      );
                      message
                        ...
                        .catch((err) => {});
                      i.reply({
                        content: `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye} isimli üyesine \` +${puan} Puan Etkisi \` işlemi uygulandı.`,
                        ephemeral: true,
                      });
                      message.guild.kanalBul("terfi-log").send({
                        embeds: [
                          embed.setDescription(
                            `${message.member} (\`${message.member.id}\`) isimli yönetici ${uye} (\`${uye.id}\`) isimli üyeye \` +${puan} Puan Etkisi \` yetki bonusu eklendi.`,
                          ),
                        ],
                      });
                      ...
                      message.guild.kanalBul("terfi-log").send({
                        embeds: [
                          embed.setDescription(
                            `${message.member} (\`${message.member.id}\`) isimli yönetici ${uye} (\`${uye.id}\`) isimli üyeye \` -${puan} Puan Etkisi \` yetki bonusu kaldırıldı.`,
                          ),
                        ],
                      });
                      ...
                      yarram.delete().catch((err) => {});
                      i.reply({
                        content: `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla puan verme/alma işlemini iptal ettiniz.`,
                        ephemeral: true,
                      });
                      message
                        .react(
      ...
      if (button.customId === "dsr") {
        ...
        let rolBul =
          _statSystem.staffs[_statSystem.staffs.indexOf(yetkiBilgisi) - 1];
        if (!rolBul)
          return button.reply({
            content: `${message.guild.emojiGöster(emojiler.Iptal)} ${uye} isimli üye en alt yetkide daha fazla düşüremezsin.`,
            ephemeral: true,
          });
        if (Upstaffs && !Upstaffs.StaffGiveAdmin)
          await Users.updateOne(
            { _id: uye.id },
            { $set: { StaffGiveAdmin: message.member.id } },
            { upsert: true },
          );
        if (
          roller.altYönetimRolleri.some(
            (x) => rolBul.rol == x || rolBul.exrol.includes(x),
          ) ||
          roller.yönetimRolleri.some(
            (x) => rolBul.rol == x || rolBul.exrol.includes(x),
          ) ||
          roller.üstYönetimRolleri.some(
            (x) => rolBul.rol == x || rolBul.exrol.includes(x),
          )
        ) {
        ...
        uye.roles.cache
          .filter(
            (x) =>
              yetkiBilgisi.rol != x.id && !yetkiBilgisi.exrol.includes(x.id),
          )
          .map((x) => newRoles.push(x.id));
        if (rolBul && rolBul.rol) {
          newRoles.push(rolBul.rol);
          if (rolBul.exrol && rolBul.exrol.length >= 1)
            newRoles.push(...rolBul.exrol);
        }
        ...
        if (logKanalı)
          logKanalı.send({
            embeds: [
              embed.setDescription(
                `${message.member} yöneticisi, ${uye} isimli üyeyi ${message.guild.roles.cache.get(rolBul.rol)} isimli rolüne düşürdü.`,
              ),
            ],
          });
        ...
        button
          .reply({
            content: `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye} isimli yetkili, \`${message.guild.roles.cache.get(rolBul.rol).name}\` rolüne düşürüldü.`,
            ephemeral: true,
          })
          .then((x) => {
            message.react(
      ...
      if (button.customId === "ykslt") {
        ...
        if (!rolBul)
          return button.reply({
            content: `${message.guild.emojiGöster(emojiler.Iptal)} ${uye} isimli üye son yetkiye ulaştığı için yükseltme işlemi yapılamaz.`,
            ephemeral: true,
          });
        if (
          rolBul.No > ayarlar.yükseltimSınırı &&
          !roller.kurucuRolleri.some((oku) =>
            message.member.roles.cache.has(oku),
          ) &&
          !message.member.permissions.has(
          ...
          !roller.yükselticiRoller.some((x) =>
            message.member.roles.cache.has(x),
          ) &&
          roller.limitliYükselticiRolleri.some((x) =>
            message.member.roles.cache.has(x),
          ...
          return button.reply({
            content: `${message.guild.emojiGöster(emojiler.Iptal)} ${uye}, isimli üyeyi en fazla şuan ki yetkiye kadar yükseltebilirsin.`,
            ephemeral: true,
          });
        if (Upstaffs && !Upstaffs.StaffGiveAdmin)
          await Users.updateOne(
        ...
        if (logKanalı)
          logKanalı.send({
            embeds: [
              embed.setDescription(
                `${message.member} yöneticisi, ${uye} isimli üyeyi ${message.guild.roles.cache.get(rolBul.rol)} isimli rolüne yükseltti.`,
              ),
            ],
          });
        await Stats.updateOne(
        ...
        button
          .reply({
            content: `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye} isimli yetkili, \`${message.guild.roles.cache.get(rolBul.rol).name}\` rolüne yükseltildi.`,
            ephemeral: true,
          })
          .then((x) => {
            message.react(
      ...
      if (button.customId === "seç") {
        ...
        if (Upstaffs && rolBul) {
          ...
          message.member.Leaders("yetki", _statSystem.points.staff, {
            type: "STAFF",
            user: uye.id,
          });
        }
        if (!Upstaffs) {
          ...
          await Upstaff.updateOne(
            { _id: uye.id },
            { $set: { Baslama: Date.now() } },
            { upsert: true },
          );
          await Users.updateOne(
            { _id: uye.id },
            { $set: { Staff: true, StaffGiveAdmin: message.member.id } },
            { upsert: true },
          );
          await Users.updateOne(
            { _id: message.member.id },
            { $push: { Staffs: { id: uye.id, Date: Date.now() } } },
            { upsert: true },
          );
          ...
          if (yetkiliLog)
            yetkiliLog.send({
              embeds: [
                embed.setDescription(
                  `${uye} isimli üye \`${tarihsel(Date.now())}\` tarihinde ${message.author} tarafından yetkili olarak başlatıldı.`,
                ),
              ],
            });
        }
        ...
        uye.roles.cache
          .filter((x) => !checkRoles.includes(x.id))
          .map((x) => newRoles.push(x.id));
        if (rolBul && rolBul.rol) {
          newRoles.push(rolBul.rol);
          if (rolBul.exrol && rolBul.exrol.length >= 1)
        ...
        if (logKanalı)
          logKanalı.send({
            embeds: [
              embed.setDescription(
                `${message.member} yöneticisi, ${uye} isimli üyeyi ${message.guild.roles.cache.get(rolBul.rol)} isimli rolüne ekledi.`,
              ),
            ],
          });
        await Stats.updateOne(
        ...
        button
          .reply({
            content: `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye} isimli yetkili, \`${message.guild.roles.cache.get(rolBul.rol).name}\` rolüne eklendi.`,
            ephemeral: true,
          })
          .then((x) => {
            message.react(
