...
module.exports = {
  ...
  onLoad: function (client) {
    client.yetkiliSesSureCevir = (date) => {
    ...
    client.on("interactionCreate", async (modal) => {
      ...
      if (modal.customId == "myModal") {
        let bilgi = {
          ...
          periyod: modal.fields.getTextInputValue("t_selectInformationType")
            ? modal.fields.getTextInputValue("t_selectInformationType")
            : "unlimited",
        };

        if (bilgi && bilgi.rol && bilgi.periyod && bilgi.ödül) {
          let rol =
            guild.roles.cache.get(bilgi.rol) ||
            guild.roles.cache.find((x) => bilgi.rol.includes(x.name));
          if (!rol) {
            await modal.deferReply({ ephemeral: true });
            return await modal.followUp({
              content: `Belirtilen ID veya isimde bir rol bulunamadı.`,
              ephemeral: true,
            });
          }
          ...
          await modal.reply({
            components: [Row],
            embeds: [
              new genEmbed().setDescription(`**Merhaba!** ${uye.user.username}
Şimdi sırada ${rol} görevine gereksinim eklemede. Ekledikten sonra senin için görevleri dağıtacağım. Cevap vermen için "**30 Saniye**" süren var.`),
            ],
            ephemeral: true,
          });
          var filter = (i) => i.user.id == uye.id;
          let collector = modal.channel.createMessageComponentCollector({
            filter: filter,
            max: 1,
            time: 30000,
          });
          collector.on("collect", async (i) => {
              ...
              await modal.editReply({
                ...
                embeds: [
                  new genEmbed().setDescription(
                    `Görev gerekesinim menüsü açıldı işleme burdan devam edilecek. ${i.guild.emojiGöster(emojiler.Onay)}`,
                  ),
                ],
                ephemeral: true,
              });
            ...
            if (i.customId == "iptalEt") {
              ...
              await modal.editReply({
                ...
                embeds: [
                  new genEmbed().setDescription(
                    `Görev oluşturma/güncelleme işlemi başarıyla sonlandırıldı. ${i.guild.emojiGöster(emojiler.Onay)}`,
                  ),
                ],
                ephemeral: true,
              });
          ...
          collector.on("end", async (collected, reason) => {
            if (reason == "time") {
              await modal.editReply({
                ...
                embeds: [
                  new genEmbed().setDescription(
                    `İşleminiz zaman aşımı nedeniyle sonlandırıldı. Daha sonra tekrar deneyin!`,
                  ),
                ],
                ephemeral: true,
              });
      ...
      if (modal.customId == "takachiKe") {
        ...
        if (!rol) {
          ...
          await modal.deferReply({ ephemeral: true });
          return await modal.followUp({
            content: `Rol bulunamadığından işlem iptal edildi.`,
            ephemeral: true,
          });
        }
        ...
        await modal.reply({
          components: [Row],
          embeds: [
            new genEmbed().setDescription(`**Rol Adı**: ${rol.name} (${rol})
...
Şimdi sırada ${rol} görevine eklediğin gereksinimleri onaylaman gerek. Onaylaman için "**30 Saniye**" süren var.`),
        ...
        collector.on("collect", async (i) => {
          if (i.customId == "görevEkle") {
            await modal.editReply({
              ...
              embeds: [
                new genEmbed().setDescription(
                  `Başarıyla ${rol} görevi verildi/güncelledi ve dağıtım ${rol.members.size} kişi üzerine gerçekleşti. ${i.guild.emojiGöster(emojiler.Onay)}`,
                ),
              ],
              ephemeral: true,
            });
            let görevData = await Tasks.findOne({
              Active: true,
              roleID: rol.id,
            });
            if (görevData) {
            ...
            if (görevBilgilendirme)
              görevBilgilendirme.send({
                ...
                embeds: [
                  new genEmbed()
                    .setFooter({
                      text: `${sistem.botSettings.Prefixs[0]}görev komutu ile verilen görevlerinizi listeleyebilirsiniz.`,
                    })
                    .setTitle(
          ...
          if (i.customId == "iptalEt") {
            ...
            await modal.editReply({
              ...
              embeds: [
                new genEmbed().setDescription(
                  `Görev oluşturma/güncelleme işlemi başarıyla iptal edildi. ${i.guild.emojiGöster(emojiler.Onay)}`,
                ),
              ],
              ephemeral: true,
            });
      ...
      if (modal.customId == "deleteTasks") {
        ...
        let belirle =
          guild.roles.cache.get(getInput) || guild.members.cache.get(getInput);

        if (!belirle) {
          await modal.deferReply({ ephemeral: true });
          return await modal.followUp({
            content: `Rol veya üyelerde bulunamadığından işlem iptal edildi.`,
            ephemeral: true,
          });
        }
        ...
        if (guild.roles.cache.get(getInput)) {
          ...
          if (!görevData) {
            await modal.deferReply({ ephemeral: true });
            return await modal.followUp({
              content: `Belirtilen role ait bir görev bulunamadı!`,
              ephemeral: true,
            });
          }
          if (logKanal)
            logKanal.send({
              embeds: [
                new genEmbed().setDescription(
                  `${belirle} rolününe ait olan ${uyeler
                    .map((x) => x)
                    .slice(0, 2)
                    .join(
                      ", ",
                    )} ${uyeler.size > 2 ? `ve ${uyeler.size - 2} daha fazlası...` : ""} üyeler(üyelerin) <t:${String(Date.now()).slice(0, 10)}:R> ${uye} tarafından tüm görev bilgileri temizlendi.`,
                ),
              ],
            });
          await modal.reply({
            ...
            embeds: [
              new genEmbed().setDescription(
                `${uyeler
                  ...
                  .join(
                    ", ",
                  )} ${uyeler.size > 2 ? `ve ${uyeler.size - 2} daha fazlası...` : ""} üyeler(üyelerin) \`${belirle.name}\` rolüne ait görev bilgileri temizleniyor...`,
              ),
            ],
          });
          setTimeout(async () => {
            ...
            await modal.editReply({
              embeds: [
                new genEmbed().setDescription(
                  `Başarıyla ${belirle} rolüne ait tüm görev verileri temizlendi ve kaldırıldı. ${guild.emojiGöster(emojiler.Onay)}`,
                ),
              ],
              ephemeral: true,
            });
          ...
          let görevData =
            (await Tasks.findOne({
              roleID: belirle.roles.hoist ? belirle.roles.hoist.id : 0,
            })) ||
            (await Tasks.findOne({
              roleID: belirle.roles.highest ? belirle.roles.highest.id : 0,
            }));
          if (!görevData) {
            await modal.deferReply({ ephemeral: true });
            return await modal.followUp({
              content: `Belirtilen üyeye ait bir görev bulunamadı!`,
              ephemeral: true,
            });
          }
          ...
          await Stats.updateOne(
            { guildID: sistem.SERVER.ID, userID: belirle.id },
            { $set: { taskVoiceStats: new Map() } },
            { upsert: true },
          );
          if (logKanal)
            await logKanal.send({
              embeds: [
                new genEmbed().setDescription(
                  `${guild.emojiGöster(emojiler.Onay)} ${belirle} üyesinin <t:${String(Date.now()).slice(0, 10)}:R> ${uye} tarafından tüm görevleri temizlendi.`,
                ),
              ],
            });
          await modal.reply({
            ...
            embeds: [
              new genEmbed().setDescription(
                `${guild.emojiGöster(emojiler.Onay)} Şuan da ${belirle} üyesine ait görev verileri temizleniyor...`,
              ),
            ],
          });
          setTimeout(async () => {
            await modal.editReply({
              embeds: [
                new genEmbed().setDescription(
                  `Başarıyla ${belirle} üyesine ait tüm görev verileri temizlendi. ${guild.emojiGöster(emojiler.Onay)}`,
                ),
              ],
              ephemeral: true,
            });
  ...
  onRequest: async function (client, message, args) {
    ...
    message
      .reply({
        embeds: [
          new genEmbed().setDescription(
            `Aşağıda bulunan düğmeler ile görev sistemine ekleme, güncelleme ve silme işlemlerini gerçekleştirebilirsin.`,
          ),
        ],
        components: [Row],
      })
      .then(async (msg) => {
        ...
        collector.on("collect", async (i) => {
              ...
              .addComponents(
                new ActionRowBuilder().addComponents(
                  new TextInputBuilder()
                    ...
                    .setStyle(TextInputStyle.Short)
                    .setPlaceholder(
                      "Silincek görev rolü veya temizlencek üye bilgisi girin.",
                    )
                    .setRequired(true),
                ),
              );
          ...
          if (i.customId === "viewTasks") {
            const onayEmoji = message.guild.emojis.cache.find(
              (e) => e.name === emojiler.Onay,
            );
            message
              .react(onayEmoji ? onayEmoji.id : undefined)
              .catch((err) => {});
            ...
            await Tasks.find({}).exec(async (err, res) => {
              if (err)
                return message.reply({
                  content: "Hata: `Bazı hatalar oluştu :(`",
                });
              if (!res)
                return message
                  .reply({
                    content: `${ayarlar.serverName} sunucusuna ait görev bilgisi veritabanında bulunamadı.`,
                  })
                  .then((msg) => {
                    setTimeout(() => {
              ...
              message.channel.send({
                content: `Aşağıda **${ayarlar.serverName}** sunucusuna ait aktif görevler listelenmektedir.`,
              });
              ...
              .addComponents(
                ...
                new ActionRowBuilder().addComponents(
                  new TextInputBuilder()
                    ...
                    .setStyle(TextInputStyle.Short)
                    .setPlaceholder(
                      `${ayarlar.serverName} Parası ödülü belirleyin.`,
                    )
                    .setRequired(true),
                ),
                new ActionRowBuilder().addComponents(
                ...
                new ActionRowBuilder().addComponents(
                  new TextInputBuilder()
                    ...
                    .setStyle(TextInputStyle.Short)
                    .setPlaceholder(
                      "Görev süresini seçmelisin. Sınırsız için boş bırakın. Örn: 1d",
                    )
                    .setRequired(false),
                ),
              );
