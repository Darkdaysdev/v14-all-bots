...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if (kontrol && kontrol.Tagged)
      return message
        .reply(
          `${cevaplar.prefix} ${uye} isimli üye zaten bir başkası tarafından taglı olarak belirlenmiş.`,
        )
        .then((x) => {
          setTimeout(() => {
            x.delete().catch((err) => {});
    ...
    embed.setDescription(
      `${message.member.toString()} isimli yetkili seni taglı olarak belirlemek istiyor. Kabul ediyor musun?`,
    );
        ...
        collector.on("collect", async (i) => {
          if (i.customId == "OK") {
            ...
            await Users.updateOne(
              { _id: uye.id },
              { $set: { Tagged: true, TaggedGiveAdmin: message.member.id } },
              { upsert: true },
            );
            await Users.updateOne(
              { _id: message.member.id },
              { $push: { Taggeds: { id: uye.id, Date: Date.now() } } },
              { upsert: true },
            );
            ...
            message.channel
              .send({
                embeds: [
                  embed.setDescription(
                    `${message.guild.emojiGöster(emojiler.Onay)} ${uye.toString()} üyesi ${message.author} tarafından <t:${String(Date.now()).slice(0, 10)}:R> başarıyla taglı olarak belirledi!`,
                  ),
                ],
                components: [],
              })
            ...
            if (taglıLog)
              taglıLog.send({
                embeds: [
                  embed.setDescription(
                    `${uye} isimli üye <t:${String(Date.now()).slice(0, 10)}:R> ${message.author} tarafından taglı olarak belirlendi.`,
                  ),
                ],
              });
            ...
            message.member.Leaders("tag", _statSystem.points.tagged, {
              type: "TAGGED",
              user: uye.id,
            });
              ...
              .edit({
                ...
                embeds: [
                  new genEmbed()
                    .setColor("Red")
                    .setDescription(
                      `${message.guild.emojiGöster(emojiler.Iptal)} ${uye.toString()} isimli üye, **${message.guild.name}** taglı belirleme teklifini reddetti!`,
                    ),
                ],
                components: [],
              })
