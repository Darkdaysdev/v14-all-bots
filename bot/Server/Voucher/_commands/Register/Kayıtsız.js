...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if (getLimit.get(message.author.id) >= ayarlar.kayıtsızLimit)
      return message
      ...
      kayıtsızLog.send({
        embeds: [
          new genEmbed().setDescription(
            `${uye} isimli üye ${message.author} tarafından <t:${String(Date.now()).slice(0, 10)}:R> **${sebep}** nedeniyle \`${message.guild.name}\` sunucusunda kayıtsız üye olarak belirlendi.`,
          ),
        ],
      });
    message.reply({
      embeds: [
        new genEmbed().setDescription(
          `${message.guild.emojiGöster(emojiler.Onay)} ${uye} üyesi, **${sebep}** nedeniyle başarıyla kayıtsız'a gönderildi.`,
        ),
      ],
    });
    uye
      .send({
        embeds: [
          new genEmbed().setDescription(
            `${message.author} tarafından \`${sebep}\` sebebi ile <t:${String(Date.now()).slice(0, 10)}:R> kayıtsız'a atıldın.`,
          ),
        ],
      })
      .catch((x) => {});
