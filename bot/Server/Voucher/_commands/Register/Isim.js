...
module.exports = {
  Isim: "isim",
  Komut: ["i", "nick"],
  Kullanim: "isim <@takachi/ID> <İsim/Nick>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if (ayarlar.isimyas && !yas)
      return message
        .reply(cevaplar.argümandoldur)
        .then((s) => setTimeout(() => s.delete().catch((err) => {}), 5000));
    if (ayarlar.isimyas && yas < ayarlar.minYaş)
    ...
    uye
      ...
      .catch((err) => message.channel.send(cevaplar.isimapi));
    var filter = (msj) =>
      msj.author.id === message.author.id && msj.author.id !== client.user.id;
    let isimveri = (await Users.findById(uye.id)) || [];
    let isimler = isimveri.Names
      ...
      isimLog.send({
        embeds: [
          new genEmbed().setDescription(
            `${uye} isimli üyenin ismi ${message.member} tarafından \`${tarihsel(Date.now())}\` tarihinde "${ayarlar.isimyas ? `${isim} | ${yas}` : `${isim}`}" olarak güncellendi.`,
          ),
        ],
      });
    message
      .reply({
        embeds: [
          new genEmbed().setDescription(
            `${uye} üyesinin ismi "${ayarlar.isimyas ? `${isim} | ${yas}` : `${isim}`}" olarak değiştirildi${
              isimveri.Names
                ? isimveri.Names.length > 0
...
${isimler}\n\nÖnceki isimlerine \`${sistem.botSettings.Prefixs[0]}isimler <@takachi/ID>\` komutuyla bakarak kayıt işlemini\n gerçekleştirmeniz önerilir.`
                  : "."
                : "."
            }`,
          ),
