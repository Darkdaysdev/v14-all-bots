...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if (
      ...
      return message.channel.send(cevaplar.noyt).catch((err) => {});
    if (!uye)
      return message
        .reply(cevaplar.üye)
    ...
    let regPanel = await message.reply({
      embeds: [
        regPanelEmbed.setDescription(`Aşağıda ${uye} isimli üyenin cinsiyetini değiştirmek için düğmeler verilmiştir.
Tekrardan ses teyiti alarak kontrol etmenizi öneririz. Çünkü ses teyiti alırken ses teyiti alınacak üyenin cinsiyeti değişecektir.`),
    ...
    let isimler =
      isimveri.Name ||
      `${uye.displayName ? uye.displayName : uye.user.username}`;
          ...
          .edit({
            embeds: [
              new genEmbed().setDescription(
                `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye} isimli üye **Erkek** olarak cinsiyet değiştirildi.`,
              ),
            ],
            components: [],
          })
          ...
          { _id: uye.id },
          {
            $push: {
              Names: {
                ...
                Name: isimler,
                State: `Cinsiyet Değiştirme) (${roller.erkekRolleri.map((x) => uye.guild.roles.cache.get(x)).join(",")}`,
                Date: Date.now(),
              },
            },
          },
      ...
      if (i.customId === "lesbienaq") {
        await regPanel
          .edit({
            embeds: [
              new genEmbed().setDescription(
                `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla ${uye} isimli üye **Kadın** olarak cinsiyet değiştirildi.`,
              ),
            ],
            components: [],
          })
          ...
          { _id: uye.id },
          {
            $push: {
              Names: {
                Staff: message.member.id,
                Name: isimler,
                State: `Cinsiyet Değiştirme) (${roller.kadınRolleri.map((x) => uye.guild.roles.cache.get(x)).join(",")}`,
                Date: Date.now(),
              },
            },
          },
