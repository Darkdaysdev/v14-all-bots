...
module.exports = {
  ...
  Kullanim: "teyit <@takachi/ID>",
  Aciklama:
    "Belirtilen üye ve komutu kullanan üyenin teyit bilgilerini gösterir.",
  Kategori: "teyit",
  Extend: true,
  ...
  onRequest: async function (client, message, args) {
    ...
    if (teyit.Records) {
      ...
      let bazıları = teyit.Records.filter((v) =>
        message.guild.members.cache.get(v.User),
      ).map((value, index) => message.guild.members.cache.get(value.User));
      let çıkanlar = teyit.Records.filter(
        (v) => !message.guild.members.cache.get(v.User),
      ).length;
      let taglılar = teyit.Records.filter((x) => {
        let uye = message.guild.members.cache.get(x.User);
        return uye && uye.user.globalName.includes(ayarlar.tag);
      }).length;
