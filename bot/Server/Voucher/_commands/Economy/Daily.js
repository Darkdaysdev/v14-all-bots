  ...
  onRequest: async function (client, message, args) {
    ...
    if (Hesap && Hesap.Daily) {
      ...
      if (Date.now() < yeniGün) {
        ...
        return message
          .reply(
            `${message.guild.emojiGöster(emojiler.Iptal)} Tekrardan günlük ödül alabilmen için **${kalanzaman(yeniGün)}** beklemen gerekiyor.`,
          )
          .then((x) => {
            setTimeout(() => {
              x.delete();
    ...
    await Coins.updateOne(
      { _id: uye.id },
      { $set: { Daily: Date.now() }, $inc: { Coin: Günlük } },
      { upsert: true },
    );
    await message.react(
    ...
    await message
      .reply({
        content: `**Başarıyla günlük ödül aldınız!**
