...
module.exports = {
  ...
  Kullanim: "removebalance <Altın/Para> <@takachi/ID> <Miktar>",
  Aciklama: "",
  Kategori: "eco",
  Extend: true,
  ...
  onRequest: async function (client, message, args) {
    ...
    if (!args[0])
      return message
        .reply(
          `${cevaplar.prefix} Lütfen hangi birimden geri alacağını belirt. (Örn: \`${sistem.botSettings.Prefixs[0]}removebalance <Altın/Para> <@takachi/ID> <Miktar>\` )`,
        )
        .then((x) => {
          message.react(
    ...
    if (args[0] == "Para" || args[0] == "para") {
      ...
      if (!Gönderilen)
        return message
          .reply(`${cevaplar.prefix} Geri almak istediğiniz bi üyeyi belirtin.`)
          .then((x) => {
            message.react(
              message.guild.emojiGöster(emojiler.Iptal)
                ? message.guild.emojiGöster(emojiler.Iptal).id
      ...
      if (isNaN(Miktar))
        return message
          .reply(
            `${cevaplar.prefix} Geri almak istediğiniz miktarı rakam olarak girin.`,
          )
          .then((x) => {
            message.react(
              message.guild.emojiGöster(emojiler.Iptal)
      ...
      if (Miktar <= 0)
        return message
          .reply(
            `${cevaplar.prefix} Geri alınacak rakam birden küçük veya sıfır olamaz.`,
          )
          .then((x) => {
            message.react(
              message.guild.emojiGöster(emojiler.Iptal)
      ...
      await message.channel.send({
        embeds: [
          embed.setDescription(
            `${message.guild.emojiGöster(emojiler.Onay)} ${Gönderilen} üyesine başarıyla \`${Miktar}\` ${ayarlar.serverName} Parasını geri aldın.`,
          ),
        ],
      });
      return;
      ...
      await message.channel.send({
        embeds: [
          embed.setDescription(
            `${message.guild.emojiGöster(emojiler.Onay)} ${Gönderilen} üyesine başarıyla \`${Miktar}\` ${message.guild.emojiGöster(emojiler.Görev.Altın)} geri aldın.`,
          ),
        ],
      });
      return;
