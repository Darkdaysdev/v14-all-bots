...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    ürünlercik.forEach((data) => {
      arrayItems.push([
        {
          label: `${data.name} ( ${data.role ? `Rol: ${message.guild.roles.cache.get(data.roleID).name}` : data.desc} )`,
          description: `${data.coin > 0 ? data.coin.toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + ` ${ayarlar.serverName} Parası${data.gold > 0 ? " ve" : ""}` : ``} ${data.gold > 0 ? data.gold + ` Altın` : ``}${data.gold <= 0 && data.coin <= 0 ? `Ücretsiz Ürün` : ``}`,
          value: data.name,
          emoji: {
            id: message.guild.emojiGöster(data.emoji)
              ? message.guild.emojiGöster(data.emoji).id
              : "925127916348649512",
          },
        },
      ]);
      ...
      new StringSelectMenuBuilder()
        ...
        .addOptions(
          arrayItems.length > 0
            ...
            : {
                label: "Kapalı Mağaza",
                description: "Mağazamız kapalıdır!",
                value: "1233223423",
              },
        ),
    );
    ...
    if (
      ...
      if (args[0] == "sil") {
        if (!args[1])
          return message
            .reply("Ürün ismi girmediğin için iptal edildi.")
            .then((x) => {
              setTimeout(() => {
                x.delete();
              }, 7500);
        ...
        if (!ürünBul)
          return message
            .reply("Ürün bulunamadığından iptal edildi.")
            .then((x) => {
              setTimeout(() => {
                x.delete();
              }, 7500);
      ...
      if (args[0] == "ekle" || args[0] == "güncelle") {
        let isim = args[1];
        if (!isim)
          return message.reply("Lütfen bir ürün ismi giriniz.").then((x) => {
            setTimeout(() => {
              x.delete();
            }, 7500);
            message.react(
        ...
        if (!coin)
          return message
            .reply(
              `Lütfen ${ayarlar.serverName.toLowerCase()} parası belirleyin. **0 yaparsanız otomatik olarak ücretsiz yapacaktır!**`,
            )
            .then((x) => {
              setTimeout(() => {
                x.delete();
        ...
        if (!gold)
          return message
            .reply(
              `Lütfen altın belirleyin. **0 yaparsanız otomatik olarak ücretsiz yapacaktır!**`,
            )
            .then((x) => {
              setTimeout(() => {
                x.delete();
        ...
        if (!emoji)
          return message
            .reply(
              `Lütfen bir emoji belirleyin belirlemek istemiyorsanız **yok** yazın.`,
            )
            .then((x) => {
              setTimeout(() => {
                x.delete();
        ...
        if (!desc)
          return message.reply(`Lütfen bir açıklama girin.`).then((x) => {
            setTimeout(() => {
              x.delete();
            }, 7500);
            message.react(
        ...
        if (role) {
          ...
          await Shops.updateOne(
            { name: isim },
            { desc: desc, emoji: emoji, gold: gold, coin: coin },
            { upsert: true },
          );
        }
        message.react(
    ...
    let msg = await message.channel.send({
      content: `:tada: **${ayarlar.serverName} Mağazasına Hoşgeldiniz!** ${arrayItems.length > 0 ? "Aşağıda listelenmekte olan ürünlerimizden, almak istediğiniz ürünü seçebilirsiniz." : "Şuan da ürün bulunamadığından mağazamız kapalıdır!"}`,
    ...
    collector.on("collect", async (i) => {
      if (i.customId == "ürüncükal") {
        ...
        if (!ürünBilgisi)
          return i.reply({
            content: `${message.guild.emojiGöster(emojiler.Iptal)} **Başarısız!** böyle bir ürün bulunamadı, lütfen ürünü seçin!`,
            ephemeral: true,
          });
        ...
        if (coin > 0 && para < coin)
          return i.reply({
            content: `${message.guild.emojiGöster(emojiler.Iptal)} **Başarısız!** Gereken ${coin} ${ayarlar.serverName} Parası bulunamadığından satın alamazsın!`,
            ephemeral: true,
          });
        if (gold > 0 && altın < gold)
          return i.reply({
            content: `${message.guild.emojiGöster(emojiler.Iptal)} **Başarısız!** Gereken ${gold} Altın bulunamadığından satın alamazsın!`,
            ephemeral: true,
          });
        ...
        if (dattaaa && dattaaa.Inventory) {
          ...
          if (ürünlerrrrr.includes(isim))
            return (
              msg.delete().catch((err) => {}),
              i.reply({
                content: `${message.guild.emojiGöster(emojiler.Iptal)} **Başarısız!** \`${isim}\` isimli ürün envanterinde bulunduğundan dolayı tekrardan satın alamazsın.`,
                ephemeral: true,
              }),
              message
                .react(
        ...
        if (tip == "badge")
          Users.updateOne(
            { _id: uye.id },
            { $push: { Badges: { Name: isim, Emoji: emoji } } },
            { upsert: true },
          );
        await Users.updateOne(
          { _id: uye.id },
          { $push: { Inventory: alınanÜrün } },
          { upsert: true },
        );
        msg.delete().catch((err) => {});
          ...
          kanalBul.send({
            embeds: [
              new genEmbed().setDescription(
                `:tada: ${message.member} isimli üye tarafından **${alınanÜrün.Name}** isimli ürün \`${tarihsel(Date.now())}\` tarihinde satın alındı.`,
              ),
            ],
          });
        i.reply({
          embeds: [
            new genEmbed().setDescription(
              `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla **${isim}** isimli ürünü <t:${String(Date.now()).slice(0, 10)}:R> satın aldınız.`,
            ),
          ],
          ephemeral: true,
        });
