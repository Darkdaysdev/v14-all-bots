...
Canvas.registerFont(`../../Assets/fonts/SketchMatch.ttf`, {
  family: "SketchMatch",
});
Canvas.registerFont(`../../Assets/fonts/LuckiestGuy-Regular.ttf`, {
  family: "luckiest guy",
});
Canvas.registerFont(`../../Assets/fonts/KeepCalm-Medium.ttf`, {
  family: "KeepCalm",
});
...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let kullanici =
      ...
      client.guilds.cache.get(sistem.SERVER.ID).members.cache.get(args[0]) ||
      message.member;
    if (!kullanici)
      return message
        .reply(cevaplar.üye)
    ...
    let uye = client.guilds.cache
      ...
      .members.cache.get(kullanici.id);
    ...
    ctx.quadraticCurveTo(0 + 1080, 0 + 400, 0 + 1080 - Number(30), 0 + 400);
    ...
    let background = await Canvas.loadImage(
      client.guilds.cache.get(sistem.SERVER.ID).banner
        ? client.guilds.cache.get(sistem.SERVER.ID).banner
          ? client.guilds.cache
              .get(sistem.SERVER.ID)
              .bannerURL({ extension: "png" }) + `?size=4096`
    ...
    ctx.closePath();
    ctx.stroke();
    let gold = await Canvas.loadImage(
      "https://cdn.discordapp.com/emojis/998212095306903653.png?size=96&quality=lossless",
    );
    let coin = await Canvas.loadImage(
      "https://cdn.discordapp.com/emojis/998211961462464532.png?size=96&quality=lossless",
    );
    ...
    ctx.font = applyText(
      canvas,
      uye.user.username + " UYESININ HESABI",
      40,
      600,
      "Bold",
    );
    ...
    ctx.lineWidth = 3;
    ctx.strokeText(
      para.toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") +
        " " +
        ayarlar.serverName +
        " PARASI",
      canvas.width - 650,
    ...
    const avatar = await Canvas.loadImage(
      uye.user.avatar
        ? uye.user.avatarURL({ extension: "png" })
        : "https://cdn.discordapp.com/attachments/915702481776963615/928900758135509022/dd83hdp-ef7fc5cd-ac92-4ad1-adfc-8e2e933ce256.png",
    );
    ...
    message
      .reply({
        ...
        files: [attachment],
        components: message.member.id == uye.id ? [ekonomiPanel] : null,
      })
      .then(async (x) => {
        var filter = (i) => i.user.id == message.member.id;
        let collector = await x.createMessageComponentCollector({
          filter: filter,
          max: 1,
          time: 60000,
        });
...
async function ilkYazan(
  odül = Number(Math.floor(Math.random() * 10000) + 200),
) {
  ...
  ctx.font = applyText(
    canvas,
    `${odül} ${global.ayarlar.serverName.toUpperCase()} PARASI KAZANACAKSIN!`,
    45,
    690,
    "Bold",
  );
  ctx.fillText(
    `SEN YAZARSAN ${odül} ${global.ayarlar.serverName.toUpperCase()} PARASI KAZANACAKSIN!`,
    70,
    130 + 85,
  );
  ...
  const attachment = new AttachmentBuilder(canvas.toBuffer(), {
    name: "takachi_economy.png",
  });
  let msg = await kanal.send({
    content:
      "**Hızlı Ol ve Kazan**\nAşağıda rastgele verilmiş bir kodu doğru yaz ve kazan! Bunun için 20 Saniyeniz var!",
    files: [attachment],
    components: [],
  });

  const filter = (m) => m.content.includes(kod);
  const collector = kanal.createMessageCollector({
    filter,
    max: 1,
    time: 20000,
  });
  ...
  collector.on("collect", async (m) => {
    ...
    kanal
      .send({
        content: `**Tebrikler!** Hızlı Ol Ve Kazan etkinliğini ${uye} kazandı! ${m.guild.emojiGöster("a_rich")}`,
        files: [],
        files: [],
      })
      .then((x) =>
    ...
    if (uye)
      return await client.Economy.updateBalance(uye.id, Number(odül), "add", 1);
  });
...
async function tahminEt(
  odül = Number(Math.floor(Math.random() * 4000) + 1000),
) {
  let kanal = client.channels.cache.get(global.kanallar.chatKanalı);
  if (!kanal) return;
  ...
  let msg = await kanal.send({
    content:
      "**Hızlı Ol ve Kazan**\nAşağıda 1 ile 5 arası verilen rakamlardan doğru rakamı tahmin edin. Bunun için 30 Saniyeniz var!",
    files: [attachment],
    components: [rakamSatirBir],
  });
  ...
  collector.on("collect", async (i) => {
    if (i.customId === String(cevap)) {
      if (basanlar.includes(i.user.id))
        return await i.reply({
          content: `${i.guild.emojiGöster(emojiler.Iptal)} Cevap hakkınızı doldurmuşsunuz. Üzgünüm!`,
          ephemeral: true,
        });
      await i.reply({
        content: `> **Tebrikler!** Etkinliği kazandığınız için hesabınıza +**${odül}** ${global.ayarlar.serverName} Parası aktarıldı.`,
        ephemeral: true,
      });
      let uye = i.guild.members.cache.get(i.user.id);
      kanal
        .send({
          content: `**Tebrikler!** Hızlı Ol Ve Kazan etkinliğini ${uye} kazandı! ${i.guild.emojiGöster("a_rich")}`,
          files: [],
        })
        .then((x) =>
          setTimeout(() => {
      ...
      basanlar = [];
      if (uye)
        return await client.Economy.updateBalance(
          uye.id,
          Number(odül),
          "add",
          1,
    ...
    if (i.customId != String(cevap)) {
      ...
      await i.reply({
        content: `${i.guild.emojiGöster(emojiler.Iptal)} **Hay Aksi!** Yanlış, artık birdahaki sorulara. Cevap hakkınız doldu!`,
        ephemeral: true,
      });
    }
  });
...
async function matematikOyunu(
  odül = Number(Math.floor(Math.random() * 2000) + 500),
) {
  let kanal = client.channels.cache.get(global.kanallar.chatKanalı);
  if (!kanal) return;
  ...
  let buttons = [
    new ButtonBuilder()
      .setCustomId("qwe")
      .setLabel(`${Math.floor(Math.random() * 550)}`)
      .setStyle(ButtonStyle.Success),
    new ButtonBuilder()
      .setCustomId("qwe1")
      .setLabel(`${Math.floor(Math.random() * 942) + 1}`)
      .setStyle(ButtonStyle.Success),
    new ButtonBuilder()
      .setCustomId("qwe2")
      .setLabel(`${Math.floor(Math.random() * 250)}`)
      .setStyle(ButtonStyle.Success),
    new ButtonBuilder()
      .setCustomId(`${cevap}`)
      .setLabel(`${cevap}`)
    ...
    new ButtonBuilder()
      .setCustomId("qwe6")
      .setLabel(`${Math.floor(Math.random() * 2000)}`)
      .setStyle(ButtonStyle.Success),
  ];
  ...
  let msg = await kanal.send({
    content:
      "**Cevabı Bul!**\nAşağıda sorunun cevabını doğru cevaplayın. Bunun için 30 Saniyeniz var!",
    files: [attachment],
    components: [rakamSatirBir],
  });
  ...
  collector.on("collect", async (i) => {
    if (i.customId === String(cevap)) {
      ...
      kanal
        .send({
          content: `**Tebrikler!** Cevabı bul etkinliğini ${uye} kazandı! ${i.guild.emojiGöster("a_rich")}`,
          files: [],
        })
        .then((x) =>
          setTimeout(() => {
...
async function doğruKasaBul(
  odül = Number(Math.floor(Math.random() * 100000) + 3000),
) {
  let kanal = client.channels.cache.get(global.kanallar.chatKanalı);
  if (!kanal) return;
  let cevap = Math.floor(Math.random() * 5) + 1;
  ...
  ctx.fillText(
    `${global.ayarlar.serverName.toUpperCase()} KASALARININ SANA SUPRIZI VAR!`,
    180,
    180 + 85,
  );
  ...
  let msg = await kanal.send({
    content:
      "**Doğru Kasayı Bul!**\nAşağıda düğmelerde verilen kasalardan doğru kasayı bulunuz. Bunun için 30 Saniyeniz var!",
    files: [attachment],
    components: [rakamSatirBir],
  });
  ...
  collector.on("collect", async (i) => {
    if (i.customId === String(cevap)) {
      ...
      await i.reply({
        content: `> **Tebrikler!** Etkinliği kazandığınız için hesabınıza +**${odül}** ${global.ayarlar.serverName} Parası aktarıldı. ${i.guild.emojiGöster("a_rich")}`,
        ephemeral: true,
      });
      let uye = i.guild.members.cache.get(i.user.id);
      kanal
        .send({
          content: `**Tebrikler!** Doğru kasayı bul etkinliğini ${uye} kazandı! ${i.guild.emojiGöster("a_rich")}`,
          files: [],
        })
        .then((x) =>
          setTimeout(() => {
