...
module.exports = {
  ...
  Kullanim: "addbalance <Altın/Para> <@takachi/ID> <Miktar>",
  Aciklama: "",
  Kategori: "eco",
  Extend: true,
  ...
  onRequest: async function (client, message, args) {
    ...
    if (!args[0])
      return message
        .reply(
          `${cevaplar.prefix} Lütfen hangi birimden göndereceğini belirt. (Örn: \`${sistem.botSettings.Prefixs[0]}addbalance <Altın/Para> <@takachi/ID> <Miktar>\` )`,
        )
        .then((x) => {
          message.react(
      ...
      await message.channel.send({
        embeds: [
          embed.setDescription(
            `${message.guild.emojiGöster(emojiler.Onay)} ${Gönderilen} üyesine başarıyla \`${Miktar}\` ${ayarlar.serverName} Parası gönderdin.`,
          ),
        ],
      });
      return;
