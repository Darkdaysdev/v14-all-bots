...
const { takachi } = require("../../../../Global/Clients/Global.Clients");
module.exports = {
  Isim: "bozdur",
...
${
  client.eskiDovizAltın < client.dovizAltın
    ? `:chart: **Altının** değeri güncellendi! **${client.eskiDovizAltın} ${ayarlar.serverName}** Parasından, **${client.dovizAltın} ${ayarlar.serverName}** Parasına Yükseldi!
`
...
**Hemen Altınlarını Çevir!** Çevirmek için ${kanallar.coinChat.map((x) => guild.channels.cache.get(x))} kanalından \`${sistem.botSettings.Prefixs[0]}doviz\` komutunu kullanabilir ve çevirdiğin altını anında harcamak için oyun komutlarımıza "\`${sistem.botSettings.Prefixs[0]}yardım\` > Ekonomi Komutları" kısmından bakabilirsiniz.`);
  ...
  onRequest: async function (client, message, args) {
    if (bugDeneme.get(message.member.id))
      return message
        .reply({
          content: `Şuanda aktif bir "Doviz" panelizin açık! ${cevaplar.prefix}`,
        })
        .then((x) => {
          message
            .react(
    ...
    let Row2 = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        ...
        .setLabel(
          altın < Paracık
            ? "Yetersiz Altın"
            : `1 Altın 💱 ${client.dovizAltın} ${ayarlar.serverName} Parası`,
        )
      ...
      new ButtonBuilder()
        ...
        .setLabel(
          altın < Paracık ? "Yetersiz Altın" : `Tüm Altını Paraya Çevir!`,
        )
        .setDisabled(altın < Paracık ? true : false)
    ...
    let Row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        ...
        .setLabel(
          para < Altıncık
            ? "Yetersiz Para"
            : `${client.dovizAltın} ${ayarlar.serverName} Parası 💱 1 Altın`,
        )
      ...
      new ButtonBuilder()
        ...
        .setLabel(
          para < Altıncık ? "Yetersiz Para" : `Tüm Paranı Altına Çevir!`,
        )
        .setDisabled(para < Altıncık ? true : false)
        .setEmoji(message.guild.emojiGöster(emojiler.Görev.Altın).id)
        .setStyle(para < Altıncık ? ButtonStyle.Danger : ButtonStyle.Secondary),
      ...
      .reply({
        ...
        embeds: [
          new genEmbed().setAuthor({
            ...
            .setDescription(`**Merhaba!** __${ayarlar.serverName}__ Doviz işlemleri menüsüne hoş geldiniz.
        ...
        collector.on("collect", async (i) => {
          if (i.customId == "para") {
            if (zaman.get(message.author.id) >= 1)
              return (
                ...
                i.reply({
                  content: `${cevaplar.prefix} Doviz işlemleri sadece **15 Saniye** aralığla yapılabilir. **Lütfen Daha Sonra Tekrar Deneyin!**`,
                  ephemeral: true,
                })
              );
            ...
            await message.reply({
              embeds: [
                embed.setDescription(
                  `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla \`${Altıncık} ${ayarlar.serverName} Parası => ${Paracık.toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")} Altın\` olarak doviz kuru tarafıyla dönüştürüldü.`,
                ),
              ],
            });
          ...
          if (i.customId == "gold") {
            ...
            await Coins.updateOne(
              { _id: uye.id },
              {
                $push: {
                  Transfers: {
                    Uye: uye.id,
                    Tutar: Number(Altıncık),
                    Tarih: Date.now(),
                    Islem: "Para (Dönüştürülen Altın)",
                  },
                },
              },
              { upsert: true },
            ...
            await message.reply({
              embeds: [
                embed.setDescription(
                  `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla \`${Paracık.toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")} Altın => ${Altıncık} ${ayarlar.serverName} Parası\` olarak doviz kuru tarafıyla dönüştürüldü.`,
                ),
              ],
            });
            zaman.set(message.author.id, zaman.get(message.author.id) || 1);
          ...
          if (i.customId == "topluPara") {
              ...
              { _id: uye.id },
              {
                $push: {
                  Transfers: {
                    ...
                    Tutar: verilcekPara,
                    Tarih: Date.now(),
                    Islem: `Para (Toplu Doviz) [${alıncakAltın} Altın]`,
                  },
                },
              },
              { upsert: true },
            ...
            await message.reply({
              embeds: [
                embed.setDescription(
                  `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla \`${altın.toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")} Altın => ${verilcekPara} ${ayarlar.serverName} Parası\` olarak doviz kuru tarafıyla dönüştürüldü.`,
                ),
              ],
            });
            zaman.set(message.author.id, zaman.get(message.author.id) || 1);
          ...
          if (i.customId == "topluAltın") {
              ...
              { _id: uye.id },
              {
                $push: {
                  Transfers: {
                    ...
                    Tutar: verilcekPara,
                    Tarih: Date.now(),
                    Islem: `Altın (Toplu Doviz) [${alıncakAltın} Para]`,
                  },
                },
              },
              { upsert: true },
            ...
            await message.reply({
              embeds: [
                embed.setDescription(
                  `${message.guild.emojiGöster(emojiler.Onay)} Başarıyla \`${para.toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")} ${ayarlar.serverName} Parası => ${verilcekPara} Altın\` olarak doviz kuru tarafıyla dönüştürüldü.`,
                ),
              ],
            });
            zaman.set(message.author.id, zaman.get(message.author.id) || 1);
