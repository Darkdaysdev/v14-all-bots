...
module.exports = async (oldEmoji, newEmoji) => {
  ...
  let entry = await newEmoji.guild
    .fetchAuditLogs({ type: AuditLogEvent.EmojiUpdate })
    .then((audit) => audit.entries.first());
  if (
    !entry ||
    !entry.executor ||
    entry.createdTimestamp <= Date.now() - 5000 ||
    (await client.checkMember(entry.executor.id, "emoji", "Emoji Düzenleme!"))
  )
    return;
  client.punitivesAdd(entry.executor.id, "jail");
  client.allPermissionClose();
  embed.setDescription(
    `${entry.executor} (\`${entry.executor.id}\`) tarafından (${newEmoji.guild.emojis.cache.get(oldEmoji.id)}) \`${oldEmoji.name}\` isimli emojiyi \`${newEmoji.name}\` olarak güncellediği eski haline getirilerek cezalandırıldı.`,
  );
  await newEmoji.edit(
    { name: oldEmoji.name },
    `${entry.executor.username} tarafından güncellenmeye çalışıldı.`,
  );
