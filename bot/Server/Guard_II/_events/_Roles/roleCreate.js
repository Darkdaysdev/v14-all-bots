...
module.exports = async (role) => {
  ...
  if (
    ...
    (await client.checkMember(entry.executor.id, "roles", "Rol Oluşturma!"))
  )
    return;
  client.punitivesAdd(entry.executor.id, "ban");
  client.allPermissionClose();
  embed.setDescription(
    `${entry.executor} (__${entry.executor.id}__) tarafından bir rol oluşturuldu! Oluşturan kişi yasaklandı ve rol silindi.`,
  );
  await role.delete();
