...
module.exports = async (role) => {
  ...
  if (!entry || !entry.executor || entry.createdTimestamp <= Date.now() - 5000)
    return;
  await deletedRoles.updateOne(
    { roleID: role.id },
    { $set: { Date: Date.now(), Remover: entry.executor.id } },
    { upsert: true },
  );
  embed.setDescription(
    `${entry.executor} (__${entry.executor.id}__) tarafından ${role.name} (__${role.id}__) rolü silindi. \`${sistem.botSettings.Prefixs[0]}rolkur ${role.id}\` komutu ile kurulum yapabilirsiniz.`,
  );
