...
module.exports = async (oldRole, newRole) => {
  ...
  let entry = await oldRole.guild
    .fetchAuditLogs({ type: Discord.AuditLogEvent.RoleUpdate })
    .then((audit) => audit.entries.first());
  if (
    !entry ||
    !entry.executor ||
    entry.createdTimestamp <= Date.now() - 5000 ||
    (await client.checkMember(entry.executor.id, "roles", "Rol Düzenleme!"))
  )
    return;
  ...
  embed.setDescription(
    `${entry.executor} (__${entry.executor.id}__) tarafından **${oldRole.name}** rolü güncellendi! Güncelleyen kişi yasaklandı ve rol eski haline getirildi.`,
  );
