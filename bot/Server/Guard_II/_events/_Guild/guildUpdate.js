...
module.exports = async (oldGuild, newGuild) => {
  ...
  let entry = await newGuild
    .fetchAuditLogs({ type: AuditLogEvent.GuildUpdate })
  ...
  if (
    newGuild.iconURL({ extension: "png", size: 2048 }) !==
    oldGuild.iconURL({ extension: "png", size: 2048 })
  )
    await newGuild.setIcon(oldGuild.iconURL({ extension: "png", size: 2048 }));
  ...
  embed.setDescription(
    ` ${entry.executor} (\`${entry.executor.id}\`) tarafından sunucudan sunucu güncellendi! Güncelleyen kişi banlandı ve sunucu eski haline getirildi.`,
  );
