...
module.exports = async (ban) => {
  ...
  await ban.guild.members
    .unban(ban.user.id, "Sağ Tık İle Banlandığı İçin Geri Açıldı!")
    .catch(console.error);
  embed.setDescription(
    `${ban.user} (\`${ban.user.id}\`) üyesi, ${entry.executor} (\`${entry.executor.id}\`) tarafından sunucudan \`Sağ-Tık\` ile yasaklandı! yasaklayan kişi ise yasaklandı.`,
  );
