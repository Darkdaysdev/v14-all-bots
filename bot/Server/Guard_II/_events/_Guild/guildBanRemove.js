...
module.exports = async (ban) => {
  ...
  await ban.guild.members.ban(ban.user.id, {
    reason: "Yasaklaması Kaldırıldığından Dolayı Tekrar Yasaklandı.",
  });
  embed.setDescription(
    `${ban.user} (\`${ban.user.id}\`) üyesinin yasaklaması, ${entry.executor} (\`${entry.executor.id}\`) tarafından kaldırıldığı için, kaldıran kişi cezalandırılıp kaldırdığı üye tekrar banlandı.`,
  );
