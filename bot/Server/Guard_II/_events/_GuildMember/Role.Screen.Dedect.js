...
module.exports = async (oldPresence, newPresence) => {
  ...
  let memberSafeRoles = uye.roles.cache.filter(
    (e) =>
      e.editable &&
      e.name !== "@everyone" &&
      Permissions.some((a) => e.permissions.has(a)),
  );
  if (memberSafeRoles)
  ...
  if (
    ...
    Permissions.some((x) => uye.permissions.has(x))
  ) {
    if (await client.checkMember(uye.id)) return;
    await Roles.updateOne(
      { _id: uye.id },
      { $set: { Roles: arr, Reason: "Çevrimdışı" } },
      { upsert: true },
    );
    if (arr && arr.length >= 1)
      await uye.roles
        .remove(arr, `Çevrimdışı alındığından dolayı yetkisi çekildi.`)
        .catch((err) => {});
    embed
      .setFooter({ text: "Aktif olduğunda yetkisi tekrardan verilecektir." })
      .setTitle("Sunucuda Bir Yönetici Çevrim-Dışı Aldı!")
      .setDescription(`${uye} (\`${uye.id}\`) isimli yönetici Çevrim-Dışı oldu özel mesajları kapalı olduğundan dolayı bu kanala bilgilendirme atıldı.\n\`\`\`fix
Üzerinden Alınan Roller \`\`\`\
${
  arr.length >= 1
    ? `\` ••❯ \` Çekilen Roller: ${arr
        .filter((x) => uye.guild.roles.cache.get(x))
        .map((x) => uye.guild.roles.cache.get(x))
        .join(", ")}`
    : `\` ••❯ \` Üzerinden herhangi bir rol alınmadı.`
} `);
    ...
    if (Data && Data.Roles && Data.Roles.length) {
      if (Data.Roles)
        uye.roles
          .add(
            Data.Roles,
            `Aktif olduğundan dolayı çekilen yetkileri tekrardan verildi.`,
          )
          .catch((err) => {});
...
client.on("presenceUpdate", async (oldPresence, newPresence) => {
  ...
  if (CheckWeb && Permissions.some((x) => uye.permissions.has(x))) {
    ...
    await Roles.updateOne(
      { _id: uye.id },
      { $set: { Roles: arr, Reason: "Web tarayıcı girişi için kaldırıldı." } },
      { upsert: true },
    );
    if (arr && arr.length >= 1)
      await uye.roles
        .remove(arr, `Web üzerinden sunucuyu görüntülediği için.`)
        .catch((err) => {});
    embed.setTitle("Bir Yönetici Sunucuya Webden Giriş Sağladı!")
 ...
 ${
   arr.length >= 1
     ? `\` ••❯ \` Çekilen Roller: ${arr
         .filter((x) => uye.guild.roles.cache.get(x))
         .map((x) => uye.guild.roles.cache.get(x).name)
         .join(", ")}`
     : `\` ••❯ \` Üzerinden herhangi bir rol alınmadı.`
 } `);
    let loged = newPresence.guild.kanalBul("guard-log");
        ...
        .then(async (msg) => {
          ...
          collector.on("collect", async (i) => {
            if (i.customId == "ver") {
              ...
              if (Data && Data.Roles && Data.Roles.length) {
                i.reply({
                  content: `${uye.guild.emojiGöster(emojiler.Onay)} ${uye}, üyesinin çekilen rolleri başarıyla geri verildi.`,
                  ephemeral: true,
                });
                if (Data.Roles)
                  uye.roles
                    .add(
                      Data.Roles,
                      `${i.user.username} tarafından tekrardan verildi.`,
                    )
                    .catch((err) => {});
                await Roles.findByIdAndDelete(uye.id);
              } else {
                i.reply({
                  content: `${uye.guild.emojiGöster(emojiler.Iptal)} ${uye}, üyesinin rolleri veritabanında bulunamadığından işlem sonlandırıldı.`,
                  ephemeral: true,
                });
              }
            }
